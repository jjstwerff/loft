...
0 – 4\_294\_967\_294, storage promotes to i64 (8 bytes) under Phase 2a's "wide-limit → Long" rule so values up to u32::MAX round-trip.  One sentinel (`i64::MIN`) reserved for null — `u32 not null` when the full 2^32 range is needed.  RGBA pixel math, large file offsets, and bitmasks wider than i32 all fit here.
