...
- [`doc/claude/plans/README.md`](../../../doc/claude/plans/README.md) — docs-vs-plans rule, value categories (S/R/G/F/U/C/Q/N), closure rule, roadmap workflow
- [`doc/claude/plans/_TEMPLATE.md`](../../../doc/claude/plans/_TEMPLATE.md) — canonical new-plan README shape
...
## When to use a plan vs not

| Work shape | Path |
|---|---|
| Bug fix (single root cause, fits in one commit) | PROBLEMS.md row + regression test + commit.  No plan. |
| Tiny deliverable (demo deploy, version bump) | ROADMAP row only.  No plan. |
| Feature work (multi-step, design choices, multiple files) | **Plan required**, even if small. |
| Internal refactor with multiple phases | Plan required. |
| Operational change (CI tweak, doc fix) | Direct commit, no ROADMAP row needed. |

When in doubt: features go through plans.
...
2. Decide destination based on audit:
   ...
   - Mostly trigger-deferred → `plans/deferred/`
   - Genuinely future work → `plans/future/` or `lib_plans/future/`
   - Reference content + open-tail → **leave doc at root**, create thin pointer-plan that links back
   - **NEW: pointer-plan trial result (2026-05-09)**: pointer-plans were over-engineering for QUALITY/NATIVE/PERFORMANCE/PACKAGES.  Prefer instead: add an `## Open work` section IN the reference doc.  Single source of truth, no indirection.
