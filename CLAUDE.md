...
| [GOALS.md](doc/claude/GOALS.md) | What "loft stable" means, made concrete — the 5 goals (A soundness / B release & adoption / C capability via dogfood / D cross-platform parity / **E predictable memory** — the programmer's model is the truth, freeing at scope end, enforced by `LOFT_STORE_GUARD`; the "surpass Rust on safe-AND-predictable" aim), each with a runnable **Check** + honest status; the two-engine (dogfood + sanitizer) model |
