...
| [PACKAGES.md](doc/claude/PACKAGES.md) | Reference — package format spec (`loft.toml`), package layout, function binding model, build pipeline, target matrix (interpreter / native / WASM), OpenGL case study, security model.  Format is SHIPPED: 6 `lib/*` packages already use it.  Open follow-up work (registry MVP, lock file, library extraction) tracked in [lib_plans/future/11-packages/](doc/claude/lib_plans/future/11-packages/README.md). |
