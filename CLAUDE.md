...
| [CODEGEN_METHOD.md](doc/claude/CODEGEN_METHOD.md) | **How to do compiler work**: code-gen is the WRONG place for complexity — any complex work there is a DIAGNOSTIC of a type-system flaw; fix the type, never the codegen. Codegen is a mechanical translation TYPES → bytecode. Build bottom-up per scale (bytecode→types→code), working-vs-broken bytecode proven first on both backends. The whole of loft moves onto this method, plan by plan |
