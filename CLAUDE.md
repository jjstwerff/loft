...
| [SANDBOX.md](doc/claude/SANDBOX.md) | Letting users run scripts without breaking the host (player playgrounds / game mods): admission-time validation (capability/library/loop/recursion limits) + effect-containment (transactional store) + fault-isolation, each pinned to a runnable Check. **Admission half BUILT + CLI-enforced ([@PLN86](doc/claude/plans/86-sandbox-subset-flag/README.md)) — S1–S5 GREEN, S7/S8 partial**; transactional-world + `run_script` boundary are the post-v1 runtime pieces |
