...
| [QUALITY.md](doc/claude/QUALITY.md) | Reference — open programmer-biting issues, active sprint (P54 JsonValue enum), active designs (Q1-Q4 JSON ecosystem, P54-U unified parser, Dep-inference for native fn returns), compiler blockers (B2-B7 struct-enum bugs), enhancement tiers, recommended landing order.  C54 (integer→i64) historical record kept as the canonical "LANDED via …" closure pattern.  Active items tracked as a pointer-plan in [plans/future/35-quality-followups/](doc/claude/plans/future/35-quality-followups/README.md). |
...
| [NATIVE.md](doc/claude/NATIVE.md) | Reference — native code generation pipeline (`src/generation/`), architecture + `codegen_runtime`, per-Op dispatch, N1-N8 implementation history.  `--native` is shipped (CI-gated, 108/108 native tests pass).  Open follow-up work tracked in [plans/future/33-native-codegen-followups/](doc/claude/plans/future/33-native-codegen-followups/README.md). |
...
| [lib_plans/future/09-lsp/README.md](doc/claude/lib_plans/future/09-lsp/README.md) | Future library plan — `loft-lsp` (LSP server) + `loft-dap` (DAP debug adapter) + thin Eclipse / JetBrains / Neovim plugins.  ROADMAP series: LSP.1 (MVP — diagnostics + outline + hover, 0.8.6), LSP.2 (editing — completion / def / refs / rename / semantic tokens / code actions, 0.9.0), LSP.3 (loft-dap interpreter-mode debug, 0.9.0), IDE.ECLIPSE / IDE.JETBRAINS / IDE.NEOVIM (1.0.0).  Browser IDE (lib_plans/07-web-ide) consumes loft-lsp; native debugging (plans/25-native-debug) is the GDB/LLDB complement. |
...
| [lib_plans/future/07-web-ide/README.md](doc/claude/lib_plans/future/07-web-ide/README.md) | Future library plan — fully serverless single-origin browser IDE for loft.  Zero-server: runs from `file://` or any static host.  Full WASM interpreter, CodeMirror 6 editor, problems panel, console, outline, multi-project IndexedDB storage, PWA offline.  Roadmap: W2-W6 deferred past 1.0; uses `loft-lsp` (LSP plan) for diagnostics + symbol nav. |
...
| [lib_plans/future/06-web-services/](doc/claude/lib_plans/future/06-web-services/) | Future library plan — client-side web services.  Three-file split: README.md (overview + scope + ship order + "fully functioning" expansion sketch covering URL handling, auth helpers, streaming, SSE/WebSocket client, TLS refinements, diagnostics, adjacent formats), [JSON.md](doc/claude/lib_plans/future/06-web-services/JSON.md) (currently-shipped JSON via `Type.parse()` + `:j` reference), [HTTP_CLIENT.md](doc/claude/lib_plans/future/06-web-services/HTTP_CLIENT.md) (locked-in `HttpResponse` + `http_*` design, deferred to 1.1+ per H4). |
| [lib_plans/future/08-server/README.md](doc/claude/lib_plans/future/08-server/README.md) | Future library plan — `server` library: HTTP routing + middleware, HTTPS with PEM certs, WebSocket support, JWT/session/API-key auth, ACME / Let's Encrypt, CORS / rate limiting / static files, game-loop additions (`ws_poll`, broadcast, ConnectionRegistry).  `lib/server/` has 1234 lines of starting code; the design covers the full feature set still to build (SRV.1-SRV.G in ROADMAP).  Companion to game-client library (still at doc root, awaiting promotion). |
...
| [plans/future/32-tic-tac-toe/README.md](doc/claude/plans/future/32-tic-tac-toe/README.md) | Future plan — protocol-validation vehicle: v1 shipped (server-arbited handshake, integer-id wire format).  v2/v3/v4 are protocol-only ground layers (multi-client, asset-serving + browser, server-side compile + hot-swap) — all verified text-mode.  Visual / playable tic-tac-toe is deferred indefinitely (deliberate scope ceiling); real-game UX lives in plans/future/24-multiplayer-editor/. |
...
| [lib_plans/future/10-game-client/README.md](doc/claude/lib_plans/future/10-game-client/README.md) | Future library plan — `game_client` library: WebSocket client, GameEnvelope multiplayer protocol, lobby + matchmaking, fixed-timestep game loop, client-side prediction + reconciliation, WASM script loading + Ed25519 verification.  ROADMAP series GC.1-GC.6.  Companion to lib_plans/08-server (server-side); supersedes its early Dispatcher / run_game_loop sketches via plans/23-event-loop. |
...
| [plans/finished/31-html-export/README.md](doc/claude/plans/finished/31-html-export/README.md) | Closed plan — W1.1 single-file HTML export.  All 10 implementation steps shipped; `loft --html` produces a self-contained `.html` running compiled WASM in the browser.  Doc kept as historical implementation record + reading-order guide for the export pipeline.  Subsequent W1.x work (W1.18 Worker Threads etc.) tracked separately in CHANGELOG_TECHNICAL.md. |
