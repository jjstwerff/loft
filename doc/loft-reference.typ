...
#set document(title: "Loft Language Reference", keywords: ("v2026.7.1"))
...
#text(size: 10pt, fill: luma(100))[Version 2026.7.1]
...
Write to a temporary path so the example leaves nothing behind in the project.

```rust
  scratch = "/tmp/loft-doc-swatch.png";
...
Plural form of `store\_load\_key`: fetch the given integer keys' entries into `local` in one call (the paged reader is opened once and its cache reused), returning how many were found. Keys absent from the remote are skipped. Same FLAT-struct restriction. \@PLN97 arc G (loft\#522). tiles: hash\<Tile\[id\]\> = \[\] got = store\_load\_keys(tiles, "block.store", \[7, 13, 42\])   // got == 3
