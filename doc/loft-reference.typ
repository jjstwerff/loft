...
⚠ CALL THIS BEFORE YOU BELIEVE A BOUND STORE'S FILE SIZE. A store bound with `store\_persist\_bind` FIRST keeps its file AS the live arena, and the arena's capacity grows by 7/3 and never shrinks by itself — so the file is a rung on a ladder, not a measure of content, and it can sit 57% above what it holds. Two builds one rung apart differ by 133% with identical records, and one holding twice the data can be exactly equal. A generator that ends with this call has a file that means something; one that does not, does not (loft\#752). world: hash\<Hex\[q, r\]\> = \[\] store\_persist\_bind(world, "world.store") // …a region is unloaded for good… store\_reclaim(world)          // the file follows what the world holds Returns 0, changing nothing, for a store that is read-only, shares another store's memory, or carries a `store\_durable\_seal` sidecar — truncating behind that sidecar's back would report a healthy store as corrupt.
