...
Each bound guarantees EXACTLY the operations in this table, and no more — a `+` under `Numeric` is refused as surely as one under no bound at all:

```
Ordered    — `<`, and `>`, `<=`, `>=` derived from it
Equatable  — `==`, and `!=` derived from it
Addable    — `+` only.  NOT `-`
Numeric    — `*` and the UNARY `-`.  NOT `+`, NOT `/`
Scalable   — the method `scale(factor: integer) -> integer`.  Not an operator,
             and no built-in type satisfies it — it is for your own types
Printable  — the method `to_text() -> text`, and `"{x}"` interpolation
```

Reach for `Ordered + Addable` when you want `+` and a comparison; there is no single bound that gives all four arithmetic operators, and no bound at all gives SUBTRACTION today.

⚠ Do not write a binary `a - b` under `Numeric`. It compiles, and it answers `-a` — the second operand is discarded, because the bound's `-` is the unary negation and the binary spelling binds to it anyway (loft\#1274). Subtract at a concrete type until that is closed.
