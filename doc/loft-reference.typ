...
#set document(title: "Loft Language Reference", keywords: ("v0.8.4"))
...
#text(size: 10pt, fill: luma(100))[Version 0.8.4]
...
=== Division by zero — explicit null with `?? null`

Most languages crash on division by zero.  Loft has TWO modes:

- \*\*Defended\*\* (with `?? null` or `if x != null { ... }` after):

```
the divide silently produces null and execution continues.
This lets you handle missing or bad data gracefully.
```

- \*\*Undefended\*\* (bare `1 / 0`): in development the program halts

```
with a typed `error: divide by zero` and the source location;
in production the program logs a warning + continues with the
null sentinel so deployed games / servers never abort on
user-attributable edge cases.
```
...
'long' is a 64-bit signed integer — use it when values might exceed 2 billion. Write a long literal by appending 'l': '1', '100000000000'. Long values support the same arithmetic and format specifiers as integer. Convert a long back to an integer with 'as integer'.

```rust
  big = 1000000000 * 5;
  assert(big == 5000000000, "Long arithmetic");
  assert("{1 + 1:+4}" == "  +2", "Long in format string");
  assert(12 as integer == 12, "Long to integer conversion");
```

=== Common pitfall: integer overflow

If a 32-bit integer calculation overflows the max value (~2 billion), the result wraps around silently. Use 'long' when you expect large values. For example, a score that multiplies two large numbers can silently wrap. Switching to 'long' avoids this: 'score = big_a as integer \* big_b as integer'.
...
Without `?? null`, a bare `1 / 0` halts in development and logs a warning + continues with the sentinel in production.
...
A `null` pattern matches when the value is absent (e.g. defended division by zero — `?? null` opts into the silent-null behaviour instead of the typed-error halt path).
...
This means there is one value per type that you cannot distinguish from null. For integers, that is -2 147 483 648.  When defended with `?? null` (or a following null-check), division by zero produces this same value, so both paths look the same to your code:
...
Mitigation: Use `long` (64-bit) when multiplying or summing large values: `big as integer + big as integer` avoids the wrap. Design: a future `--debug` flag will turn silent overflow into a logged WARN with file:line context so overflows are visible during development without affecting release performance.

```rust
  big = 2000000000;
  assert(big as integer + big as integer == 4000000000, "long avoids wrap");
```
...
Typical dryopea-style pattern: pw = PaintedWorld { painted: hash\<PaintedHex\[q, r\]\>::new() } store\_persist\_bind(pw.painted, "dryopea\_world.store") // …mutations to pw.painted now hit mmap'd bytes…
