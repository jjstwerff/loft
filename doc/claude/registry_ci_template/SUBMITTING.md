...
This file is shipped to `loft-lang/registry/SUBMITTING.md`.  The
canonical in-loft-repo copy is `doc/claude/REGISTRY_SUBMIT.md`
(uses relative links — works when viewed in the loft repo).
This deploy copy uses absolute GitHub URLs for cross-doc
references so they resolve from the registry repo.  When you
edit the canonical, regenerate this file from it and rewrite
the links — see `registry_ci_template/README.md`.
-->
...
| Trigger uniqueness | Every Tier-1 `method:receiver` trigger is owned by at most one package across the whole registry | Your `[triggers]`-enabled package declares a method-on-type (`matches:text`, …) another package already claims |
