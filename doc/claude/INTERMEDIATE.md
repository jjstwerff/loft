...
## 254 Bytecode Operators — `src/fill.rs`
...
### Opcode budget: 256 slots, 254 used

The opcode byte is a `u8`, so the address space is `0x00..=0xFF` —
**256 slots total**.  The `254` literal that appears in
`src/fill.rs:10` —
...
— is the **current count of defined operators**, baked into the
array length at `src/create.rs::generate_code` via the `{count}`
template placeholder.  It is **not** a language limit; a future
definition that adds another opcode bumps the generated literal
automatically.  Only 2 slots sit unused today (0xFE, 0xFF).
