...
|---|---|---|---|---|
| **A `vector<vector<single>>` SIGSEGVs when built (@P380, OPEN)** | Building a vector whose elements are `vector<single>` (e.g. `vv = [a, b]` where `a`/`b` are `vector<single>`) crashes on both backends.  Root-caused: the nested-vector literal's element type is mis-resolved one level too shallow, so each new element is default-initialised as a `single` scalar (whose 32-bit NaN sentinel has non-zero low bits) instead of an empty inner vector — later read as a garbage record id.  `vector<vector<integer>>` / `vector<vector<float>>` (8-byte elements) escape by luck; `single` (4-byte) does not. | Wrap the inner vector in a struct (`struct W{v: vector<single>}` then `vector<W>`), or use `vector<vector<float>>`/`<integer>`, until the type-inference fix lands | @P369 fix surfaced it (was masked by the silent-fault test harness); root-caused 2026-05-28 | `tests/wrap.rs::vector_advanced_known_broken` (#[ignore], open @P380); repro `/tmp/p_followups/p380_vov_single_crash.loft` |
...
| **Store return + struct param leaks per call on interp (@P377, OPEN — hard)** | Under `--interpret`, a fn with a **struct value parameter** returning a freshly-allocated store-backed struct via an intermediate local (`cv = canvas(...); cv`, e.g. `graphics::Canvas`) leaks ONE store per call — unbounded in a render loop (the dryopea editor leaks one full-screen Canvas per frame).  Root-caused, but TWO fix attempts were unsound (the dep-inference can't tell a fresh return from a deep borrowed view) — needs deeper work.  Scalar-param + `--native` are clean | Hoist the allocation out of the loop (allocate once, `clear()` + redraw), or run under `--native` | dryopea editor render loop 2026-05-27 | `/tmp/p_followups/p377_canvas_store_leak.loft` (open @P377) |
