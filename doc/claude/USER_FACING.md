...
|---|---|---|---|---|
| **Recursive vector-returning code can SIGSEGV after many vector ops (@P380, OPEN)** | A long sequence of vector-of-vectors deep-copies / recursive vector-returning functions on one program run can corrupt the store heap and crash (a merge-sort over vectors SIGSEGVs after ~40 prior vector functions; runs fine alone).  Same store-lifetime family as @P377/@P378 | Split vector-heavy work across fresh program runs; avoid deep vector-of-vectors deep-copy chains until the fix lands | @P369 fix surfaced it (was masked by the silent-fault test harness) 2026-05-28 | `tests/wrap.rs::vector_advanced_known_broken` (#[ignore], open @P380) |
...
| **~~Store return + struct param leaks per call on interp (@P377)~~ FIXED 2026-05-28** | Under `--interpret`, a fn with a **struct value parameter** returning a freshly-allocated store-backed struct via an intermediate local (`cv = canvas(...); cv`, e.g. `graphics::Canvas`) leaked ONE store per call — unbounded in a render loop (the dryopea editor leaked one full-screen Canvas per frame).  FIXED via the adopt-path: a fresh (all-hidden-dep) return is taken over directly (no deep-copy, no source-free), so no leak and no corruption.  Genuine borrowed-view returns still deep-copy | n/a (fixed) | dryopea editor render loop 2026-05-27 | `tests/leak.rs::p377_struct_param_fresh_return_adopt` |
