...
| **Vector element-set of an inline struct literal loses nested data (@P313)** | `vec[i] = Struct{ m: V{a:…} }` in a loop keeps only the first element's nested-vector data (`total=30` not `120`).  Same root as @P311 but on the vector element-set path (a different code site, not yet fixed).  `vec += [Struct{…}]` (append) and `vec[i].field = Struct{…}` (field-of-element) are unaffected | Bind the literal to a local first: `c = Struct{…}; vec[i] = c` | @P311 sibling sweep 2026-05-21 | none yet — file with the fix |
