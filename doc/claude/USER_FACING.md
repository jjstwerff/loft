...
|---|---|---|---|---|
| **Keyed field from a struct-literal expression is empty (@P308)** | `F{ h: build_index(...) }` or `F{ h: c }` leaves the `hash`/`sorted`/`index` field SILENTLY EMPTY (content not deep-copied); a call source also leaks.  Both backends | Init empty then populate: `f = F{ h: [] }; f.h += [...]` (or `f.h[key] = value`) | gridmesh ChunkField (lib-plan 19) 2026-05-21 | none yet — file with the fix |
