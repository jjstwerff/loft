...
| **Keyed collection of nested struct-of-vectors crashes (@P311)** | A `hash<E[k]>` whose element `E` holds a NESTED struct-of-vectors (e.g. `E { k, mesh: SegMesh }`) panics OOB (`search.rs:366`, index 65535) once it holds several entries — a deeply-nested vector content type is left unresolved.  Single-level keyed struct-of-vectors works; the extra nesting layer is the trigger | Flatten the element (put the vectors directly on `E`, no nested struct), copying via local vars not field-access; or don't cache in a keyed collection (rebuild on demand) | gridmesh C3 incremental rebuild 2026-05-21 | none yet — file with the fix |
| **Native E0503 on nested-field mutation through `&`-ref (@P312)** | A function `fn(s: &Struct, …)` that mutates a NESTED field of the by-`&`-ref struct — `s.inner.vec += [x]` or `s.keyed[k] = Elem{…}` — fails `--native`/`--check` with `error[E0503]: cannot use var_s because it was mutably borrowed` (the generated nested-mutation `&mut` borrow conflicts with a later use of `var_s`).  Interp unaffected.  No in-tree trigger today (the gridmesh-C3 code that hit it was removed) | Bind the inner value to a local and mutate that, or pass the inner struct/vector by `&` directly instead of reaching through the outer ref | gridmesh C3 native-compile 2026-05-21 | none yet — file with the codegen fix |
