...
| **Native graphics FFI `*const i32`/`vec<i64>` mismatch (@P310)** | `loft --native` / `--check` on any graphics program that calls `save_png`, `gl_upload_canvas`, or `rasterize_text_into` fails with `error[E0308]: expected *const i64, found *const i32` — codegen narrows the `const vector<integer>` data pointer to i32 (P184) but the FFI wrappers still declare `vec<i64>`.  Interp is unaffected | Run graphics programs under `--interpret` (the demos already do).  Fix: widen the codegen pointer or narrow the wrapper element type | gridmesh C1 native-check 2026-05-21 | none yet — file with the fix |
