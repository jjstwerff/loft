...
| **Native miscompile: `?? <int>` on a `vector<u8>` element (@P316)** | Reading a narrow-int vector element with a null-coalesce default — `bytes[i] ?? 0` on a `vector<u8>` — fails to compile under `--native` (`error[E0308]`: the `??` result is mis-typed as `u8` instead of widening to the integer arithmetic).  Runs fine on the interpreter | Use `vector<integer>`, or avoid `?? <int>` on a `u8` element | native u8 read 2026-05-22 | none yet |
| **gridmesh C3 incremental crystal rebuild diverges from a full rebuild (@P314)** | `crystal_incr_paint` + `crystal_incr_rebuild` produces a non-deterministic, corrupt mesh vs a full build (heap corruption).  A SEPARATE root from @P315 (re-checked: SegMesh uses `as single` casts) — under investigation.  The full build (`crystal_incr_new` + `crystal_incr_full_mesh`) is correct | Use the full build (`crystal_segments_aged`, or a fresh `crystal_incr_new(snap)`) instead of incremental paint+rebuild | gridmesh C3 re-enable 2026-05-22 | under valgrind-guided investigation |
