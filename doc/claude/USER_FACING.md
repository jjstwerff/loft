...
| **`lib/graphics` text has no kerning (@P339)** | All rendered text is un-kerned: `gl_measure_text("AV")` == `measure("A")+measure("V")` exactly, and strings like "AVA WoTo" show loose A‑V / W‑o / T‑o spacing.  The measure and rasterize paths sum bare glyph advance widths and never consult the font kern table | None — text renders un-kerned (visually loose but legible) | `/tmp/p_followups/loft_no_kerning.loft` 2026-05-25 | none yet — file with the fix (add fontdue `horizontal_kern` to measure + rasterize) |
| **`lib/graphics` no text baseline / ascent metric (@P340)** | `draw_text(x,y)` anchors by the bitmap TOP, line metrics are hardcoded approximations (`size*1.2` height, `size*0.8` baseline), and no font ascent is exposed — so mixed-size text can't be aligned on a shared baseline | Manually offset `y` by an estimated ascent (`≈ size*0.8`) | `/tmp/p_followups/loft_text_baseline.loft` 2026-05-25 | none yet — file with the fix (use fontdue `horizontal_line_metrics`; expose `gl_font_ascent`) |
