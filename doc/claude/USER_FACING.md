...
| **Native E0503 when a call passes both a `&`-ref and a field of it (@P312)** | `f(c, g(c.field))` where `f` takes `c: &Struct` compiles to `f(&mut var_c, g(…var_c…))` and fails `--native` with `error[E0503]: cannot use var_c because it was mutably borrowed` (a two-phase-borrow limitation in the call-arg emit).  Interp unaffected | Hoist the field read into a local first: `x = g(c.field); f(c, x)` | gridmesh C3 native-compile 2026-05-21 | worked around in C3 2026-05-22; codegen fix pending |
| **Nested struct-of-vectors in a keyed collection corrupts the heap on read (@P315)** | A `hash<E[k]>` whose element holds a NESTED struct-of-vectors (`E{ k, m: Sm }` where `Sm` has `vector<…>` fields) corrupts the heap when iterated and the nested vector is read (`for e in h { len(e.m.a) }`) — SIGSEGV at a varying location.  A DIRECT vector on the element (`E{ k, a: vector<…> }`) works.  Same family as @P311/@P313 (both fixed) but on the iterate/read path | Flatten the element to one nesting level (vectors directly on the element, no inner struct); or don't store nested struct-of-vectors in a keyed collection | nested-keyed read 2026-05-22 | none yet — needs a dedicated fix |
| **gridmesh C3 incremental crystal rebuild diverges from a full rebuild (@P314)** | `crystal_incr_paint` + `crystal_incr_rebuild` produces a mesh with the right segment COUNT but different GEOMETRY than a full build.  ROOT is @P315 (heap corruption reading the nested `SegMesh` out of the `hash<ChunkMeshEntry[ck]>` chunk cache during `crystal_incr_full_mesh`) — NOT dirty-propagation, which was verified correct.  The full build (`crystal_incr_new` + `crystal_incr_full_mesh`) is correct | Use the full build (`crystal_segments_aged`, or a fresh `crystal_incr_new(snap)`) instead of incremental paint+rebuild | gridmesh C3 re-enable 2026-05-22 | gated on @P315 |
