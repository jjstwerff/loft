...
| **`s = "literal" + s` self-prepend (P223, S1)** | `s = "world"; s = "hello " + s` gives `"worldhello worldhello "` on interp (work text aliased with `s`), native rejects with E0502.  P217's fix didn't reach this case because the lowering goes through the fresh-work-text branch but assignment-back-to-`s` still aliases | Hoist: `t = "hello " + s; s = t;`.  Or use interpolation: `s = "hello {s}";` | P217 fix-variant probing 2026-05-05 | none yet — file with the fix |
| **Coroutine `yield text` + format-with-param (P218, S1)** | A `iterator<text>` generator yielding a format string that interpolates a parameter (`yield "hello, {name}"`) rejects under native with E0425 — the `__work` text-format buffer is declared in one match arm of the state machine but referenced from another scope.  Interp works | Pre-format outside the yield: `m = "hello, {name}"; yield m;` | P211 fix-variant probing 2026-05-05 | none yet — file with the fix |
| **Coroutine for-loop over vector literal (P219, S2)** | `for n in [10, 20, 30] { yield n; }` inside a generator body emits invalid Rust for the factory function (E0308 unit/Box mismatch).  Range-for (`for i in 0..n`) works.  Affects integer yields too — not text-specific | Use a range-for: `for i in 0..3 { yield [10, 20, 30][i]; }` | P211 fix-variant probing 2026-05-05 | none yet — file with the fix |
...
| **Capturing closures in struct fields (P213 + P215, same root cause)** | `Type::Function` fields are 4 bytes (just the function id) but capturing-closure values need 16 bytes (id + captured-environment DbRef).  P213 covers user struct fields; P215 covers synthetic closure records when one closure captures another.  Parse-time diagnostics shipped on both surfaces.  Plan revised 2026-05-04 (v3) to **co-locate the closure record in the host's Store** via OpAppendVector — host's free cascade naturally frees the closure data; smallest blast radius (only struct-field path changes; tuple/vector containers keep current behaviour).  Full design in [PROBLEMS.md § 213](PROBLEMS.md#213-typefunction-storage-layout-limit--full-design-for-the-proper-fix) | Define inner functions at file scope, or inline their bodies — same workaround serves both the user struct-field case and the nested-closure case | plan-15 phase 03 cluster | `tests/parse_errors.rs::p213_capturing_closure_in_struct_field_rejected` + `p215_captured_fn_in_nested_closure_rejected` (regression-only; both removed when the layout fix lands and replaced with positive end-to-end tests per the design) |
