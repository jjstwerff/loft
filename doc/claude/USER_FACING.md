...
| **Capturing closure in struct field — design limit** | Capturing closures cannot be stored in struct fields because the field is 4 bytes (function id only) and a capturing closure needs 16 bytes (id + captured-env DbRef).  P213 added a clear parse-time diagnostic; the proper fix (16B fn-ref fields) is a multi-session refactor.  Non-capturing closures and named fn-refs work | Define the function at file scope, or pass the closure via a fn-typed parameter / return value | plan-15 D3 cluster | `tests/parse_errors.rs::p213_capturing_closure_in_struct_field_rejected` (the diagnostic stays stable until the layout fix lands) |
