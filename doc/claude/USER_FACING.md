...
|---|---|---|---|---|
| **Cross-module store return + struct param leaks per call on interp (@P377, OPEN)** | Under `--interpret`, a fn with a **struct value parameter** that returns a freshly-allocated **library-defined store-backed struct** (e.g. `graphics::canvas(...) -> Canvas`) leaks ONE store per call — unbounded in a render loop (the dryopea editor leaked one full-screen Canvas per frame).  Scalar-param helpers and `--native` are both clean | Hoist the allocation out of the loop (allocate once, `clear()` + redraw into it), or run under `--native` | dryopea editor render loop 2026-05-27 | `/tmp/p_followups/p377_canvas_store_leak.loft` (open @P377) |
