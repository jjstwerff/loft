...
|---|---|---|---|---|
| **Two libraries with a same-named struct panic at load (@P379, OPEN)** | `use libA; use libB;` where both define a struct of the same name (e.g. `Chunk`/`Hex`) crashes with an internal panic `Double structure type <Name>` — not a clean error.  Struct-type registration is a flat global keyed by bare name (function access IS namespaced; types aren't), and isn't idempotent for a lib reachable via two load paths | Rename one library's struct so no two loaded libraries share a struct name | dryopea Plan 07 adopting moros_map 2026-05-28 | `loft_repros/dup_struct_type_across_libs.loft` (open @P379) |
