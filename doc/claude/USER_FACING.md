...
| **Store return + struct param: LEAK class FIXED 2026-05-28 (S1, parse-time NRVO); CORRUPTION class still OPEN (@P377, `p377-fix`)** | LEAK FIXED: under `--interpret`, a fn with a **struct value parameter** returning a freshly-allocated store-backed struct via an intermediate local (`cv = canvas(...); cv`) used to leak ONE store per call.  Fix: parse-time NRVO in `src/parser/control.rs::block_result` (helper `nrvo_collapse_tail_set`) substitutes the inner Call's hidden-buffer arg with the outer fn's hidden buffer var — inner Call writes directly into the outer buffer; no separate store; no leak.  Retires runtime-layer dep-tracking complexity for this class.  Regression `139-p377-libfree-leak.loft` runs in default `loft_suite`.  CORRUPTION still OPEN: the silent-corruption oracle (`140-p377-corruption-oracle.loft`, ≥ 2 struct-param fns interleaved in a loop, `cv_q[0] = null(oob)` on iter 1) is a SEPARATE pre-existing interpret-only runtime bug — fails identically with S1 enabled or disabled.  Native passes the oracle.  Held behind `p377_corruption_oracle_known_broken` until a dedicated fix lands | LEAK: no workaround needed.  CORRUPTION: hoist the allocation out of the loop (allocate once, `clear()` + redraw), or run under `--native` | dryopea editor render loop 2026-05-27 | LEAK regression in `loft_suite`; `140-p377-corruption-oracle.loft` still `#[ignore]`'d as `p377_corruption_oracle_known_broken` |
