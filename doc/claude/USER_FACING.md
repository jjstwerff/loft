...
| **Store return + struct param leaks per call on interp (@P377, OPEN — hard; corruption oracle + lib-free repro landed)** | Under `--interpret`, a fn with a **struct value parameter** returning a freshly-allocated store-backed struct via an intermediate local (`cv = canvas(...); cv`, e.g. `graphics::Canvas`) leaks ONE store per call — unbounded in a render loop (the dryopea editor leaks one full-screen Canvas per frame).  Root-caused, but THREE fix attempts have been unsound (the dep-inference can't tell a fresh return from a deep borrowed view) — needs deeper work.  Scalar-param + `--native` are clean.  **Latest (`p377-fix` branch):** committed lib-free reproducer + a deterministic **corruption oracle** that turns the previously-silent failure mode into a loud assertion; sets up the next attempt to be gateable | Hoist the allocation out of the loop (allocate once, `clear()` + redraw), or run under `--native` | dryopea editor render loop 2026-05-27 | `tests/scripts/139-p377-libfree-leak.loft` + `tests/scripts/140-p377-corruption-oracle.loft` on branch `p377-fix` (`#[ignore]`'d until fix) |
