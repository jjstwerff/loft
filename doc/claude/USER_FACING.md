...
|---|---|---|---|---|
| **Two libraries with a same-named struct (@P379 — struct-types FIXED 2026-05-28; native fn-names open)** | `use libA; use libB;` where both define a struct of the same name (e.g. `Chunk`) used to crash with an internal `Double structure type` panic.  FIXED: struct types now namespace per library (the second definer is registered as `lib::Name`); cross-library programs load + run correctly under `--interpret`.  Still open: the `--native` backend flattens FUNCTION names, so two libs with a same-named function fail native compile (`E0428`) | Run cross-library programs under `--interpret` (functions are namespaced there); for native, rename a clashing library function until native fn-namespacing lands | dryopea Plan 07 adopting moros_map 2026-05-28 | `tests/issues.rs::p379_two_libs_same_struct_name` |
