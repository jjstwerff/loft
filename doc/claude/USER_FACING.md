...
| **`Type::Function` storage layout limit (P213 + P215, same root cause)** | `Type::Function` fields are 4 bytes (just the function id) but fn-ref values are 16 bytes (id + captured-environment DbRef).  Surfaces wherever fn-types are stored: user struct fields (P213), synthetic closure records when one closure captures another (P215), tuples / vectors / hashes / sorted / index containers with fn-typed elements.  Parse-time diagnostics shipped on the two surfaces we've hit so far.  Full design + 21-cell test matrix in [PROBLEMS.md § 213](PROBLEMS.md#213-typefunction-storage-layout-limit--full-design-for-the-proper-fix); marked as "fix the layout once, remove the entire class" rather than continue filing per-symptom P-issues | Define inner functions at file scope, or inline their bodies — same workaround serves both the user struct-field case and the nested-closure case | plan-15 D3/D4/C4/C6 cluster | `tests/parse_errors.rs::p213_capturing_closure_in_struct_field_rejected` + `p215_captured_fn_in_nested_closure_rejected` (regression-only; both removed when the layout fix lands and replaced with the positive end-to-end matrix per the design) |
