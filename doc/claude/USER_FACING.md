...
| **Native codegen E0282 on bounded-T method + concat (P208)** | `<T: Printable>(x: T) -> text { x.to_text() + "!" }` runs cleanly under `--interpret` but `--native` rustc rejects: "type annotations needed" — generated code wraps the inner P205-style scratch.push around an outer P205-style scratch.push that never executes (the inner `return` makes the outer unreachable, so rustc can't infer the unreachable expression's type). | `--interpret` works; for native, restructure to avoid `+` on the bounded-T method result | plan-17 phase 01 (B) follow-up; filed as P208 | none yet — to be added when the fix lands |
