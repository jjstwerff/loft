...
| **Keyed struct-FIELD clear `s.h = []` broken (@P307)** | Clearing a keyed field by reassignment fails to compile through `&` ("never modified"), and even bypassed is a runtime no-op + store leak.  Keyed LOCAL clear (`s = []`) works (@P302); the field path doesn't | Hold the collection as a local, or rebuild the struct field from a fresh-empty function return | gridmesh `clear_dirty` (lib-plan 19) 2026-05-21 | `hash_spec.loft` (plan 44) field-clear probe — file with the fix |
