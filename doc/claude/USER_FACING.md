...
| **`vector<single>` from float literals corrupts the heap when copied (@P315)** | Building a `vector<single>` from float literals (`Sm{ a: [1.0, 2.0] }`) writes each element 8 bytes wide instead of 4, overflowing the slot.  It looks fine until the vector is copied (assigned into a struct field, inserted into a keyed collection, or concatenated), then it corrupts the heap → wrong values or SIGSEGV.  `vector<integer>` is unaffected | Build `vector<single>` by appending single-typed VARIABLES (`v += [x]` with `x: single`) rather than float literals; or use `vector<integer>` | nested vector<single> 2026-05-22 | none yet — needs a dedicated fix |
