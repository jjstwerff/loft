...
| **Capturing closure in struct field — near-term planned** | Capturing closures cannot yet be stored in struct fields because the field is 4 bytes (function id only) and a capturing closure needs 16 bytes (id + captured-env DbRef).  P213 ships a clear parse-time diagnostic.  The layout-widening fix is **planned to land before** the `server` library, the `game_client` / game-loop work, or any other consumer that wants handler-registry / event-bus / state-machine patterns.  Full design in [PROBLEMS.md § 213](PROBLEMS.md#213-capturing-closures-cannot-be-stored-in-struct-fields--full-design-for-the-proper-fix) | Define the function at file scope, or pass the closure via a fn-typed parameter / return value (workaround scales poorly past a few handlers — that's the trigger to do the fix) | plan-15 D3 cluster + server/game-loop sequencing | `tests/parse_errors.rs::p213_capturing_closure_in_struct_field_rejected` (regression-only; gets removed when the proper fix lands and replaced with positive end-to-end tests per the design plan) |
