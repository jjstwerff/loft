...
| **gridmesh C3 incremental crystal rebuild diverges from a full rebuild (@P314)** | `crystal_incr_paint` + `crystal_incr_rebuild` (re-running only dirty chunks, reusing the rest) produces a mesh with the right segment COUNT but different GEOMETRY than a full build — a halo/dirty-propagation reuse bug (chunks whose halo includes a newly-painted cell are not all re-dirtied).  The full build (`crystal_incr_new` + `crystal_incr_full_mesh`) is correct.  A separate chunk_shift=2 + non-empty-`new()` variant also duplicates keyed chunk-cache entries (find/dedup stale after the struct is deep-copied) | Use the full build (`crystal_segments_aged`, or a fresh `crystal_incr_new(snap)`) instead of incremental paint+rebuild | gridmesh C3 re-enable 2026-05-22 | none yet — incremental two-level reuse gated on the fix |
