...
| **Capturing closure in struct field — wanted future feature** | Capturing closures cannot yet be stored in struct fields because the field is 4 bytes (function id only) and a capturing closure needs 16 bytes (id + captured-env DbRef).  P213's parse-time diagnostic is the correct behaviour for the current release.  The layout-widening fix is queued for a future release — co-scheduled with the `server` library, `game_client` / game-loop work, or plan-15 phase 03 (whichever lands first).  Full design in [PROBLEMS.md § 213](PROBLEMS.md#213-capturing-closures-cannot-be-stored-in-struct-fields--full-design-for-the-proper-fix) | Define the function at file scope, or pass the closure via a fn-typed parameter / return value | plan-15 D3 cluster | `tests/parse_errors.rs::p213_capturing_closure_in_struct_field_rejected` (regression-only; gets removed when the proper fix lands and replaced with positive end-to-end tests per the design plan) |
