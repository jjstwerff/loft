...
|---|---|---|---|---|
| **Sorted/index field from a struct-literal expression is empty (@P309)** | `S{ s: build() }` / `S{ s: c }` leaves a `sorted`/`index` field silently empty (the @P308 deep-copy fix is hash-only; sorted/index deferred — a deep-copy hangs when an `index<sameT>` mutates the content struct).  HASH fields now deep-copy correctly (@P308 fixed) | Init empty then populate: `s = S{ s: [] }; s.s += [...]` | gridmesh ChunkField (lib-plan 19) 2026-05-21 | none yet — file with the fix |
