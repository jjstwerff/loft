...
| 244 | `--native` compilation of any program that links `lib/server` fails with rustc E0308 "expected `Str`, found `LoftStr`" on the generated wrappers for `n_ws_message_native` and `n_ws_event_payload_native`.  Reproducer: `./target/release/loft lib/server/tests/server.loft` (default `--native` path, no flag).  The generated wrapper looks like `fn n_ws_message_native(cell: &UnsafeCell<Stores>) -> Str { unsafe { loft_server::n_ws_message() } }` — the underlying Rust extern returns `LoftStr` (from `loft_ffi::ret_ref(&...)`) but the loft codegen declares the wrapper return type as `Str`.  Same shape on `n_ws_event_payload`.  Surfaced 2026-05-10 while wiring TTT v5's binary-WS bindings (lib/server/native gained `n_ws_send_binary` + exposed `n_ws_opcode`) — the new functions don't return text and are unaffected; the failure is in the pre-existing text-returning n_ws_* wrappers.  Likely root cause: the native-codegen path that wraps `text`-returning native functions doesn't insert the `LoftStr → Str` conversion that `n_ws_*` actually needs (cf. P205 / P232 lifetime-style fixes for text returns; this looks like the FFI-boundary equivalent).  Workaround: run the test under `--interpret` (`./target/release/loft --interpret lib/server/tests/server.loft` passes cleanly).  Pinned by `lib/server/tests/server.loft` (the test compiles + runs under interpret; once P244 closes the same file should run under default `--native` as well). | Medium (open) — blocks `--native` for any lib/server consumer; affects TTT v3 / v4 / v5 paths and plan-36 server | Run server tests / programs under `--interpret`; the interpreter path fully exercises the new bindings. |
