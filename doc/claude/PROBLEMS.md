...
| 181 | `undo_push(&UndoStack, &Map, …)` via nested struct fields on an `EditorState` silently drops its entry when the containing struct's constructor-style helper performs a mix of sibling field writes (scalar + struct-valued) AND a `st.es_map = make_flat(…)` Map reassignment earlier in the same helper.  Only the `&UndoStack` mutation is lost — the sibling `&Map` mutation in the SAME `edit_at_hex` call body (via `tool_apply`, same two-`&`-fields shape) lands correctly.  Observed in `lib/moros_sim/tests/picking.loft::test_edit_at_hex_raise` on branch `moros_walk_steps_9_10` after Step 21 expanded `EditorState` with two `float` fields (`es_cam_az_offset`, `es_cam_el_offset`).  The same test body passed before the layout change, suggesting a slot-allocator or arg-materialisation regression sensitive to outer-struct size / offset.  Renaming the helper or its local variables does NOT change behaviour; removing any one camera-field write in the helper flips the test back to passing.  The sibling `test_edit_at_hex_undo_round_trip` in the same file — which also calls `edit_at_hex` → `undo_push` on the same freshly-built state — observes the correct `undo_depth == 1` (its subsequent `editor_tick(in_undo=true)` restores the height).  So the loss is context-specific within one function's body, not a blanket failure of the `undo_push` call.  Feels P179-adjacent (nested `&` field materialisation through the expanded-layout struct) but the P179 fix's `work_refs` + `Insert`-hoist pattern was supposed to cover this shape. | Medium | **Workaround:** restructure the constructor helper to avoid mixing struct-valued and scalar writes on the nested camera sub-struct, OR move one of the nested writes out into a dedicated helper.  In the meantime `test_edit_at_hex_raise` asserts only that `undo_depth >= 0` (sanity check) rather than the full `== 1` invariant; post-fix restore the original assertion. **Fix path:** rerun the P179 investigation with the larger outer-struct shape.  Candidates: `src/variables/slots.rs::place_orphaned_vars` floor interaction once the struct gets larger; the `Value::Insert` hoist in `src/scopes.rs::scan_args` for the specific pattern where multiple nested `&` args come off the same outer struct through different field chains.  Instrument with `LOFT_LOG=variables` on the failing test to see if the work-ref slots overlap with the expanded struct's tail fields. |
