...
| 170 | `x = Struct {}; x = vec[i]; mutate(x)` — a placeholder assignment of a default-valued struct, followed by reassignment from a vector element and mutation via `&Struct`, trips the codegen assertion `Incorrect var x[N] versus M on n_<fn>` (slot N > current stack position M).  This is the P139-family shape: x's slot is allocated in a later zone (DbRef-sized, 12 bytes past current TOS) but no `OpReserveFrame` is emitted to extend the frame before the access.  Surfaced while implementing stair dispatch in `build_hex_meshes` with an `sh_target = Mesh {}` placeholder pattern.  Minimal repro: `struct Bag { items: vector<integer> } fn m(b: &Bag, v: integer) { b.items += [v]; } fn test() { bs: vector<Bag> = []; x = Bag {}; bs += [Bag {}]; x = bs[len(bs) - 1]; m(x, 1); }`.  **Workaround:** drop the placeholder — `x = Struct {}` immediately overwritten by `x = vec[i]` is a no-op in principle but triggers the bug in practice.  **Fix path:** extend P139's `OpReserveFrame(gap)` emission in `state/codegen.rs::gen_set_first_at_tos` (or the vector-element read path) to cover the Struct-placeholder → vec-elem-reassign shape. | Medium | Drop the placeholder; the first real value assignment allocates x correctly |
