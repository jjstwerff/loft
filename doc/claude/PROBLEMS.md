...
| [P255](#open-issues--quick-reference) | Medium | Capturing the result of a generic-fn `vector<T>` return into a local variable rejects with `"cannot change type from vector<X> to vector<X>"` (X identical on both sides) — `change_var` Vector→Vector branch fires diagnostic without checking type equality first |

**Pattern**: the entire 5-bug bounded-generic cluster
(P252 + P239 + P240 + P243 + P241) closed today via
...
the substituted primitive type).  P255 (filed today during
P241 closeout) is a pre-existing pattern-matching bug in
`change_var`'s Vector→Vector arm that blocks capturing the
returned `vector<T>` into a local — affects struct-T more
than primitive-T because primitive-T tests use `expr` directly.
P250 + P251 are tuple-of-non-trivial-element bugs around
per-element projection in storage and lifetime.  P253 + P254
are local-attacker security issues (hash-DoS + cache
poisoning); both are low priority because remote exploitation
requires loft to be deployed in a shape it doesn't currently
have (auth'd web endpoint indexed by hash; multi-tenant CI
...
| 255 | Capturing the result of a generic-fn `vector<T>` return into a local variable rejects with `"Variable X cannot change type from vector<S> to vector<S>"` — both sides of the diagnostic show the IDENTICAL type.  Reproducer: `struct P { v: integer } fn make<T>(x: T) -> vector<T> { o: vector<T> = []; o += [x]; o } fn main() { p = P{v:1}; vp = make(p); }` rejects on parse.  Surfaced 2026-05-11 during P241 slice-3 regression test development.  Independent of P241 — the diagnostic fires before any IR rewrite or codegen runs.  Root cause: `src/variables/mod.rs:977-989`'s Vector→Vector branch in `change_var` emits the error any time `tp.is_unknown()` is false (i.e., the variable has a known type), without first checking whether the new type is `is_equal` to the existing type.  The Vector→Vector path was added to support assigning to an empty `v: vector<T> = []` (which has unknown content) and forgot the equal-types-are-fine fallthrough.  Other primitive paths in the same function (`!var_tp.is_unknown()` at line 991) have the same gap but rarely surface because the parser uses a different code path for primitive re-assignment.  Likely fix: add a leading `if var_tp.is_equal(type_def) { return false; }` (or equivalent `is_equal`-aware check) at the top of the Vector→Vector arm AND the primitive arm, so identical-type re-assignment is silently accepted (the underlying var stays the same; nothing to update).  Workaround: rename the destination variable on each call (e.g., `vp1 = make(p1); vp2 = make(p2);`).  Affects ALL generic fns that return a `vector<X>` (not just struct-T) — pure-primitive cases hit this too if the local was previously assigned `vector<T> = []` shape.  Pinned by manual reproducer; no regression test yet (would need to wait for the fix to assert successful capture; see also P241 slice-3 deferred regression test). | Medium (open) — silent block on a common pattern; no compile-error workaround beyond renaming | Rename the destination variable each call site, or hoist the construction inline (`vp = [x]` if the type allows). |
