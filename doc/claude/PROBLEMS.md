...
| 164 | Trailing comma after the LAST VARIANT of an enum declaration fails to parse (P158 covered trailing commas inside a variant's field list, not the variant list itself).  Repro: `enum E { Alpha { x: integer }, Beta { y: integer }, }` → `Expect name in type definition` / `Syntax error: unexpected '}'` at the enum's closing `}`.  **Workaround:** drop the comma after the last variant.  **Fix path:** mirror the P158 guard into whatever loop in `parser/definitions.rs` closes the enum's variant list — likely a second `\|\| self.lexer.peek_token("}")` check analogous to the field-list one. | Low | Drop the trailing variant comma (stylistic; common reformatters add it back — fragile) |
| 165 | Variable declaration `x: Enum = Variant { ... }` rejected as "cannot change type from `Enum` to `Variant`"; the type-change check treats a struct-enum variant as a distinct type from its parent enum rather than accepting it as a subtype.  Workarounds: drop the annotation (`x = Variant { ... }` infers as the variant, still flows to `fn(y: Enum)` params fine) or use explicit `as`: `x = Variant { ... } as Enum`.  **Fix path:** in the type-change check (somewhere in `parser/objects.rs` / `parser/expressions.rs::parse_assign_op`), allow an annotated LHS whose type is the parent enum when the RHS is a variant of that enum. | Low | Drop annotation, or append `as Enum` |
