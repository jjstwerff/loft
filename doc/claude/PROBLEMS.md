...
| 233 | `tests/testing.rs::code!()` test harness hangs the loft parser/lexer when the loft source AND the expected `Value::Text` value both contain JSON-escape sequences (`\"`, `\\`, `\n`).  The harness embeds the expected value TWICE in the auto-generated assertion script — once as the comparison RHS (`test_value == "<res>"`), once in the `"Test failed {test_value} != \"<res>\""` message — and the doubly-embedded escape sequences appear to trigger an infinite loop in loft's lexer when scanning the format-string body.  Surface symptoms (PR #210 CI, 2026-05-07): macOS CI's `q3b_struct_to_json_string_escapes_control_chars` FAILS with a round-trip mismatch (the Rust raw `\n` survives as 2 chars in Rust but loft's lexer interprets it as a newline when the expected value is re-embedded into a loft string literal); Ubuntu CI's `q3b_struct_to_json_string_escapes_quote_and_backslash` then HANGS indefinitely (2h+ on the runner before manual cancellation).  Same loft source run STANDALONE via `--interpret` does NOT hang; the harness's `Drop for Test` assembly is what triggers it.  Fix shapes: (a) `replace_tokens` in `tests/testing.rs` should escape `\` as well as `"` so loft's lexer can't reinterpret backslash sequences in the expected literal; (b) the lexer should hard-cap iterations when scanning a string literal so a pathological nesting fails with a clear "string literal too deeply escaped" diagnostic instead of looping.  Reproducer + fix shape in `/tmp/p_followups/p233_test_harness_hang.txt`.  Workaround (taken on PR #210): drop the brittle escape-comparison loft-surface tests; rely on the Rust-level `json_escape_tests` in `src/database/format.rs` (13 unit tests covering every short-form escape, the `\uXXXX` arm, the boundary at 0x20, and UTF-8 multibyte passthrough) for per-byte JSON-escape dispatch coverage. | Medium (test infra, open) | Use Rust `#[test]` against `write_json_escaped` for per-byte escape coverage instead of `code!()`. |
