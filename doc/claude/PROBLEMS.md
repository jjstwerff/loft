...
| 151 | Forward-reference to struct-returning fn + field mutation corrupts variable type inference | High | **Open 2026-04-16.**  In a single-file program: `x = callee(); x.field = value;` where `callee` is defined LATER in the file errors with `Variable 'x' cannot change type from integer to <Struct>`.  Reordering `callee` BEFORE the caller compiles cleanly.  Discovered while adding MO.3 convenience editors to `lib/moros_map/src/moros_map.loft` (`map_paint_material`, `map_set_height`, etc.).  Workaround: define struct-returning helpers BEFORE their callers in the same file.  Minimal repro:<br>`struct H { v: integer }`<br>`fn one() { x = callee(); x.v = 99; }`<br>`fn callee() -> H { H { v: 7 } }`<br>`fn main() { one(); }`<br>Hypothesis: pass-1 sees `x = callee()` with callee's return type still unknown → `x` infers from the subsequent `x.v = value` mutation as integer (the field's type) instead of stayed unknown.  Pass-2 then can't change it to H. |
