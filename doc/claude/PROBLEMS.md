...
| 246 | File-scope `const NAME = expr;` (the `const` keyword at file scope) panics the parser with `index out of bounds: the len is 0 but the index is 65535` at `src/variables/mod.rs:938:37`.  Reproducer: a single line `const TYPE_DELTA = 2;` outside any function body triggers it.  Root cause looks like the file-scope const path calls `change_var_type(var_nr=u16::MAX, …)` without first checking the sentinel — `u16::MAX` is the "no var allocated" sentinel and the `self.variables[u16::MAX as usize]` index is out of range when `self.variables` is empty (file-scope statements run before any fn allocates locals).  The CORRECT file-scope-constant syntax is the bare-name form `NAME = value;` (see loft-write skill § Constants); `const` is for LOCALS only (`fn foo() { const x = 5; }`).  The bug is the panic message itself — the parser should diagnose "`const` declarations must appear inside a function body — at file scope use `NAME = value;`" with a clear pointer at the offending token, not crash with an internal index OOB.  Surfaced 2026-05-10 while writing v5_t2 (assumed `const` worked at file scope after misreading CLAUDE.md).  Workaround: drop the `const` keyword (`TYPE_DELTA = 2;`).  No test pinned yet — the fix-site is `parse_const_declaration` (or whichever entry-point routes file-scope `const` parses); a regression test would assert the diagnostic shape, not the absence-of-panic. | Low (open) — bad diagnostic; one-character workaround | Use the bare-name form `NAME = value;` for file-scope constants; reserve `const` for in-fn locals. |
| 245 | `parallel { server_arm(...); client_arm(...); }` hangs indefinitely when one arm performs a blocking accept on a TCP listener and the other arm tries `web::ws_handler(...)` against the same loopback port.  Both arms run on real OS threads (`std::thread::scope`), separate kernel sockets, separate `WS_CONNS` thread-locals — they should compose, but the client-side `try_open` blocks past the configured `connect_timeout(2s)` and never returns even after 30 s.  Lone-arm scenarios work in isolation: a single parallel arm calling `web::ws_handler` against an external server returns normally, and the sequential (non-parallel) `lib/server` smoke test runs cleanly.  Two compounding sub-bugs surface together: (a) parallel-arm function-call args reading from outer-scope variables (`port = 18092; parallel { server_arm(port); ... }`) arrive as 0 in the worker — the worker's stack does not contain main's locals, so the arm reads garbage; literal args (`server_arm(18092)`) work.  (b) `web::sleep_ms(N)` from a parallel-arm worker thread never returns — same set-up sleeps fine on the main thread, and a busy-wait substitute (loft for-loop) advances normally.  Surfaced 2026-05-10 while writing `v5_t1_binary_ws.loft` (the proposed self-contained binary-frame round-trip test).  Reproducer: `lib/game_protocol/examples/v5_smoke_parallel.loft` (kept locally for diagnosis; not committed).  Workaround: structure t1-t5 as separate server + client subprocess loft programs orchestrated by a Rust harness in `tests/multiplayer_v5.rs` — the same pattern that `tests/multiplayer_v2.rs` already uses successfully.  No regression: A15 was never claimed to support cross-arm I/O coordination and 80-parallel-block.loft only exercises pure-compute arms.  Likely root causes to investigate: (i) `clone_for_worker` may not clone certain native-side state needed for outbound TCP from the worker thread; (ii) `gen_parallel` (`src/state/codegen.rs:747`) emits arm bodies that bake outer-scope variable references that the worker's stack can't resolve.  Open until parallel{}+I/O composition is either (1) made to work for the natural use case or (2) explicitly documented as out-of-scope with the subprocess pattern as the recommended substitute. | Medium (open) — parallel{}+I/O hangs; outer-scope vars in arms read 0; cross-process subprocess pattern works fine | Use the subprocess + Rust-harness pattern (see `tests/multiplayer_v2.rs` / `tests/multiplayer_v5.rs`) when an integration test needs both a server and a client.  Avoid outer-scope variables in `parallel {}` arm calls — pass literals or hoist into file-scope constants. |
