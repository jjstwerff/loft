...
| [P241](#open-issues--quick-reference) | High | Building or pushing into `vector<T>` inside a generic fn crashes both backends — **needs design phase** before fix; substitution can't bridge Reference-vs-primitive IR shape locally |
...
**Pattern**: only 1 of 6 open issues (P241) remains in the
bounded-generic cluster — the four other generics bugs
(P252 + P239 + P240 + P243) closed today via
substitution-time pattern extensions in
`parser/mod.rs::substitute_type_in_value` and
`generation/emit.rs::patch_hoisted_returns` /
`generation/emit.rs::infer_type`.  P241 is structurally
harder: vector-element write IR is `OpNewRecord` +
`OpCopyRecord` (Reference shape) for parametric T at parse
time, but should be `OpPreAllocVector` + `OpSetInt` (or
typed setter) for primitive T.  The two shapes differ in op
sequence + variable types + record-type IDs, so substituting
T → primitive can't bridge them locally; needs either a
parse-time abstract-element-write op that defers shape
selection until substitution, OR a re-monomorphisation pass
that re-runs `parse_object` for each concrete T.  Either is
a substantial design phase, not a focused fix.  P250 + P251
are tuple-of-non-trivial-element bugs around per-element
projection in storage and lifetime.  P253 + P254 are
local-attacker security issues (hash-DoS + cache poisoning);
both are low priority because remote exploitation requires
loft to be deployed in a shape it doesn't currently have
(auth'd web endpoint indexed by hash; multi-tenant CI
cache).  Both have well-known fix patterns from other
languages (Rust `RandomState` for the hash; HMAC-stamped
cache filenames for the cache).
...
| 241 | Building or pushing into `vector<T>` inside a generic function (any bound or no bound) crashes both backends.  Reproducer: `fn singleton<T>(x: T) -> vector<T> { out: vector<T> = []; out += [x]; out }` invoked with `singleton(42)` — interp panics with `index out of bounds: the len is 3 but the index is 42` at `src/database/allocation.rs:347`; native rejects with rustc E0308 because the generated code emits `OpCopyRecord(cell, var_x, var__elm_1, 9_i32)` but `var_x` is typed `i64` (the substituted T), giving `expected DbRef, found i64`.  Sibling of P239 (`for x in v` over `vector<T>`).  P239 is the consume side; P241 is the construct/push side.  Same root cause family: generic-fn vector codegen always emits DbRef-shaped element ops regardless of T's concrete type.  Surfaced 2026-05-09 during plan-17 phase 05 cell write-out.  Independent of bound — even `<T>` no bound triggers it; even a single-element push.  **Investigation 2026-05-11 (during P243 closeout):** the simple substitution-time fix that closed P252 / P239 / P240 / P243 doesn't work here.  The IR shape for vector-element writes is fundamentally different between Reference T (`OpNewRecord(out, t_T, MAX); OpCopyRecord(x, _elm_, t_T); OpFinishRecord(out, _elm_, t_T, MAX)`) and primitive T (`OpPreAllocVector(out, 1, 8); OpNewRecord(out, t_int_elem, MAX); OpSetInt(_elm_, 0, x); OpFinishRecord(out, _elm_, t_int_elem, MAX)`).  Three differences: the OpPreAllocVector prefix, the OpCopyRecord-vs-OpSetInt middle op, and the record-type ID (which differs between `main_vector<T>` and the integer-vector's element struct, e.g. `i_parse_errors`).  At parse time, only the Reference shape is emitted because the parametric `T` looks like `Reference(T_d_nr)` to the type-dispatch in `vectors.rs:1511`.  At substitute time, individual op rewrites can't bridge this — the variable types, op sequence, and record IDs would all need to change consistently, AND substitution doesn't have access to the database's vector-element record type for the substituted concrete T.  Likely fix paths: **(a)** parse-time abstract `OpAddVecElem<T>` op that defers shape selection until substitute (needs new opcode + runtime + native dispatchers); **(b)** re-monomorphisation pass that re-runs `parse_object` for each concrete T (clean but expensive); **(c)** generic-vector representation that ALWAYS uses Reference-shape storage (with auto-boxing for primitives), losing the per-T optimisation but unifying the path.  Each is a substantial design phase, not a focused fix.  Workaround: caller specialises to a concrete type, or take a `fn(T) -> ...` callback so vector construction happens at the call site. | High (open) — interp panic + native compile error on basic shape; needs design phase before fix | No clean workaround; caller must specialise to concrete element types or use a `fn(T)` callback so vector construction happens at the call site. |
