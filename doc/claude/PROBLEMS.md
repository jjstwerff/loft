...
| 181 | **Root cause identified; latent, not a Step-21 regression.**  An inline struct-returning call inside an assertion's **format-string interpolation** — e.g. `"hex raised; got {map_get_hex(e.es_map, 3, 2, 0).h_height}"` — is transformed by `src/scopes.rs`'s inline-call lift pattern (P135 family) into `__lift_N = map_get_hex(...)` + `OpCopyRecord(src, to=__lift_N, tp = def_nr \| 0x8000)`.  The **0x8000 free-source flag** on `OpCopyRecord` frees the source DbRef after copying.  When `map_get_hex`'s return is a view into a vector element (e.g. `gh_c.ck_hexes[idx]`) the freed "source" is part of the owning vector's store — corrupting sibling fields of the same `EditorState` a few instructions later.  Symptom in `lib/moros_sim/tests/picking.loft::test_edit_at_hex_raise`: `edit_at_hex` runs and `undo_depth` internally reads `1`; the subsequent inline `{map_get_hex(...)}` interpolation clobbers memory so that `undo_depth(e.es_undo)` from the caller reads `0`.  Predates Step 21 — the layout change just reshaped the corrupted bytes enough to flip the undo-stack length from some other value into the "looks empty" one.  Always-on latent in any codebase that combines an inline struct-returning call inside a format string with a store that has sibling fields of the same parent. | Medium | **Workaround (applied):** hoist the inline call result into a local binding and reference the local in both the assertion condition AND the message.  The `test_edit_at_hex_raise` fix does exactly this (`h = map_get_hex(…); assert(h.h_height == 4, "… got {h.h_height}"); d = undo_depth(e.es_undo); assert(d == 1, "… got {d}")`).  **Fix path:** audit `src/scopes.rs::scan_args` (or whatever picks the 0x8000 flag for inline-lift OpCopyRecord).  The flag is correct for constructor calls that allocate a fresh store, but wrong for accessor calls that return a vector-element / struct-field view.  Decide per-callee: a return type carrying a `depend()` chain means "borrowed view" and the flag must be cleared.  Regression expectation: any test using inline `{fn_returning_struct(…).field}` interpolation without the workaround should still compile; runtime corruption should go away once the flag is gated on ownership. |
