...
| 155 | Push/mutate/undo/mid-assert/redo/final-read sequence SIGSEGVs in `OpGetVector` | **High** (crash) | Omit the mid-assert between undo and redo, or inline `c.items[i]` rather than going through a helper fn |
| 156 | `struct X { v: vector<C> }` where C is the name of a stdlib constant (e.g. `E`, `PI`) panics in `typedef.rs:309` instead of emitting the "struct conflicts with constant" diagnostic | Low | Rename the struct (e.g. `Elem`, `PiType`); `sorted<C[k]>` and `hash<C[k]>` already emit the clean error |
