...
| [P253](#open-issues--quick-reference) | Low (security) | Hash-table collision DoS — `keys::hash` uses `DefaultHasher::new()` with fixed seed; attacker-controlled keys collide deterministically, O(N²) insertion |
| [P254](#open-issues--quick-reference) | Low (security) | Native binary cache poisoning — `loft --native` executes `.loft/cache/<file>-<hash>` without integrity verification when the file exists |

**Pattern**: 3 of 8 open issues (P240 / P241 / P243) are in
the bounded-generic + something-specific cluster (tuple
return + 2 operator-locals, vector construction, text-in-tuple).
Same root-cause neighborhood as the just-closed P252 + P239;
fix family is mostly the same machinery (extending parser /
codegen pattern-matches to recognise the Nullable peers AND/OR
specialise vector / text shapes correctly through the
type-variable substitution).  P250 + P251 are tuple-of-
non-trivial-element bugs around per-element projection in
storage and lifetime.  P253 + P254 are local-attacker security
issues (hash-DoS + cache poisoning); both are low priority
because remote exploitation requires loft to be deployed in a
shape it doesn't currently have (auth'd web endpoint indexed
by hash; multi-tenant CI cache).  Both have well-known fix
patterns from other languages (Rust `RandomState` for the
hash; HMAC-stamped cache filenames for the cache).
...
| 240 | A bounded-generic function whose body computes TWO OR MORE bound-supplied operator results into local variables, then returns them in a tuple constructor, produces wrong values under one backend in a way that depends on whether the body has intervening side-effects (e.g., a `println!`).  Reproducer: `fn classify<T: Ordered>(a: T, b: T) -> (integer, integer) { lt = if a < b { 1 } else { 0 }; gt = if a > b { 1 } else { 0 }; (lt, gt) }` invoked with `classify(3, 5)` — **interp returns `(0, 0)`** (WRONG; expected `(1, 0)`); native returns `(1, 0)` (correct).  Adding a `println("BODY: lt={lt} gt={gt}")` in the body BEFORE the tuple constructor flips which backend is correct: interp now returns `(1, 0)` (correct) and **native returns `(0, 0)`** (WRONG).  Concrete-type version `classify_concrete(a: integer, b: integer)` works on both.  Single-bounded with ONE bound-operator local also works (`pair_eq<T: Equatable>(a, b) -> (integer, integer) { eq = ...; (eq, 99) }` returns `(1, 99)` correctly on both).  The bug requires: (a) bounded-generic context, (b) two-or-more locals computed from bound-supplied operators, (c) tuple constructor referring to those locals.  Surfaced 2026-05-09 during plan-17 phase 04 cell write-out for U7×B2.  Likely root cause: the bound-method-resolution path's per-pass slot/temp allocation for bounded operator-call results conflicts with the tuple constructor's per-element evaluation order; the side-effecting println shifts which slot gets clobbered.  Pinned by /tmp/p_followups/p240_two_locals_then_tuple.loft. | High (open) — silent value swap depending on body side-effects; backend-dependent direction | No clean workaround.  Inlining the operator into the tuple constructor itself triggers P237.  Best workaround: extract the entire pattern into a non-generic helper (concrete-typed body works correctly). |
| 241 | Building or pushing into `vector<T>` inside a generic function (any bound or no bound) crashes both backends.  Reproducer: `fn singleton<T>(x: T) -> vector<T> { out: vector<T> = []; out += [x]; out }` invoked with `singleton(42)` — **interp panics** with `index out of bounds: the len is 3 but the index is 42` at `src/database/allocation.rs:347`; **native rejects with rustc E0308** because the generated code emits `OpCopyRecord(cell, var_x, var__elm_1, 9_i32)` but `var_x` is typed `i64` (the substituted T), giving `expected DbRef, found i64`.  Sibling of P239 (`for x in v` over `vector<T>`).  P239 is the consume side; P241 is the construct/push side.  Same root cause family: generic-fn vector codegen always emits DbRef-shaped element ops regardless of T's concrete type.  Surfaced 2026-05-09 during plan-17 phase 05 cell write-out.  Independent of bound — even `<T>` no bound triggers it; even a single-element push.  Likely fix: extend `parse_object` / `OpCopyRecord` codegen to substitute T into the element-copy op (`OpCopyRecord` for DbRef vs `OpSetInt` for primitives, etc.).  No clean workaround that keeps the function generic — caller must specialise to a concrete type, or take a `fn(T) -> ...` callback so vector construction happens at the call site.  Pinned by /tmp/p_followups/p241_*.loft. | High (open) — interp panic + native compile error on basic shape | No clean workaround; caller must specialise to concrete element types. |
...
| 243 | A bounded-generic function returning a tuple containing one or more `text` elements where one element is built by a bound-supplied method call (e.g. `fn show_pair<T: Printable>(x: T) -> (text, text) { (x.to_text(), "x") }`) discards the body's tuple expression as a statement and synthesises a `(String::new(), String::new())` return on `--native`.  Reproducer: `show_pair(42)` returns `("", "")` on native (interp returns `("42", "x")` correctly).  The emitted Rust looks like `(t_7integer_to_text(...), "x"); ; return (String::new(), String::new())` — the tuple expression is at statement position, not return position.  Distinct from P237 (the bound-method call IS now correctly resolved to the concrete `t_7integer_to_text` thanks to the P237 fix; the residual bug is that the tuple-of-text return-position emission for generic monomorphisations doesn't elevate the trailing tuple to a return).  Likely root cause: the parser's tuple-of-text → synthetic-struct rewrite (Plan-14 phase 07 / P234) only fires for non-generic fn bodies; generic bodies retain their `Value::Tuple` tail, but the codegen emits it as a statement instead of a return for the text-tuple shape.  The integer-tuple analogue (`pair_t<T: Equatable>(a: T) -> (T, T)` with T=integer) works because Plan-14's text-only synthetic-struct rewrite doesn't touch it.  Surfaced 2026-05-09 during plan-17 P237 closure validation.  Workaround: hoist each text-tuple element into a local first.  Pinned by /tmp/p_followups/p243_text_tuple_return_from_generic.loft. | High (open) — silent native miscompilation (returns empty strings); interp correct | Hoist each text-tuple element into a local first: `s = x.to_text(); t = "x"; (s, t)` instead of `(x.to_text(), "x")`. |
...
| 254 | **Security — local cache poisoning.**  `loft --native script.loft` checks `.loft/cache/<source_stem>-<source_hash>` (`src/main.rs:2286`) and, if the file exists, executes it directly via `Command::new(&binary)` (`src/main.rs:2485`) without verifying that loft itself produced it.  The filename is content-addressed (`<u64 hash>` of source bytes + `--native-release`/`--native-debug` flags + `libloft.rlib` mtime + per-package native rlib mtimes) so an attacker who knows the source AND the host's libloft.rlib mtime can pre-compute the cache filename and drop a malicious ELF there.  The cache directory at `<source_dir>/.loft/cache/` is created with default mode (typically `775` — group-writable on most umasks); cached binaries are mode `755` (executable).  No HMAC, no signature, no owner check at execute time.  **Threat scenarios** (low severity — local file access required): shared dev machine where a colleague drops a poisoned cache before you run; cloned project that ships a `.loft/cache/` directory unintentionally (the dir lives next to source so it travels via `git clone` unless `.gitignore`d); CI cache volumes restored from a previous job's artifact when another tenant could write to the artifact storage.  Same shape as the well-known `ccache` / `sccache` / Bazel-remote-cache poisoning class.  Surfaced 2026-05-11 during a security audit prompted by the user pointing at the hash-DoS (P253).  **Likely fix (cheapest first)**: (a) embed an HMAC of the source bytes + a per-user secret key (read from `~/.loft/cache_secret`, mode `0600`) into the cached filename; refuse to use cache entries without a matching HMAC; (b) refuse to use the cache if the directory is group/other-writable, and `chmod 0700` on creation; (c) re-stat at execute time and verify the file's owner matches the current uid; (d) hash the binary content and compare against an expected hash in a sidecar file (overkill — doubles disk I/O).  Pin (a) is the right default; (b) and (c) are good defense-in-depth.  No regression test yet — needs a fixture that creates a fake cache file and asserts loft refuses (or warns) to execute it under each mitigation. | Low (open) — local-machine attacker required; no remote exploit | Delete `.loft/cache/` before each run; or set `LOFT_NATIVE_NO_CACHE=1` (not yet implemented — would need adding) to bypass cache entirely. |
| 253 | **Security — hash-table collision DoS.**  `src/keys.rs:325` uses `std::collections::hash_map::DefaultHasher::new()` which constructs a SipHash-1-3 hasher with **fixed seed (k0=0, k1=0)** instead of the seeded `RandomState::new().build_hasher()`.  Every loft process uses the identical hash function, so an attacker who can supply hash-table keys (e.g., HTTP query params hashed into a `hash<Entry[key]>`, JSON object keys parsed into a hash collection, log-tag strings indexed by a hash) can pre-compute N strings that all collide to a single bucket.  Insertion is then O(N²) (linear-probe through the bucket chain at `src/hash.rs:52-67`), and lookup is O(N).  Rehash on growth (`src/hash.rs:21-41`) does NOT break the cluster: it uses the same `keys::hash` function with the same fixed seed, so colliding keys still collide in the larger table.  Same root-cause class as the famous 2011/2012 hash-DoS in Python / Ruby / PHP / Java / Node.js (CVE-2011-4815 et al.).  Severity is bounded by who can supply keys: today's `lib/web` accepts attacker-controlled strings (HTTP query params, JSON body, WebSocket frames) and a hash-keyed library could move CPU usage from O(1) to O(N²) on a few hundred KB of crafted input — a small server takes seconds-to-minutes per request, large enough to DoS a single-threaded async loop.  Surfaced 2026-05-11 by the user during a security audit.  **Fix**: replace `DefaultHasher::new()` in `src/keys.rs::hash` and `key_hash` with a properly-seeded hasher.  Three options: (a) `std::collections::hash_map::RandomState` (per-process seed via `getrandom`; standard library; zero deps); (b) `ahash` crate (faster, also seeded); (c) a per-`Stores`-instance seed stored on the struct + `SipHasher::new_with_keys(k0, k1)` (deterministic within a process, varies between processes — same property as RandomState but explicit storage).  Option (a) is the smallest change.  After the fix, the rehash-on-growth path naturally redistributes existing keys via the seeded hash so the resize behaviour is also correct.  **Compatibility note**: changing the hash function changes iteration order of hash collections.  Loft's `hash` is documented as unordered (`STDLIB.md` § Hash collections — "Hash cannot be iterated directly") so user-visible behaviour shouldn't break, but tests that rely on specific bucket placement would.  Audit `tests/scripts/37-stress.loft` and `tests/lib/*.loft` hash sections before flipping.  No regression test yet — would need an attack fixture that times insertion of N pre-computed colliding keys vs N random keys; assert the ratio stays bounded after the fix. | Low (open) — DoS only when attacker controls hash-table keys; not memory corruption | Don't accept attacker-controlled keys into hash collections; or transform incoming keys (e.g., `BLAKE3(salt + key)`) before insertion. |
...
| 251 | Storing a tuple whose element is a `Type::Function` (closure / fn-ref) INTO a struct field fails native compilation with rustc E0605 `(u32, DbRef) as i32`.  Reproducer: `struct S { t: (fn(integer) -> integer, integer) } fn main() { add5 = fn(x: integer) -> integer { x + 5 }; s = S { t: (add5, 99) }; print("{s.t.1}\n"); }` — interp prints `99`, native rejects with `let _v_val = ((var__elm_3.0).0 as i64); let _v_val_clamped = if _v_val == i64::MIN { i32::MIN } else { _v_val as i32 }; … set_i32_raw(…)` where `var__elm_3.0` is the `(u32, DbRef)` runtime fn-ref tuple — the projection `(...).0` to get the u32 d_nr is missing on this code path even though P196 added it for the OTHER fn-ref-in-tuple-in-struct shape (Var-of-fn-ref-tuple source).  Surfaced 2026-05-11 during plan-14 phase 05 D3 cell write-out.  Calling through the field (`s.t.0(arg)`) compounds the issue: even the workaround `f = s.t.0; f(arg)` panics interp with "index out of bounds: the len is 3 but the index is 10" at `src/database/allocation.rs:162` (calling a wildly-out-of-range `d_nr` because the projection bug feeds garbage into the call dispatch).  Likely fix: extend the same per-element fn-ref-tuple projection P196 added to the codegen path that handles "tuple-with-fn-ref-element being stored into a struct field" — currently only the path for "fn-ref-tuple struct field directly" is covered; the wrapper case (struct field is a tuple where ONE element is fn-ref) needs the same `(...).0` projection inside its element-write loop.  Workaround: store the fn-ref in a separate field, not nested inside a tuple struct field.  Pinned by `/tmp/p_d3_e4_minimal.loft`; the e4_d3_field_closure_local cell is intentionally omitted from `tests/tuple_matrix.rs` (see comment at the e5_d3_field_struct_ref entry) until P251 closes. | Medium (open) — silent native compile failure (interp passes); the call-through-field shape produces interp panic too | Use a separate `fn_field` instead of nesting fn-ref inside a tuple struct field. |
| 250 | Tuple-of-`Reference` returned from a function and destructured inside a loop body shows a **stale-DbRef on the destination variable that picked up the FIRST argument** on iterations >0.  Reproducer: `struct P { v: integer }; fn make_pair(a: P, b: P) -> (P, P) { (a, b) } fn main() { for i in 0..5 { pa = P{v:i}; pb = P{v:i+100}; (q1, q2) = make_pair(pa, pb); print("{i}: {q1.v},{q2.v}\n"); } }` prints `0: 0,100` then `1: null,101` `2: null,102` … on BOTH backends.  `q1` (= `a`'s ref, the first arg) reads `null` once the loop body re-enters its scope; `q2` (= `b`'s ref, the second arg) stays correct.  Swap variant `(b, a)` shows the same shape — the bug is positional (FIRST arg's ref), not order-of-declaration.  Surfaced 2026-05-11 during plan-14 phase 04 pre-flight (otherwise all single-iteration E5 cells pass).  Two-call same-scope variant works (`(q1, q2) = make_pair(pa1, pb1); (r1, r2) = make_pair(pa2, pb2);` prints both pairs correctly), so the trigger is loop-iter scope re-entry combined with same-name argument variable reassignment.  Likely root cause: the synthetic `__tuple<...>` struct's first DbRef element shares storage / dep-tracking with the source argument's slot, and when iter-0's cleanup frees `pa`'s store, the destructured `q1` (which has only a copy of `pa`'s OLD DbRef, not an independent owning ref) reads from a slot that's been freed and reused by the next iter's `pa = P{v:1}`.  Probably needs either: (a) the destructure-from-synthetic-struct path in `src/parser/expressions.rs` (lines 1260-1276) to deep-COPY each element record into the destination var (not just project the DbRef field), so each `q1` / `q2` owns its own store; or (b) propagate proper dep links from `q1` / `q2` to `pa` / `pb` so cleanup order keeps the source alive until the destructure is read.  Workaround: hoist the destructure out of the loop body, or call the function once per iteration with two-call shape.  Pinned by `/tmp/p_e5_loop_simple.loft` (intentionally not added as a tuple_matrix cell — see comment in `tests/tuple_matrix.rs` at the e5 section). | Medium (open) — silent miscompile under loops on both backends; positional (FIRST arg only) | Hoist the destructure out of the loop, or use a struct field instead of a tuple element to hold the ref. |
