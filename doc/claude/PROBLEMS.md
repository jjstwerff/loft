...
| 236 | A function whose return type is a heap-owned reference (`Reference`, `Vector`, struct-enum) and whose body's tail expression is an `if/else` (or `match`) returning newly-constructed records corrupts the return value on `--native` (returns the typed null sentinel `DbRef { store_nr: u16::MAX, rec: 0, pos: 8 }` instead of the if/else's value).  Interpreter works because `OpReturn(value=12)` reads from the eval-stack top (which the if/else branches populate); native generates `if cond { …; w } else { …; w }; OpFreeRef(w); return DbRef::null();` and the if/else's value is dropped on the floor.  Reproducer: `fn make_p(c: bool) -> P { if c { P{x:1, y:2} } else { P{x:3, y:4} } }` — interp prints `(1, 2)`, native prints `(null, null)`.  Verified pre-existing (eebaaa4 worktree, 2026-05-08).  Surfaced 2026-05-08 during ARC.md A7.1 (par tuple wide-return) when widening the parse_function gate to route pure-value tuple returns through `Reference(__tuple<…>)` exposed the same shape — `min_max(...) -> (integer, integer) { if cond { (a, b) } else { (b, a) } }` regressed `tests/docs/28-tuples.loft`.  Root cause: scopes.rs (line 877+) `is_value_return_type` only matches scalars; for heap-owned returns, the tail expression's value is dropped and `Return(Null)` is appended after free ops.  The B5-L3 wrap (save tail to `__ret_N` temp, run free ops, return temp) cannot apply to Reference returns naively because OpFreeRef on the tail's work-ref would free what `__ret_N` points to.  Fix shape (estimated M-L): extend `ref_return` to track work-refs reachable through If/Match tail branches (propagate dep through control-flow), and either skip OpFreeRef on hidden-arg work-refs OR have B5-L3 use the work-ref as the temp directly.  **Blocks A7.1** (the 3 remaining par_tuple_return canaries that need the wide-return widen — `int_int`, `three_arity`, `nested`).  Workaround: compute the tuple/struct in a local first, return the local — `fn min_max(...) { result = if cond { (a, b) } else { (b, a) }; result }` (verified workaround for the tuple case via P234 runtime path; structs unverified). | High (open) — silent data corruption | Compute the result in a local variable first, return the local: `result = if cond { ... }; result`. |
