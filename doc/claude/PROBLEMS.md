...
| 180 | Assigning a `single` (f32) literal to a `float` (f64) struct field is silently accepted by the parser and corrupts the record at runtime.  Minimal repro: `struct P { a: float not null, b: integer not null } fn main() { p = P { a: 1.0, b: 42 }; p.a = 1.2f; }` — interpreter panics `index out of bounds: the len is 3 but the index is 8` at `src/database/allocation.rs:362:30`.  Under `--native`, rustc aborts with E0308 "expected f64, found f32" in the generated code (which at least surfaces the mismatch, but the raw rustc error leaks up to the user instead of a proper loft diagnostic).  Symmetric cases (float literal → single field, and return-type mismatches in both directions) are likely affected too, not verified.  Surfaced while writing Step 16 mouse-look tests where `pl_facing: float` was set via `z.es_player.pl_facing = 1.2f`. | Medium | **Workaround:** use literal suffixes that match the target type — `1.2` for `float` fields (no suffix = f64), `1.2f` for `single` fields, explicit cast otherwise.  **Fix path:** the assignment type-check for struct field writes should reject `single` / `float` mixing without an explicit `as` cast.  Start at `src/parser/operators.rs` / `src/parser/objects.rs` wherever field-assignment type narrowing happens; the type system already distinguishes `Type::Float` and `Type::Single` for declarations — the assignment path needs to consult it.  Regression fixture at `tests/lib/p180_single_to_float_field.loft`; `tests/issues.rs::p180_single_literal_into_float_field` gated `#[ignore]` until fixed. |
