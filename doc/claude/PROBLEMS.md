...
| 237 | A bounded-generic function returning a tuple where ANY element expression contains a bound-supplied operator call (e.g. `(x + x, 1)`, `(a, a + b)`, `(a == b, c)`) breaks both backends.  Reproducer: `fn pair_with_one<T: Addable>(x: T) -> (T, integer) { (x + x, 1) }` invoked with `pair_with_one(5)` — **interp SIGSEGVs** (or returns garbage like `21509196218368`); **native rejects with rustc E0308** because the call-site monomorphisation substitutes `T → i64` for the parameter type but the codegen for the body's operator call still references `t_1T_OpAdd(cell, var_x, var_x) -> DbRef`, giving a return-type mismatch (`expected i64, found DbRef`).  Distinct from P208 which was about text-return wrap.  Pre-flight Phase 02 of plan-17 surfaced this 2026-05-09; broadened during cell write-out same day when `(a, a + b)` (uniform-T tuple) failed identically to `(x + x, 1)` (mixed-T tuple) — the bug isn't about the element type variation, it's about T's operator INSIDE a tuple constructor element.  Uniform-T tuple-of-T returns where elements are simple var refs (`-> (T, T)` returning `(a, b)`) work — the (A) fix in `Parser::substitute_type` for `Type::Tuple` recurses through the elements, but the second-pass instantiation's body codegen doesn't substitute T inside operator-method calls when those calls live inside a tuple constructor.  Likely fix: extend the second-pass fn body substitution to recurse through tuple-element expressions and rewrite each `Call(t_1T_OpAdd, …)` to `Call(t_<concrete>_OpAdd, …)` per the bound-method-resolution path.  Workaround (verified): hoist the operator-using element to a local first — `s = x + x; (s, 1)` instead of `(x + x, 1)`.  Pinned by /tmp/p_followups/p237_mixed_generic_tuple.loft. | High (open) — interp silent corruption + native compile error | Hoist any operator-using element to a typed local first: `s: integer = x + x; t = (s, 1);` instead of inlining `(x + x, 1)`. |
| 238 | Bounded-generic function with `T: Equatable` (or any text-satisfying bound) returning a UNIFORM tuple of `T` (`-> (T, T)`) instantiated for `T = text` fails native compilation with rustc E0308 `expected &str, found String`.  Reproducer: `fn pair_t<T: Equatable>(a: T) -> (T, T) { (a, a) }` then `s = pair_t("hi");` — interp prints `hi|hi|hi|hi` correctly; native rejects with `let mut var_s: (String, String) = t_4text_pair_t(cell, "hi".to_string());` — call site passes `String` but the generated specialised fn's signature expects `&str`.  Distinct from P237 (mixed types) and P208 (text-return wrap).  Pre-flight Phase 02 of plan-17 surfaced this 2026-05-09.  Uniform-T `(T, T)` returns work for `T = integer` / `float` — the bug is text-specific.  Likely fix: in `src/generation/calls.rs::output_call`, the text-T monomorphisation needs to wrap the arg with `Str::new(&...)` instead of `.to_string()`, OR the specialised fn signature needs to accept `String` to match the call site.  Workaround: use `--interpret`. | High (open) — native silent compile-error on a basic shape | Use `--interpret` for now; or wrap the text input in an explicit local before passing. |
...
| 242 | Format-string interpolation of a `T` variable inside a generic function body (`println!("val={x}")` where `x: T`) fails on both backends.  Reproducer: `fn show<T: Printable>(x: T) { println("val={x}") }` invoked with `show(7)` — **interp prints `val=null` then panics** at `raw_vec/mod.rs:812` (`Tried to shrink to a larger capacity`); **native rejects with rustc E0308** because the codegen emits `OpFormatDatabase(cell, &mut work, var_x, 9_i32, 0_i32)` but `var_x` is typed `i64` (the substituted T), giving `expected DbRef, found i64`.  Sibling of P237 (operator-inside-tuple-element), P239 / P241 (vector ops): all share the root cause that the generic-fn codegen emits DbRef-shaped ops for T-typed values without substituting T's concrete type.  P242 is the format-string consumer of T; default codegen path for an unknown-type interpolation falls back to the DbRef formatter.  Surfaced 2026-05-09 during plan-17 phase 05 cell write-out.  Workaround verified: call `.to_text()` explicitly first, then interpolate the resulting text — `s = x.to_text(); println("val={s}")` works on both backends across integer / float / text monomorphisations.  Pinned by /tmp/p_followups/p242_*.loft. | High (open) — interp produces "null" then panics; native compile error | Hoist the value to a `text` local via explicit `to_text()` before interpolating: `s = x.to_text(); println("val={s}")` instead of `println("val={x}")`. |
