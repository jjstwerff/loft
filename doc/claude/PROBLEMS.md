...
| 209 | Match guard arms select wrong branch on **both** backends, *differently* wrong on each.  Reproducer: `match n { x if x < 0 => "neg", x if x == 0 => "zero", _ => "pos" }`.  For inputs `-3 / 0 / 7` interp returns `zero / neg / zero` (arms appear shifted by one); native returns `zero / zero / zero` (always second arm).  Surfaced by plan-18 phase 02 pre-flight (2026-05-04).  Likely a parser/match-arm body-binding bug — the `x if cond` pattern probably binds `x` to the wrong scrutinee value or the guard expression doesn't see the bound `x`.  Most common conditional-match idiom; high user-visible severity.  Reproducer in /tmp/p18_p02_probes/guard_pattern.loft. | High (S0, open) | Use `if/else if/else` chain instead of `match … if`. |
| 210 | Native coroutine `yield integer` + `for v in gen() { sum = sum + v; }` silently produces 0 instead of the correct sum.  Interp returns the correct value (10 for `count_to(5)`).  Reproducer: `fn count_to(n: integer) -> iterator<integer> { i = 0; while i < n { yield i; i = i + 1; } }`.  Either the for-loop body never executes under native, or `v` always reads as 0.  Coroutine state-machine lowering bug.  Surfaced by plan-16 pre-flight (2026-05-04).  Reproducer in /tmp/p16_probes/y1_x1_int_for.loft. | High (S0, open) | Use interp for coroutine code until fixed. |
...
| 212 | Nested tuple literals (`((1,2), (3,4))` or any tuple containing a tuple) panic at `src/state/codegen.rs:1527:38` on **both** interp and native.  Single shared root cause across all 3 plan-14 phase 02 probes (local var, return value, triply-nested).  Surfaced by plan-14 phase 02 pre-flight (2026-05-04).  Reproducers in /tmp/p14_p02_probes/. | High (S1, open) | Decompose into separate scalars: `t0a = 1; t0b = 2; …` then access individually. |
| 213 | Capturing closure stored in struct field panics under interp at `src/store.rs:963` ("Write to locked store") and rejects in native with E0308 type mismatch.  Affects all of C1/D3, C2/D3, C3/D3 (basic-type, text, and Reference captures).  Single shared root cause: closure record DbRef isn't being widened correctly when written into the struct field slot.  Surfaced by plan-15 pre-flight (2026-05-04).  Reproducers in /tmp/p15_probes/c{1,2,3}_d3_*.loft. | High (S2, open) | Don't store capturing lambdas in struct fields; use a top-level fn or pass via fn-typed parameter. |
