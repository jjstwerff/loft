...
| 176 | `&T` parameter passed to a method-style callee (`self: T` signature) that mutates via `self.field += …` trips the "Parameter '<p>' has `&` but is never modified; remove the `&`" compile error.  The compiler doesn't see the callee's internal `self.field` mutation as modifying the caller's `&T` argument.  Minimal repro: `struct Box { items: vector<integer> }; fn add(self: Box, x: integer) { self.items += [x]; }; fn caller(b: &Box) { add(b, 1); }` → error at caller's `b` parameter.  Using direct `b.items += [x]` inside `caller` works; using `self`-method style does not.  Surfaced while implementing `emit_player_avatar` in `moros_render`: switched from `add_vertex(m, …)` / `add_triangle(m, …)` graphics-lib methods to direct `m.vertices += [...]` / `m.triangles += [Triangle { … }]` mutation. | Low-Medium | **Workaround:** inside any function taking `&T`, mutate the struct's fields directly rather than via method-style calls.  Loft accepts `v.field += [x]` through a `&`-param but not a method indirection.  **Fix path:** `src/scopes.rs::find_written_vars` (or the "was this `&` parameter mutated?" analysis in parser/scope): teach it that a method call `f(self_arg, …)` where the callee's body contains `self.field = …` / `self.field += …` counts as a mutation of `self_arg`, propagating the "mutated" flag to the caller's `&` parameter.  Today the analysis is purely syntactic per function body. |
...
| 179 | Passing a `&struct.field` expression as an argument alongside other arguments produces silently corrupted reads in the callee.  Concretely, given `fn f(n: integer, r: &T) { r.x = n; }` and `o = Outer { ... }`, the call `f(42, o.field)` sets `r.x = 3` (not 42) — `n` inside the callee reads a garbage-looking integer (appears to be a fragment of the DbRef representation of the `&` arg).  Boolean / text / second `&` args corrupt similarly.  Two-`&`-siblings variant (`f(o.a, o.b)` where both are `&T`) loses BOTH mutations.  Aliasing each field to a local first — `la = o.a; lb = o.b; f(la, lb);` — works, as does the single-`&`-arg case (`f(o.field)` with no preceding args).  Surfaced while writing `editor_tick` in `lib/moros_sim/src/editor.loft`: `undo_pop(st.es_undo, st.es_map)` silently failed to restore the map state, while the local-alias rewrite works.  Minimal repro at `tests/lib/p179_ref_field_arg_corrupts_siblings.loft`. | Medium | **Workaround:** before a call that passes `&struct.field` as anything other than the sole argument, alias every `&` source to a local binding and pass the locals.  **Fix path:** almost certainly a slot-allocator or argument-materialisation bug where `Value::Get(struct, field)` as a `&`-arg expression overlaps with the preceding positional arg's slot.  Start at `src/parser/control.rs::parse_call` (how ref-source expressions are materialised for struct-field accessors) and `src/state/codegen.rs` arg-push codegen.  Regression fixture exists; an `#[ignore]`-gated `tests/issues.rs::p179_*` regression test is pending. |
