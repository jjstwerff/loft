...
| ~~146~~ | Wrap-suite store leak on user-fn alias return + deep-copy | — | **Fixed 2026-04-16** — added the missing companion to the existing skip-free branch in `src/scopes.rs::scan_set` (lines 493-526): when the call's `has_ref_params == true` (codegen will take `gen_set_first_ref_call_copy`'s deep-copy path), strip the LHS variable's parser-inferred deps via `make_independent` so `get_free_vars` emits OpFreeRef at scope exit.  Without this, the parser's "borrows from arg N" inference suppressed the FreeRef and the deep-copied independent store leaked.  Tests: `tests/leak.rs::p146_script_95_alias_copy_leak` (un-`#[ignore]`'d, panics on regression via `execute_log`).  Closes script 95's leak (1 store).  Scripts 45/62/76/81 still leak with different shapes — see P147. |
| 147 | Wrap-suite leaks on scripts 45/62/76/81 (not P146 family) | Medium | **Investigating 2026-04-16.**  After the P146 fix landed, four wrap-suite scripts still report leaks: `45-field-iter.loft` (8 stores at bc:6514+), `62-index-range-queries.loft` (1 at bc:4561), `76-struct-vector-return.loft` (1 at bc:4561), `81-iterator-protocol.loft` (3 at bc:4686/4875/5077).  Initial trace of script 76 (constructor returning struct-with-vector) hit a SIGSEGV in `execute_log` deep inside `test_p105_nested_struct_in_vector` — `OpCopyRecord(data=ref(0,0,0x40080000), to=ref(2,5,24), tp=68)` — bogus source DbRef pointing at store_nr=0.  Distinct shape from P146; needs separate investigation per script. |
