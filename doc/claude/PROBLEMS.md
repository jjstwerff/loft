...
| 239 | A `for x in v { ... }` loop over `vector<T>` inside a generic function (any bound or no bound) crashes both backends.  Reproducer: `fn count<T>(v: vector<T>) -> integer { n = 0; for x in v { n = n + 1; } n }` invoked with a `vector<integer>` — **interp SIGSEGVs** on opcode dispatch; **native rejects with rustc E0610** (`i64` is a primitive type and therefore doesn't have fields) because the generated code emits `if !((var_x).rec != 0) { ... }` — `var_x` typed as `i64` (the substituted T) is treated as a `DbRef` (using `.rec` to test for null).  The generic-fn iterator codegen path doesn't substitute T into the for-loop's iterator-termination check; it always emits the DbRef-shaped null-check regardless of whether T monomorphises to a primitive.  Surfaced 2026-05-09 during plan-17 phase 03 cell write-out.  Independent of Printable / Equatable bounds — even `<T>` no bound triggers it, even with `x` unused in the body.  Likely fix: in `src/parser/collections.rs::parse_for` or the iterator codegen, generate a type-aware termination check (sentinel `i64::MIN` for integer T, `&str` empty for text T, etc.) instead of a hardcoded `var.rec != 0`.  No simple workaround that keeps the function generic — caller must specialise to a concrete type, OR the generic body must take a callback `<T>(v: vector<T>, f: fn(T))` and let the caller handle iteration outside the generic boundary.  Pinned by /tmp/p_followups/p239_generic_vector_iter.loft. | High (open) — interp SIGSEGV + native compile error on basic shape | No clean workaround; caller must specialise the function to concrete element types or use a `fn(T)` callback so iteration happens at the call site. |
...
| 252 | Bounded-generic for-loop over a struct-ref vector: the bound method dispatched on the loop variable returns the FIRST item's result for every iteration, instead of the per-item result.  Reproducer: `interface V { fn ok(self: Self) -> boolean } struct P { v: integer } fn ok(self: P) -> boolean { return self.v > 0; } fn check<T: V>(items: vector<T>) { for it in items { print("v?={it.ok()}\n"); } } fn main() { check([P{v:1}, P{v:0}, P{v:3}]); }` prints `v?=true / v?=true / v?=true` (3× true) instead of `v?=true / v?=false / v?=true`.  The non-generic variant (`for it in [P{...}, ...] { print("v?={ok(it)}\n"); }`) works correctly — bug requires the bound-method dispatch path AND the for-loop iteration combined.  The generic non-loop variant (`fn check<T: V>(it: T) -> boolean { it.ok() }` called 3× with explicit args) also works — bug requires both pieces.  Both backends agree on the wrong answer.  Surfaced 2026-05-11 during plan-07 phase 4 cleanup; bisect points at slice-3 commit `6016655e` (vec/text OOB raise + Nullable opcode peers).  The slice-3 reroute swapped `OpVectorRef` to `OpVectorRefNullable` in the for-loop iter step (parser/collections.rs:217-237); the Nullable opcode body is identical to the original (`stores.get_ref(&vector::get_vector(&@r, 4, @index, &s.database.allocations), 0)`), so the bug must live in some downstream pattern-match or dispatch that recognises `OpVectorRef` but NOT `OpVectorRefNullable` — most likely the bounded-generic specialisation re-resolution path that copies the loop var's value into the t_<n>T_ok stub's argument slot.  Likely fix: extend the bound-method-resolution / re-resolve path to treat `OpVectorRefNullable` equivalently to `OpVectorRef`, OR have the loop iteration step always assign the result into a named variable (so the generic stub call sees a stable arg position rather than reaching back into the iter expression).  Workaround: extract the bound call into a non-generic helper, or hoist `it` into a temp before the bound call (`for it in items { ti = it; if ti.ok() { ... } }` — needs verification).  Pinned by /tmp/p_iface_loop_print.loft. | Medium (open) — silent miscompile (returns wrong boolean per loop iter); only generic-bound + for-loop combination | Extract bound call into a non-generic helper, or use indexed iteration (`for i in 0..len(items) { items[i].ok() }`) which routes through a different codegen path. |
