...
| 247 | Native codegen E0382 / E0597 when a nested tuple containing a non-Copy element (text / Reference) is read for format-string interpolation: `t = ((1, "a"), (2, "b")); print("{t.0.1}|{t.1.1}\n");` rejects with `use of moved value: 'var_t.0'` (and the borrow-lifetime sibling).  The format machinery extracts a `__ref_N: (i64, String) = var_t.0;` temporary that MOVES `var_t.0` (String isn't Copy), then a sibling read of `var_t.1` panics because the move already invalidated the parent tuple in tracking.  Surfaced 2026-05-11 by plan-14 phase 02's `e3_d1_text_inside` cell.  Workaround: hoist each leaf into a flat-typed local first (`a = t.0.1; b = t.1.1; print("{a}|{b}\n")`).  Likely fix: format-string lowering should emit `&var_t.0.1` / `var_t.0.1.clone()` for nested-tuple text reads instead of moving the whole inner tuple.  Pinned by `tests/tuple_matrix.rs::e3_d1_text_inside` (currently failing under `--ignored` run; the test is the regression guard once fixed). | Medium (open) | Hoist each leaf into a flat-typed local first; format the locals. |
| 248 | Element-of-element assignment on nested tuples (`t.0.1 = 99`, `t.1.0 = 77` where `t: ((integer, integer), (integer, integer))`) rejected by both backends with `Not implemented operation = for type integer` at the parser level.  The parser's assignment LHS handling supports single-level `t.0 = x` (T1.x) but doesn't recurse into nested-tuple chains — `t.0.1 = …` falls through to a default arm that emits the integer-= operator on a non-lvalue.  Surfaced 2026-05-11 by plan-14 phase 02's `e3_d1_elem_elem_assign` cell.  Workaround: rebuild the inner tuple — `t.0 = (t.0.0, 99); t.1 = (77, t.1.1);` — verbose but correct.  Likely fix: extend the LHS-walker in `src/parser/operators.rs::assign_op` (or wherever `parse_assign` recognises `Value::TupleGet` LHS) to recurse one more level for `Value::TupleGet(Value::TupleGet(_, _), _)`.  Pinned by `tests/tuple_matrix.rs::e3_d1_elem_elem_assign` (currently failing under `--ignored`). | Medium (open) | Rebuild the inner tuple via single-level assignment: `t.0 = (t.0.0, 99); t.1 = (77, t.1.1);`. |
