...
| 223 | `s = "literal" + s` (and any prepend that puts the destination on the RHS) is broken on both backends.  Interp produces self-aliased garbage (`s = "world"; s = "hello " + s` gives `"worldhello worldhello "` — the work text and `s` end up sharing storage so subsequent appends see the partially-built result).  Native rejects with E0502.  P217's fix doesn't reach this case because the lowering goes through the `orig_var == u16::MAX` branch (allocates a fresh work text), not the self-append branch — but the assignment back to `s` (via `assign_text`'s self-reference path with `code_references_var`) still aliases the work to `s`'s storage.  Surfaced 2026-05-05 by P217 fix-variant probing.  Reproducer in /tmp/p217_prepend.loft.  Distinct from P217 (which was the self-append-on-LHS-piece shape, now closed). | High (S1, open) | Hoist: `t = "hello " + s; s = t;`.  Or use format-string interpolation: `s = "hello {s}";`. |
| 218 | Format string interpolating a parameter inside a coroutine generator body declares the `__work` text-format buffer in the wrong scope.  `fn greet(name: text) -> iterator<text> { yield "hello, {name}"; yield "bye, {name}"; }` rejects under native with E0425 "cannot find value `var___work_2` in this scope" — the buffer is declared inside one match arm of the state machine but referenced from another (or at the top of the impl body).  Interp works correctly.  Surfaced 2026-05-05 while probing P211 fix variants.  Reproducer in /tmp/p_followups/p218_coro_format_param.loft.  Likely shared root cause with the `__work` buffer placement logic in `src/parser/expressions.rs` or wherever format-buffer locals are emitted into the IR. | High (S1, open) | Pre-format the string outside the yield: `m = "hello, {name}"; yield m;` — the format buffer then lives in the factory function's parameter materialisation, not the per-state body. |
| 219 | Vector-element ForLoopBody in a generator body emits invalid Rust for the factory function.  `fn nums() -> iterator<integer> { for n in [10, 20, 30] { yield n; } }` rejects under native with E0308 "expected `Box<dyn loft::codegen_runtime::LoftCoroutine>`, found unit type `()`" — the factory's body emits `return 'l4: loop {…}` for the for-block, which is unit-typed, while the function signature requires `Box<dyn LoftCoroutine>`.  Range-based for loops (`for i in 0..n { yield … }`) work because they use a different IR shape that doesn't trigger the bogus `return`.  Affects integer yields too — not specific to text.  Interp works correctly.  Surfaced 2026-05-05 while probing P211 fix variants.  Reproducer in /tmp/p_followups/p219_vector_for_yield.loft.  Likely lives in `src/generation/coroutine.rs::emit_for_body_factory` interaction with the for-loop IR walker — the body's outer Block tail position is being treated as a return value when it should be a statement followed by `Box::new(struct_name { ... })`. | Medium (S2, open) | Use a range-for instead: `for n in 0..3 { yield [10, 20, 30][n]; }` (or pre-compute the values outside the for-loop and yield them sequentially).  Native works for range-driven generators today. |
