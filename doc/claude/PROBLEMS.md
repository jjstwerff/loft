...
| 150 | 1-store leak: orphaned `__ref_*` placeholder when callee returns a fresh store | Low | **Open 2026-04-16.**  When a Reference variable is assigned from a user-fn call with no visible Reference params (`m = make_or_default(t)`), the codegen pre-allocates a placeholder store via `OpConvRefFromNull` and marks `__ref_*` as `skip_free` to avoid double-free in the typical adoption case.  When the callee instead returns a fresh store from a different path (e.g. early-return through a constructor call AND a fall-through `T.parse(text)`), the placeholder is orphaned.  Discovered while running `lib/moros_map/tests/serial.loft` (1 leak warning).  Reproducer: `tests/leak.rs::p150_*` (both `#[ignore]`'d).  Fix attempted 2026-04-16 by removing `set_skip_free` in `src/scopes.rs::scan_set` + `src/state/codegen.rs:1062`; closes the leak in the minimal case but causes `serial.loft`'s `map_to_json` to HANG (infinite loop) — the skip_free is load-bearing for the typical adoption case.  Reverted; needs a narrower callee-shape-aware fix that only drops `skip_free` when the callee can return a fresh store via a non-tail constructor call.  Workaround: leak is 1 store per affected call (not per-iteration), no accumulation; `--native` is silent because Rust's no-op-on-already-free hides it. |
