...
| 146 | Wrap-suite store leak on user-fn alias return + deep-copy | Medium | **Investigating 2026-04-16.**  `var = user_fn(arg)` where `user_fn` returns its parameter (`fn ac_identity(p: AcPoint) -> AcPoint { p }`) leaks the result variable's store at function exit.  Runtime takes the P143 lock-args + OpCopyRecord deep-copy path so the result is an independent store, but scope analysis treats the call as "returns alias" and skips the OpFreeRef.  Affects 5 wrap-suite scripts (45, 62, 76, 81, 95).  Reproducer: `tests/leak.rs::p146_script_95_alias_copy_leak` (un-ignore for sharp panic with alloc op + pc).  Fix lives in `src/scopes.rs` — same family as P143 / dep-inference. |
