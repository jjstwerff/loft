...
| 148 | Wrap-suite leak on script 45 (A10 field iteration) | Medium | **Open 2026-04-16.**  `for f in s#fields { ... }` leaks 8 stores per main run.  Distinct shape: codegen for the A10 field-iteration feature emits `OpDatabase(__ref_N)` directly (not via Set/Call dispatch) inside the loop body for each match arm's `__ref_*` work-ref.  My P146/P147 fixes target Set/Call paths and don't apply.  Reproducer: `tests/leak.rs::p148_script_45_field_iteration_leak`.  Needs targeted investigation of `__ref_*` work-ref lifetime in field-iteration codegen. |
| 149 | SIGSEGV in `OpCopyRecord` for nested struct construction (script 76) | High | **Open 2026-04-16.**  `tests/scripts/76-struct-vector-return.loft::test_p105_nested_struct_in_vector` SIGSEGVs under `execute_log` at `OpCopyRecord(data=ref(0,0,0x40080000), to=ref(2,5,24), tp=68)`.  Source DbRef has store_nr=0 with pos=0x40080000 (the bit pattern of float 2.125), suggesting a stack slot was reused for a float and then read back as a DbRef.  Pre-existing memory issue (observed before the P146 fix); the warning-only `state.execute` path doesn't catch it, so wrap-suite reports the script as clean.  Reproducer: `tests/leak.rs::p149_script_76_nested_struct_segv`. |
