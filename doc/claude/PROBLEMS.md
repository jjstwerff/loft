...
| 158 | Trailing comma after last field in struct-enum variant triggers parse error | Low | Remove the trailing comma (e.g. `Alpha { x: integer }` not `Alpha { x: integer, }`) — regular structs and constructor calls accept trailing commas; only the enum-variant parser rejects them |
