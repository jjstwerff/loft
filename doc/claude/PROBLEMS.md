...
| 160 | Vector element `v[i]` cannot be passed as `&` parameter — "assign to a variable first" | Low | Assign to a local: `tmp = v[i]; modify(tmp);` — but mutations don't propagate back; use a for-loop to find + mutate in-place instead |
