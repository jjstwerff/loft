...
| 135 | Inline struct argument to function call leaks store | Medium | **Partially fixed** — `scan_args` in `scopes.rs` now lifts inline struct-returning call arguments to temporary variables when the outer call is an operator (returns scalar). Reduced GL renderer leak from ~16k to ~7k stores in 5s. Remaining leaks: user-function-to-user-function patterns (`normalize3(vec3(...))`) where lifting is unsafe due to potential store borrowing. |
