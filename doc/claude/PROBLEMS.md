...
| 159 | `Type.parse(json)` fails on struct-enums with "Unknown field Type.parse"; only plain structs supported | Low | Serialize/deserialize each variant as its own struct, or use `json_parse()` + manual dispatch |
