...
| 207 | Native codegen E0308 when comparing a `character` element read from a `Type::Tuple` against a `character` literal.  Reproducer: `t = ('a', 42); if t.0 == 'a' { ... }` — under `--native` rustc rejects the generated `let _v_v1 = (var_t.0); if _v_v1 == char::from(0) { ... } else { i64::from(_v_v1 as u32) }` because `var_t.0` is typed as `i32` (per the tuple element layout) but the null-sentinel comparison uses `char::from(0)` (a `char`).  The non-tuple form (`c = 'a'; if c == 'a'`) compiles fine — the divergence is specific to tuple-element char reads.  **Open** — root cause is in the OpEqInt / character-null-sentinel template at the `Type::Tuple` element-access path; suspect `src/generation/ops/int_compare.rs` (the P200 fix site) doesn't normalise char-element width when the source is a tuple `OpVarChar` read.  Surfaced by plan-14 phase 01 cell `e1_d1_char_int_local`. | Medium | Compare via integer cast: `t.0 as integer == 97`.  Plan-14 cell `e1_d1_char_int_local` is `#[ignore = "P207"]` until fix lands. |
| 208 | Native codegen E0282 "type annotations needed" when a bounded-generic method call returns text and the result is used in `+ "!"` text concatenation.  Reproducer: `fn label<T: Printable>(x: T) -> text { x.to_text() + "!" }`.  Surfaced by plan-17 phase 01 (B) follow-up — was masked by the earlier type-inference failure that rejected the snippet at parse time.  After the parser fix landed, the interpreter case works but `--native` rustc rejects: the generated code has `stores.scratch.push((<expr containing inner return>).to_string())` — the inner `return { stores.scratch.push(...); Str::new(...) }` from `Value::Return` text-wrapping makes the surrounding `to_string()` unreachable, so rustc can't infer the type.  **Open** — likely fix is in `src/generation/emit.rs` Value::Return text-wrapping: suppress the outer scratch-wrap when the inner expression is itself a returning text-wrap, OR suppress the inner return-wrap when the outer is a text-wrap that consumes it.  Workaround: keep the call interpreter-only.  Tracked in plan-17 phase 01 follow-up + USER_FACING.md. | Medium | Use `--interpret` for now; native form blocked.  Restructuring the body (e.g. `s = x.to_text(); s + "!"`) compiles fine under interpreter but native still fails the same way. |
