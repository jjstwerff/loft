...
| 249 | Closure-typed tuple elements have a wrong-width layout AND no call-through-tuple path — `t = (lambda, 99); t.0(10)` crashes the interpreter with `fn_call_ref: d_nr=<garbage> out of range` at `src/state/mod.rs:319`, and the workaround `f = t.0; f(10)` ALSO crashes (same call site).  Two layered root causes: (1) **TupleGet layout** — `src/state/codegen.rs:398` and friends bucket `Type::Function` into the `Type::Integer(_)` arm and emit `OpVarInt` (reads 4 bytes), but P213 v4's closure-in-struct-field fix made fn-ref slots 16 bytes (4 B fn-id + 12 B closure DbRef).  Reading only 4 bytes leaves the upper 12 uninitialised; storing through `f = t.0` copies just 4 bytes too.  (2) **Call shape** — even with the right bytes, `t.0(args)` doesn't go through `parser/control.rs::parse_call` (which addresses bare-name `name(args)` Function vars only); the CallRef IR's first arg is a `u16` var-nr, so there's no shape today for "call the function pointer in this expression's slot."  Surfaced 2026-05-11 by plan-14 phase 03's e4 cells.  4 of 5 fail with this combined root cause: `e4_d1_closure_call`, `e4_d1_closure_swap`, `e4_d1_capture_survives`, `e4_d2_closure_arg`.  e4_d1_closure_local (store-only — never reads or calls the closure-typed element) passes.  Likely fix path: (a) extend `Type::Function` in TupleGet/TuplePut/`emit_tuple_var_push_recursive` codegen to use the 16-byte fn-ref layout (read both halves) — mirrors P213 v4's struct-field treatment; (b) detect `<TupleGet of Function>(args)` in `operators.rs`'s postfix-call path; allocate a Function-typed temp var with the correct 16-byte layout, emit `Block[Set(temp, TupleGet(...)), CallRef(temp, args)]`.  Pinned by `tests/tuple_matrix.rs::e4_d1_closure_call` + 3 siblings (currently failing under `--ignored`).  Out-of-scope today: T1.8a-style closure-tuple return convention. | Medium (open) | No clean workaround — even hoisting `f = t.0` crashes because the layout is wrong.  Use a struct field instead of a tuple element when storing a closure (struct fields use the P213 v4 layout). |
