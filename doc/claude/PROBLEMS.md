...
| 229 | `tests/multiplayer_v2.rs::v2_two_clients_with_spectator_routing` flakes on macOS CI: the test spawns clients A and B with a 50ms stagger and asserts that at least one observes the other's `Spectator*` frames.  On macOS CI the scheduler runs both clients fast enough that they each complete their 3 X moves in <0.4s with no observable overlap, so neither sees spectator frames.  Bumping the stagger to 200ms makes A finish before B connects (failure flips direction).  Linux/Windows pass; only macOS hits this.  Marked `#[ignore]` with this P-id to keep CI green pending a real fix.  Fix shape: either add real `sleep()` to the loft client between moves (loft has no sleep API today — needs stdlib addition), OR add a server-side rendezvous handshake ("hold A's X moves until B connects").  Surfaced 2026-05-05 during PR #209 CI debugging. | Medium (S2, open) | Run with `--ignored` locally to validate; or use `--test-threads=1` (already serialised via nextest test-group) and accept occasional macOS flakes. |
