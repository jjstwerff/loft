...
| **`--native`** linking a native library (C-ABI dylib link, @PLN26 ph.4) | 🟡 **Verify-before-flip** — Linux/macOS link the package's cdylib `.so` by C-ABI (sealing its Rust crate graph, killing the StableCrateId class).  Windows links the DLL through its import library `<stem>.dll.lib` via `-l dylib=<stem>`, has **no RPATH** (the DLL is staged beside the binary, `native_utils::stage_native_dlls`).  Built behind an env gate — `native_cabi_enabled()` returns `cfg!(not(windows))` by default; `LOFT_NATIVE_CABI=1` forces it on.  **Verified in focused CI, not a VM**: `.github/workflows/win-cdylib.yml` job `win-cdylib-cabi` (`LOFT_NATIVE_CABI=1`, native subset, `windows-latest`).  Green → flip the default on for Windows.  Until then Windows uses the proven rlib-link row above. |
