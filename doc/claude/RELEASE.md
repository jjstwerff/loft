...
they cannot go stale; the PDF is a committed file that only `make pdf` updates
by hand, which means a release can ship four bundles carrying a reference that
does not describe it, in silence.  `A-pdf` compares it against
`doc/loft-reference.typ` — and found it 5½ hours stale the first time it ran.
