...
| `has_ref_params` at the call site | adopt-vs-copy re-derived; vector returns alias (the `a = getv(x)` case of [probe 06](plans/85-store-lifetime-retirement/probes/06-field-read-adopt-vs-copy.loft)) | caller reads the return dep: empty ⇒ adopt, `{Attr}` ⇒ copy. **Cluster-A.4 partial (commit `20610eaf`):** the *borrowed-view* read (the `is_borrowed_view` 0x8000 source-free decision) is now ONE shared `Definition::returns_borrowed_view()` — both backends read it, the H4 divergence is closed. The 11 `has_ref_params` adopt-vs-copy sites still re-derive a coarser proxy; their clean collapse needs the return-path funnel (row 104) first. |
