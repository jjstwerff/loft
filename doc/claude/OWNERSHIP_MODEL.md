...
| `has_ref_params` at the call site | adopt-vs-copy re-derived; vector returns alias (the `a = getv(x)` case of [probe 06](plans/85-store-lifetime-retirement/probes/06-field-read-adopt-vs-copy.loft)) | caller reads the return dep: empty ⇒ adopt, `{Attr}` ⇒ copy |
| field read binds without an owner | `a = x.v` / `a = getv(x)` ALIAS the field's store ([probe 06](plans/85-store-lifetime-retirement/probes/06-field-read-adopt-vs-copy.loft), both backends) while plain `a = x` already copies — silent value-semantics violation (single-owner, principle 1) | a heap FIELD read yields a `{Attr(base)}` borrow that copies on bind/escape — row-102's adopt-vs-copy fact extended to field reads, not only param returns (the `return <param>` row-101 fix does not cover a returned field) |
