...
| **N1** — Direct-emit local collections in native codegen | § Design: N1 | **O4** | Native | Open — design ready.  Cooperates with `lib_plans/future/03-lazy-stdlib/`.  (`plans/finished/21-retire-scratch/` shipped, so the scratch consumer set is now zero — N1 is independent.) |
