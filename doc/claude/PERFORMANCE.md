...
| **P5** — `vector +=` capacity reservation (amortised growth) | — | — | Interpreter + Native | Open — discovered 2026-05-21 via the new `store_memory()` builtin while profiling the @PLAN36 crystal mesh.  Building a large vector by repeated single-element `+= [x]` reallocates the backing record on (nearly) every append, fragmenting the store into O(N) freed records.  A 12 738-segment mesh built this way produced **101 815 uncoalesced free blocks / ~250 MB** per build (vs ~0.8 MB of data).  Fix: amortised (e.g. ×2) capacity growth on `+=`, or a `reserve(v, n)` builtin callers can pre-size with.  Helps every vector-building consumer. |
| **P6** — Free-block coalescing in `Store` (merge mergeable neighbours) | — | — | Interpreter + Native | Open — partner of P5 (same `store_memory()` finding).  `Store::delete` is documented to merge adjacent free blocks, but the crystal-mesh access pattern leaves ~100 k *mergeable* adjacent free-block pairs uncoalesced, so a store can't reuse freed space and grows unboundedly under repeated deep-copy reassignment.  `store_memory()` reports the `mergeable-pairs` count (currently a first-cut linear-adjacency detector; finer coalescing analysis is the work).  Fixing this reclaims the fragmentation P5 avoids. |
