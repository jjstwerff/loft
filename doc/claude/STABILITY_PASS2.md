...
| `src/parser/mod.rs` | `base_var_of`, `find_capturing_fn_ref` (`&Value`); `type_carries_closure` (pub), `type_contains_tv`, `is_*_vector_element_target` ×2, `type_element_size` (`&Type`); `is_addressable` | `base_var_of` → `Value::base_var`; `find_capturing_fn_ref` → `any_node` closure (needs `&Data` — stays parser-side as closure); Type classifiers → `Type` impl in data.rs (carries `&Data`/`&Stores` params where needed) | ▶ wave 1: base_var moved |
...
| `src/parser/collections.rs` | `is_capturing_fnref`, `worker_returns_capturing_closure` (`&Value`); `narrow_route_for` (`&Type`) | → `any_node` closures / Type impl | ☐ todo |
| `src/parser/expressions.rs` | `leaf_tuple_lhs`, `inline_ref_set_in` (`&Value`) | → `any_node` closures | ☐ todo |
...
| `src/generation/emit.rs` | nested local `walk`, `tail_is_return` (`&Value`) | → `any_node` closures | ☐ todo |
...
| `src/state/codegen.rs` | `emit_typed_null`, `known_type`, `add_const` (`&Type`); `ir_contains_var` (debug assert walker); Definition `.code/.variables` writes (it IS the bytecode owner of those fields) | walker → `any_node`; Type helpers → Type impl | ☐ todo |
