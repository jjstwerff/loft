...
| [operational.md](operational.md) + family | 2 + 2 | **D-op-1/2** the differential oracle (@PLN89, the META deviation — differential-not-definitional conformance; D-op-4 the spreadsheet runtime is CLOSED). The operational rules are now written across sibling files (2026-07-04): heap / iteration / coroutines / concurrency / calls / matching / tuples (0 own each) + **closures.md (2 OPEN — D-clo-1: `\|…\|` can't capture, unhelpful error; D-clo-2: a stored short lambda passed to `map` PANICS at data.rs:4569, a real crash to fix + file)** |
