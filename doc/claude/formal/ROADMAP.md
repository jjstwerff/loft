...
- **1 alone** — `D-own-8` (a Join's ownership fact true on one path only — its Face A SYMPTOM
  closed 2026-09-03 by giving each branch arm its own binding, loft#1320; the literal-mint arm
  and two declined shapes keep it open).  `D-bind-11` CLOSED 2026-09-03: a `&(τ,…)` with a heap
