...
| [capabilities.md](capabilities.md) | 3 | sandbox admission — call gate enforced; field rights + the parameter lock (D-cap-1, @PLN86 F4–F7), the capturing-closure residual (D-cap-2), and the owned-vs-host dependency on ownership D-own-2 (D-cap-3) |
...
| C4 | **D-cap-1, D-cap-2** | capability admission: parse field/parameter links into a `member_access` carrier and gate each field read/update/append + non-default argument (D-cap-1); carry a capturing closure's host references into the reachable-set (D-cap-2). **D-cap-3 folds into C3** — `Cap-Own` reads ownership's owned-vs-host fact. | **[@PLN86](https://github.com/loft-lang/plans/issues/86)** (§7 F4–F7) | M |
