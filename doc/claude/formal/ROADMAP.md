...
|---|---|---|
| [types.md](types.md) | 1 | **@PLN25 value/null model landed (2026-07-02): DN1–DN6 CLOSED**, D2 closed (C83); DN3 covers integer `/`/`%`, indexing, text-parse; overflow-arith a decided edge (C85). **OPEN — @PLN102 null-flow generalisation (spec-first 2026-07-11):** the general § Null-flow laws across EVERY type — (N-Domain) fit-failing→`τ?`, (N-Prop) null propagates through arithmetic, (N-Cast) a cast/parse asserts (parse folds in, no auto-`τ?`), (N-Store) warns except narrow widths (which error) — plus the float instance DN3-Float. Rules written ahead of the code; implementation pending ([design](../plans/102-stability-contract/float-null-domain-typing.md)) |
| [binding.md](binding.md) | 1 | @PLN40 two-level `const` model shipped (Const-Bind/Value/ScalarCollapse/Compose); **D-const-1 open** — enum-variant `const` not enforced (Phase 3). `&`-ladder: D-bind-7 (bare `&a;`) landed, 0 own |
