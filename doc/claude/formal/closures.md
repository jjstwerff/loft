...
- **D-clo-14** — CLOSED for every spelling this doc can own (loft#1257): inline, bound, as an argument, at every collection kind, by store IDENTITY against the `Join` base. ⚠ *"bound"* was measured on the BORROW arm only when this was first written; the bound spelling's MINT arm (`t = g(none)` in a loop, 470 stores at N=500) was found and closed the next morning by the same rule applied at the bind (loft#1320). ⚠ The residual it was recorded with on 2026-09-03 — a call in an `if`/`match` ARM whose value escapes the block — was measured the next hour and is **not a closure or a collection defect at all**: the NAMED twin leaks in the same position, a struct leaks as readily as a `vector`, and the axis is that the joined binding carries TWO arms' deps. It belongs to [ownership.md](ownership.md)'s **D-own-8**, whose Face A it is the first measured symptom of, and it is tracked there
