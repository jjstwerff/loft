...
| [ownership.md](ownership.md) | the `deps` / borrow **checker** (lifetimes) — distinct from binding.md's surface | **1 open** — RE-OPENED after the 2026-07-04 zero, now down to D-own-8 (a Join's ownership fact is true on one path only). D-own-16 and D-own-26 both closed 2026-09-03; D-own-26's gate had been reporting `0 violations` over its own violations for a week, because it searched for a free in a region the free cannot occur in. The ORIGINAL five stay resolved: D-own-1/2/3/4/5 ALL CLOSED; every store-lifetime decision reads the one total `deps` fact. The soundness proof heap.md's free rules rest on |
