...
| [ownership.md](ownership.md) | the `deps` / borrow **checker** (lifetimes) — distinct from binding.md's surface | **1 deviation open** (2026-07-04) — the 5 sound+complete invariants we steer toward (the rules); D-own-2/3/4/5 CLOSED, only D-own-1 (O-Deps single-fact unification) remains. Written deliberately ahead of the implementation: the target steered the work |
