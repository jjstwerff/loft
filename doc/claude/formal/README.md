...
| [tuples.md](tuples.md) | tuples — construct / project / destructure | **1 open** (D-tup-9, collection half: a type-variable member of a tuple literal is copied for a record or scalar `T` since loft#1365, and still aliases for a vector or keyed `T`) — positional products (n≥2), `.i` a compile-time index, destructuring, tuple returns, the reference tuple (`T-Ref-Rep`); ⚠ its differential oracle is all-`(integer, integer)`; register in [tuples-history.md](tuples-history.md) |
