...
| [closures.md](closures.md) | lambdas / closures / fn-refs — capture + apply | **rules written (2026-07-04), 2 OPEN** — `fn(){}` captures (first-class: store/pass/return/escape), `\|…\|` is non-capturing; scalar-by-value / heap-shared capture. D-clo-1 (short form can't capture, unhelpful error) + D-clo-2 (stored short lambda → `map` PANICS) are real open deviations |
