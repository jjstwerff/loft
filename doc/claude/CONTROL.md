...
| **N6** | **loft#1308 / loft#1309 are the only issues with no fix anywhere**, and both were found by review guards rather than by a consumer: a returned closure reads a captured LOCAL collection after its store dies (a `(L-Escape)` deviation), and an inline `!call()` in argument position pushes 24 B into an 8-byte boolean slot. | Both fail only under debug assertions, which an ordinary build strips — so both were invisible to every gate that runs by default. |
