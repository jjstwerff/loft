...
Real apps built with loft — reference examples of HOW to build such things, not installable components. Sourced from `showcase` GitHub issues (internal apps + accepted community submissions), not a hand-kept list; propose one via the `application_showcase` issue (see @PLN112).

- **Brick Buster** — A Breakout-style brick-breaker — a self-contained, double-click-to-play browser game demo. · demonstrates a complete browser game in loft — loft→WASM, a canvas game loop, input, and power-ups · [repo](https://github.com/loft-lang/loft/tree/main/tools/brick-buster) · [demo](https://loft-lang.org/loft/brick-buster.html)
- **crawler** — A clean-room hex roguelike written in the loft language — 2D now, with a renderer-agnostic kernel for a 3D browser version later. · demonstrates a hex roguelike with a renderer-agnostic kernel — the first cross-game consumer of the hex_* world libraries · [repo](https://github.com/jjstwerff/crawler)
- **Crystal Editor** — The audience 'crystal' generative-art editor — a self-contained browser demo (built on lib/audience_crystal). · demonstrates generative art in the browser — loft→WASM, an interactive editor, real-time GL rendering · [repo](https://github.com/loft-lang/loft/tree/main/tools/audience-demo) · [demo](https://loft-lang.org/loft/crystal-editor.html)
- **dryopea** — Sci-fi free-build / tower-defence with scramble-and-salvage runs, built on loft. · demonstrates a 3D free-build / tower-defence game on the lavition engine · [repo](https://github.com/jjstwerff/dryopea)
- **GL demo gallery** — The gallery — one page collecting the loft GL/graphics demos, each a short program that runs live in the browser. A single entry point to the whole demo collection. · demonstrates the graphics / GL API by example — a browser gallery of short loft programs (2D canvas, 3D, shaders) that run live · [repo](https://github.com/loft-lang/loft/blob/main/doc/gallery-examples.js) · [demo](https://loft-lang.org/loft/gallery.html)
- **moros** — Moros role-playing game. · demonstrates a full RPG — game systems, world state, rendering · [repo](https://github.com/jjstwerff/moros)
- **routing** — Phone-first route planner — sketch points, instant length, match to real paths (loft→WASM), export GPX. · demonstrates a phone-first WASM app — loft→WASM, interactive canvas, GPX export · [repo](https://github.com/jjstwerff/routing)
- **ssh_home** — A pure-loft SSH terminal for your phone — password-auth login, tmux in a phone-width view, native. · demonstrates a native terminal app — SSH + PTY, phone-width tmux, no browser · [repo](https://github.com/jjstwerff/ssh_home)
