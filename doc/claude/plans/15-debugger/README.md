...
end-to-end through the binary. **Open G1 gap (filed):** breakpoints attach to
source-mapped ops, emitted only at fault-prone arithmetic, so a pure-`if`/constant
body may not pause (**G1-span** row). **Next**: **G1-span** (statement-level source
spans — makes every line breakable) · **B** (interpret-scope) · the **G2**
browser/DAP surface · text/heap live edits + the file-run (`prog.loft:42`) entry
point.
