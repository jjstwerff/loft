...
edits a live integer local that the resumed call picks up, **and evaluates any
expression against the paused frame** (`n * 3`, `pt.x * pt.y` — every type) — all
proven end-to-end through the binary. **Next**: **B** (interpret-scope) · the **G2**
browser/DAP surface · non-integer live edits + the file-run (`prog.loft:42`) entry
point · exact value-for-value seeding (the remaining @PLN14 tie-in).
