...
| **D** — `.loftc` bytecode cache: file format caches bytecode + stores + const_refs + function positions; SHA-256 cache key from source content + version; `byte_code_with_cache()` skips the `def_code()` loop on cache hit; `src/cache.rs` module | Skip stdlib re-parse | **SHIPPED then RETIRED.**  Removed in plan-01 (integer-i64 migration) — its cache key missed stdlib edits and there were no external users yet.  Revisit the full-bytecode-cache design if/when Phase C demands it. |
