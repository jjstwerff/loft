...
Open — design settled, nothing built. loft encodes *"this operation could not produce a
...
|---|---|---|---|
| **A** — measure the interpreter cost: carry the flag on the five integer ops behind a build switch | this README | `make speed` delta vs baseline on the arithmetic corpus; a large dispatch-loop regression kills the clean design and routes to the static-approximation fallback | Open |
...
1. **The interpreter representation** — a parallel flag stack, a widened eval slot, or the
   flag computed only where an op *can* be invalid (a static property, recoverable but more
   work). Phase A's measurement decides; do not pre-commit.
