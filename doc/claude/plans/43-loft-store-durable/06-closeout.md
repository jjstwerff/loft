...
references to @PLN43's per-phase docs for design context.
...
- Remove @PLN43 from active section.
...
| `doc/claude/CHANGELOG_TECHNICAL.md` | ADD @PLN43 retrospective |
| `doc/claude/ROADMAP.md` | REMOVE @PLN43 from active; ADD to closed |
| `doc/claude/CAVEATS.md` | ADD per-OS caveats if needed |
| `doc/claude/plans/43-loft-store-durable/` | `git mv` to `plans/finished/38-loft-store-durable/` |
