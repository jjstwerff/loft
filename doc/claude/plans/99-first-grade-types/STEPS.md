...
- **Arc B Part 2 (thread `{x:spec}`) — DONE.** `objects.rs` branches on `tp`: a
  struct whose `t_<len><Type>_to_text` exists reads the spec as **raw** (v1: one
  identifier — `{m:dollars}` → `"dollars"`; arbitrary DSL like `%Y-%m-%d` is v2)
  instead of the width/radix grammar, stashing it in `OutputState.spec`.
  `try_bound_to_text_call` then branches on the `to_text` **arity**:
  `(self, spec, __work)` (3 params) gets the spec; `(self, __work)` (2) does not.
  Built-in `{n:05d}`/`{n:x}` grammar is untouched; **two-pass stable** (the
