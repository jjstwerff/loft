...
# Plan 56 — Sanitizer coverage expansion

## Status

**Status:** open — not started.  Opened 2026-05-31 as the successor to
[@PLAN53](../finished/53-sanitizer-ci-lever/README.md), which shipped
the sanitizer CI stack (Wave 1 + Wave 2 kickoff) and routed program-level
fuzzing to [@PLN53](../53-program-level-fuzzing/README.md).  This plan owns
the remaining Wave-2 sanitizer-coverage items that are not fuzzing.
...
|---|---|---|---|
| **S1** | **macOS-ARM nightly leg** — add a macOS-ARM runner to `miri.yml`'s toolchain-matrix job (and, when affordable, to the Miri/ASan jobs).  @P383 — the founding incident — surfaced exclusively on macOS-ARM; a ubuntu-only nightly would not have caught it. | macOS-ARM leg green on `main`; nightly badge reflects it. | Highest |
