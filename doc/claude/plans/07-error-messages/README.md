...
| 4e — Format-string suppression + compile-time warning at undefended sites | open | M | Two arms.  **4e.1 — Format-string suppression**: the parser already maintains an `in_format_expr` flag while parsing `{...}` interpolations (see `parser/objects.rs:773`).  Inside that scope every fault-prone op is auto-swapped to its Nullable peer (same swap 4d does for `??` / `if x != null`).  Format strings are the user's observability surface — they must NEVER halt, log, or warn.  Display "null<TypeName>" for null-record dereferences so debugging distinguishes "the value was null" from "I tried to read a field on a null record."  **4e.2 — Compile-time warning**: `Level::Warning` at every undefended, non-format-string fault site naming the three defense patterns (length check, `??`, `if x != null`).  Silenceable via `LOFT_NO_WARN_RUNTIME=1` env var or `--no-warn-runtime` CLI flag.  Defaults ON.  Defended sites + format-string sites stay quiet at compile time AND at runtime — all paths agree. | **Adds** — first user-facing nudge for the C66 defense contract; turns the runtime-log strategy into a self-improving system (each warning closure removes both compile-time noise AND production runtime-log noise).  **Pairs with** 4d — the warning fires exactly where defense dispatch did NOT swap to the Nullable peer.  **Codifies** the "format strings are observability, never raise" rule into a parser-driven mechanism.  **Closes** — debugging-via-print regressions where `println("{x}")` would otherwise halt or log on null fields. |
