...
| 4e.2 — Compile-time warning at undefended sites | open | S | `Level::Warning` at every undefended, non-format-string fault site naming the three defense patterns (length check, `??`, `if x != null`).  Silenceable via `LOFT_NO_WARN_RUNTIME=1` env var or `--no-warn-runtime` CLI flag.  Defaults ON.  Defended sites + format-string sites stay quiet at compile time AND at runtime — all paths agree.  Skip-list logic walks the IR after `rewrite_defended_fault_sites` (4d.2) and `rewrite_subtree_to_nullable` (4e.1) so the warning fires exactly where neither swap fired.  New `tests/parse_errors.rs` cells: `undefended_vec_index_warns`, `defended_vec_index_quiet`, `format_string_index_quiet`. | **Adds** — first user-facing nudge for the C66 defense contract; turns the runtime-log strategy into a self-improving system (each warning closure removes both compile-time noise AND production runtime-log noise).  **Pairs with** 4d — the warning fires exactly where defense dispatch did NOT swap to the Nullable peer. |
