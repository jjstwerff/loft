...
| 1 | [01-spans-on-ir.md](01-spans-on-ir.md) | in-progress (mostly done; 5 mechanical wraps remain — `Set` / `Iter` / `Return` / struct lit / narrow cast) | M | Attach a `Position` to every `Value` IR node that can fail at runtime via `Value::Span(Box<(Position, Value)>)` wrapper.  Wraps shipped for binary `+ - * / % << >>`, `[`, `.`, `Call` / `CallRef`, plus pc → Position table populated at codegen.  Walker discipline (every `match Value` adds a `Span(b)` recurse arm) maintained across `parser/expressions`, `parser/operators`, `parser/collections`, `scopes`, `variables/slots`, `state/codegen`, `formatter`, `state/debug`, `parser/mod::substitute_type_in_value`. |
...
| 4e.2 — Compile-time warning at undefended sites | open | M | `Level::Warning` at every undefended, non-format-string fault site naming the three defense patterns (length check, `??`, `if x != null`).  Silenceable via `LOFT_NO_WARN_RUNTIME=1` env var or `--no-warn-runtime` CLI flag.  Defaults ON.  Defended sites + format-string sites stay quiet at compile time AND at runtime — all paths agree.  **Easy-proof skip list — REQUIRED** (per 2026-05-11 workflow evaluation; warning is worse than nothing if it fires on code the compiler could trivially prove safe): bound loop variable (`for i in 0..len(v) { v[i] }`); explicit length check (`if i < len(v) { v[i] }`); constant-literal divisor (`x / 5`); constant-literal index against a known-length vector (`v[2]` where `v = [a, b, c]`).  Skip-list lives in `src/parser/operators.rs::warn_undefended_in_block` with per-pattern regression tests in `tests/parse_errors.rs::warning_skip_<pattern>`.  Skip-list grows as real-world false-positives surface; bar to add is low, bar to remove is high (silently demotes from no-warning to warning, breaks downstream goldens).  New `tests/parse_errors.rs` cells: `undefended_vec_index_warns`, `defended_vec_index_quiet`, `format_string_index_quiet`, plus four `warning_skip_<pattern>` cells. | **Adds** — first user-facing nudge for the C66 defense contract; turns the runtime-log strategy into a self-improving system (each warning closure removes both compile-time noise AND production runtime-log noise).  **Pairs with** 4d — the warning fires exactly where defense dispatch did NOT swap to the Nullable peer.  **Risk** (per 2026-05-11 evaluation): without the easy-proof skip list, devs disable the warning and lose its value forever.  Mitigation: skip list is a release blocker, not a follow-up. |
| 4e.3 — Distinct null tokens in format-string output | open | S | Format-string suppression (4e.1) makes every fault collapse to `null` (or empty for text/char), losing the *why*.  4e.3 carries the fault kind from the swapped Nullable peer through to the format renderer so each fault-produced sentinel renders with a `(reason)` suffix in format strings only.  Examples: `null(/0)` for divide-by-zero, `null(oob 999/3)` for vector OOB, `null(neg -5)` for negative index, `null(.field)` for chain-stops-at-null-record.  Genuine null values stay as bare `null`.  Text/char nulls render as `null` (never empty — empty looks like a missing variable).  **Out-of-scope contexts**: `{:j}` JSON format keeps `null` (must produce valid JSON); explicit `?? "fallback"` in a format string respects the fallback.  **Width handling**: when the format spec has a hard width (`{x:>5}`) the suffix truncates to `null(...)` so columns stay stable.  **Implementation**: side table keyed by IR-node identity carrying the `RuntimeErrorKind` from the swap point through to the renderer; no `Value` enum bloat.  `LOFT_FORMAT_BARE_NULL=1` env var suppresses the suffix for production deployments that surface format strings to end users.  New `tests/runtime_logging.rs` cells: `format_div_zero_shows_null_div`, `format_vec_oob_shows_null_oob`, `format_chain_null_shows_null_field`, `format_json_kind_keeps_bare_null`. | **Adds** — distinguishable fault tokens for debugging; closes the gap from 4e.1 where suppression killed the *why*.  **Pairs with** 4e.1 — same trigger (`in_format_expr`), additional metadata threaded through.  **Codifies** the "format strings are observability AND debugging" rule from chat 2026-05-11 (per the user follow-up that distinguishing fault-produced null from value-null is the cheapest debugging win available given that the print is already alive). |
| 4f — Remaining site conversions | open | M | Float / single `/` and `%` by zero (currently IEEE Inf / NaN), null-DbRef field / method access (currently SIGSEGV in some paths), narrow-cast overflow (currently wrap or sentinel), stack-overflow trap (currently `panic!`).  Each follows the established annotation pattern: `if cond { s.raise(KIND); sentinel } else { existing op }`.  Per-site conversion is mechanical once 4d (defense dispatch) covers the `??` opt-out for each new kind.  **Defense-coupling rule (per 2026-05-11 evaluation)**: each new fault kind ships its Nullable peer + adds itself to the 4d.1 / 4d.2 / 4e.1 swap tables + provides its 4e.2 warning message in the same commit.  A new fault site without these is a regression — `??` / `if x != null` / format-string suppression silently fail to defend it. | **Closes** — remaining 5 of phase-0's 13 silently-wrong cases (rubric in `0e-rubric.md`): `18_runtime_div_by_zero_float`, `23_runtime_null_deref`, `24_runtime_narrow_cast_overflow`, `26_runtime_recursion_overflow`, `27_runtime_stack_overflow`.  **Codifies** — phase 0's "phase-4 closes 8, phase-6 closes 5" estimate becomes "phase-4 closes 13" (the silent-wrong-answer story is finished; phase 6 then handles type-mismatch shape only). |
| 4g — Backtrace + state snapshot + soft-halt + bench + docs | open | L | Five arms.  **4g.1 backtrace** — capture `State::call_stack` into `RuntimeError.backtrace` at raise time; resolve each frame's source via phase-3's `source_loc_for`.  Render top-3 frames + "(use `LOFT_BT=full` for more)" in both dev-mode display and production log entry.  **4g.2 state snapshot at fault site** (per 2026-05-11 workflow evaluation — the highest-leverage workflow improvement after the dev-mode halt itself): capture the named-arg values at `raise` time into `RuntimeError.snapshot: Vec<(String, ValueRepr)>` using the same `Display` formatter the format-string renderer uses, so the snapshot prints exactly as `println("{x}")` would.  Bound to direct-arg values + indexed-collection `len`; do NOT walk arbitrary heap structures.  Renders inline as `= state: damage = [10, 20, 30] (len=3), idx = 5`.  Closes the "I have to add a print statement to find what `i` was" failure mode.  **4g.3 `--dev-soft-halt` flag** (per 2026-05-11 evaluation): demotes development-mode raises to log-and-continue so the developer sees every fault site in a single run instead of halting on the first.  No new machinery — toggles which branch of `State::raise` fires regardless of the logger's `production` flag.  Useful for porting / first-pass scripts where the full pattern of breakage matters more than fast loop-back on one site.  **4g.4 bench** — `make bench` re-run; outline fault-check branches with `#[cold]` if hot (≤3 % regression vs phase-3 baseline).  **4g.5 docs** — `doc/claude/LOGGER.md` § Runtime event logging + § Production setup.  Phase-4 status block close-out + plan-07 README phase-4 row → "shipped". | **Closes** — phase 4 acceptance ("`make bench` ≤ 3 % regression vs phase 3" gate from `04-runtime-error-kinds.md` § Acceptance).  **Codifies** — `STACKTRACE.md` shape extended to runtime-error backtraces (same `StackFrame` type).  **Pairs with** the next phase 4 closeout — moves phase 4 from "in-progress" to "shipped" in this README. |
| 4h — `not null` field reminder hint | open | S | Per the 2026-05-11 workflow evaluation: when a struct field is read 47× across the codebase and never compared to null, the developer probably meant to mark it `not null` at construction.  Marking it eliminates ~all field-deref fault sites for that field — strictly better than defending each read site individually.  4h emits a `Level::Hint` after parsing a struct definition for each non-`not-null` field whose all read sites are bare reads (no `??`, no `if x.field != null`).  The hint names the field, the read-count, and suggests `not null`.  Silenceable via `LOFT_NO_HINT_NOT_NULL=1` / `--no-hint-not-null`.  Defaults ON.  Implementation: lightweight per-field counter + flag in the parser's struct-definition table; second-pass walk reads it. | **Adds** — first proactive nudge to use the `not null` modifier instead of pervasive defensive reads.  **Pairs with** 4e.2 — 4e.2 says "defend this read"; 4h says "eliminate the need to defend by changing the constructor's contract."  **Closes** — the long-term "everyone writes `?? null` everywhere because the warning fired" failure mode the 2026-05-11 evaluation flagged. |
