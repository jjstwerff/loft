...
| 4f — Remaining site conversions | **shipped 2026-05-11 (slice 1: float div/mod)** | M | Float / single `/` and `%` by zero (currently IEEE Inf / NaN), null-DbRef field / method access (currently SIGSEGV in some paths), narrow-cast overflow (currently wrap or sentinel), stack-overflow trap (currently `panic!`).  Each follows the established annotation pattern: `if cond { s.raise(KIND); sentinel } else { existing op }`.  Per-site conversion is mechanical once 4d (defense dispatch) covers the `??` opt-out for each new kind.  **Defense-coupling rule (per 2026-05-11 evaluation)**: each new fault kind ships its Nullable peer + adds itself to the 4d.1 / 4d.2 / 4e.1 swap tables + provides its 4e.2 warning message in the same commit.  A new fault site without these is a regression — `??` / `if x != null` / format-string suppression silently fail to defend it.  **Slice 1 (shipped)**: float / single `/` and `%` by zero raise `RuntimeError::DivideByZero` like the integer peers; `OpDivFloatNullable` / `OpRemFloatNullable` / `OpDivSingleNullable` / `OpRemSingleNullable` peers added (silent IEEE).  4d.1 / 4d.2 / 4e.1 / 4e.2 / 4e.3 dispatch tables extended.  Tests scripts (`02-floats.loft`, `52-single.loft`) updated to use `(expr) ?? null` for intentional Inf/NaN producers.  Five new test cells in `tests/runtime_warnings.rs`.  **Slice 2 (deferred)**: 4.9 null-DbRef field/method access (`OpGetField` / `OpGetByte` etc. on `db.rec == 0`); 4.10 narrow-cast overflow (`as u8` / `as u16` with values out of range); 4.12 stack-overflow trap (recursion-depth check at fn entry). | **Closes (slice 1)** — phase-0 case `18_runtime_div_by_zero_float` (and the implicit `_single` mirror).  **Slice 2 closes** the remaining 4 of phase-0's 13 silently-wrong cases.  **Codifies** — phase 0's "phase-4 closes 8, phase-6 closes 5" estimate is on track to become "phase-4 closes 13" once slice 2 lands; the silent-wrong-answer story will be finished. |
