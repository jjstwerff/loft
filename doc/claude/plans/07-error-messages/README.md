...
| 4 | [04-runtime-error-kinds.md](04-runtime-error-kinds.md) | in-progress (reframed 2026-05-11 per [DESIGN_DECISIONS.md § C66](../../DESIGN_DECISIONS.md#c66--production-loft-programs-never-abort-on-user-attributable-edge-cases-development-may-halt)) | M | Typed `RuntimeError { kind, position, op_pc, backtrace }` for the site list (div / modulo by zero, vector / text OOB, null-DbRef deref, narrowing cast, `panic("…")`, stack overflow).  **Three-way defense contract** picks the opcode at compile time: `expr ?? fallback` → Nullable peer (no log, no halt); `if x != null { … }` after `x = expr` → Nullable peer via flow-analysis (no log, no halt); neither → raising peer.  At raising-peer runtime: **production** logs the typed event through `Stores::logger` and continues with the silent sentinel (null DbRef / `i64::MIN` / char 0) — production loft programs (games, servers, browser embeds) MUST NOT abort on user-attributable edge cases; production startup refuses to boot without a logger attached.  **Development** (default for CLI / tests / no logger) halts and renders via phase-2's `render_entry_pretty`.  Compile-time **warning** fires at every undefended fault site naming the three defense patterns; silenceable via `LOFT_NO_WARN_RUNTIME=1` / `--no-warn-runtime`.  `OpGetVectorNullable` / `OpVectorRefNullable` / `OpTextCharacterNullable` are the loop-iteration peers (no log / no halt — end-of-iteration is expected, not a fault).  Sub-arc: see `~/.claude/plans/async-toasting-nest.md` for the 10-commit landing sequence. |
