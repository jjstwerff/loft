...
| 4c — Native codegen template fixup | open | S | `s.raise(...)` and `s.vec_get_or_raise(...)` / `s.vec_ref_or_raise(...)` / `s.text_char_or_raise(...)` annotations in `default/01_code.loft` work in the interpreter (`fn(s: &mut State)`) but break `--native` compilation — the template substitutor in `src/generation/calls.rs:421-433` only handles `s.database.` → `stores.` rewrites, not the new `s.X` helpers.  Add per-call rewrites; native context exposes `cell: &UnsafeCell<Stores>` (with `let stores = unsafe { &mut *cell.get() };` already in scope).  Either add an analogous Stores-side helper (`stores.raise_at(file, line, kind)`) and route both interpreter + native to it, or add `src/generation/calls.rs` rewrites for each `s.X` template seen in `default/*.loft`. | **Closes** — restores P204 (`p204_tail_expression_return_passes_under_native`, originally closed by plan-11, re-broken by 4a's commit 129d6ee5).  **Closes** — `p200_binary_compiles_under_native`, `native_tuple_script`, `native_tuple_return_script`, `native_binary_script` (4 native suite regressions opened by 4a).  **Codifies** — every per-site annotation added by 4a / 4b / 4f works in BOTH interpreter and native after 4c lands; future fault sites won't need a per-PR native fix. |
| 4d — Defense dispatch (`??` extension + flow-analysis) | open | M | Extend `parser/operators.rs::rewrite_outer_arith_to_nullable` to also handle `OpGetVector` / `OpVectorRef` / `OpTextCharacter` when followed by `??` — `x = v[i] ?? null` then routes through the Nullable peer (no log / no halt), matching today's arithmetic-only behaviour.  Add flow-analysis in `parse_assign`: when assignment source is a fault-prone op AND the next sibling is `Value::If(Var(x) != null, …)` (or `!Var(x)`), swap source to its Nullable peer.  Single-block scope; cross-function defenses fall through to the raising peer. | **Closes** — defensive idiom log-noise reported in chat 2026-05-11 (`v[i] ?? null` should not log; `if x != null { … }` after `x = v[i]` should not log).  **Codifies** the three-way contract in C66's table; without 4d the contract's third row ("statically defended → no log") is not honored at compile time. |
| 4e — Compile-time warning at undefended sites | open | S | `Level::Warning` at every undefended fault site naming the three defense patterns (length check, `??`, `if x != null`).  Silenceable via `LOFT_NO_WARN_RUNTIME=1` env var or `--no-warn-runtime` CLI flag.  Defaults ON.  Defended sites stay quiet at compile time AND at runtime — both paths agree. | **Adds** — first user-facing nudge for the C66 defense contract; turns the runtime-log strategy into a self-improving system (each warning closure removes both compile-time noise AND production runtime-log noise).  **Pairs with** 4d — the warning fires exactly where defense dispatch did NOT swap to the Nullable peer. |
