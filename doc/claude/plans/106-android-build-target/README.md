...
- **B2 (the runtime entry)** now emits a real **`NativeActivity`**: `--native-android` wraps
  the emitted program (its `fn main` intact) in a small generated cargo crate whose crate
  root is the program plus a fixed `android_main` tail (via the `android-activity` crate,
  which exports `ANativeActivity_onCreate`) that runs `main()` and pipes the program's stdout
  into logcat. Cargo owns the `android-activity` dep tree and the lean `loft` cross-build
  (`--no-default-features`, `random`+`threading`; networking/TLS `ring` is a later phase per
  §5). The `.so` exports `android_main` + `ANativeActivity_onCreate` and drops straight into
  an APK's `lib/<abi>/` (packaging recipe in [`b2-spike/`](b2-spike/README.md)).

**Proven end to end:** `loft --native-android` output, packaged into an APK, launches on a
KVM emulator and logcat shows `loft --native-android: android_main reached` + the program's
own output (`sum of squares 0..8 = 140`). Tests: `tests/android_target.rs` (NDK-gated build
test asserting the AArch64 ELF exports the entry symbols + an always-on no-NDK error test).
**To run:** `ANDROID_NDK_HOME=<ndk> loft --native-android prog.loft` — build the emulator
twin with `LOFT_ANDROID_TARGET=x86_64-linux-android` (override the API with `LOFT_ANDROID_API`).
...
- **B2 — runtime entry (`android_main`). ✅ FEATURE LANDED (2026-07-14).** `--native-android`
  now emits a real `NativeActivity` (§1b): the hand-written wrapper the spike used is now
  generated by loft itself, replacing the B1 cdylib's stray `fn main`. Proven end to end —
  the APK built from `loft --native-android` output launches on a KVM emulator and logcat
  shows `loft --native-android: android_main reached` + the program's output
  (`sum of squares 0..8 = 140`). The [`b2-spike/`](b2-spike/README.md) `build_apk.sh` +
  `run_emulator_test.sh` remain the packaging/run recipe (loft emits the `.so`; wrapping it
  into a signed APK is still a manual step — a candidate follow-on: `--native-android` could
  drive `aapt2`/`apksigner` when the SDK is present). **Remaining:** APK packaging inside loft.
