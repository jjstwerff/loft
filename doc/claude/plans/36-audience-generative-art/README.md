...
| **@P318** | core DB (deep-copy of a `build_index`-populated hash) | **None at demo scale** — @P317 (both symptoms) is FIXED; the residual core bug (deep-copying a populated `build_index` hash field corrupts it, layout-dependent) is sidestepped everywhere the demo touches: `crystal_incr_new` builds its field in place, and the projector uses the full / incremental build, neither of which deep-copies a populated field. | Build hash fields in place; don't deep-copy a populated `build_index` hash.  Latent — won't bite the demo. |
...
- **C3-blocker P-issue family** (closed 2026-05-22, branch `audience2`) —
  ...
  populated `ChunkField`); the residual core deep-copy bug is filed as @P318
  (latent, sidestepped everywhere the demo touches).  Diagnosis tooling added
  the same session: `LOFT_LOG=zero_claim` /
