...
| **@P316** | native codegen (`vector<u8> ?? <int>`) | **Medium** — the native projector reads u8 cell fields (kind / colour); reading one with `?? <int-default>` mis-compiles (the `??` result stays `u8`, not widened to `i64`).  Bites the projector specifically (it runs native). | Avoid `vector<u8>[i] ?? <int>` reads — use `len()` / compare without `??`, or coalesce to a u8 literal.  `tests/scripts/133`'s `gsum` uses `cell_ix` / `single` precisely to dodge it. |
| **@P317** | core DB (deep-copy / keyed store) | **Low at demo scale** — C3 incremental works on the interpreter; the native incremental store-leak (symptom 2) is FIXED.  Residual symptom 1 (`crystal_incr_new(non-empty snap)` all-at-once non-determinism) only hits the all-at-once constructor, not the projector's full / incremental build. | Build incrementally from empty, or use the full build (`crystal_segments_aged`) — both correct on both backends. |
...
- **C3-blocker P-issue family** (closed 2026-05-22, branch `audience2`) —
  ...
  re-enabled.  The only open residual is @P317 symptom 1 (`crystal_incr_new`
  all-at-once non-determinism), which does NOT affect the projector's build
  path.  Diagnosis tooling added the same session: `LOFT_LOG=zero_claim` /
