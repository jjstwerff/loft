...
| 02-dispatch | The N9 per-function dispatch table, **interpreter target first** (= C71 minimal): swap one fn of the *running* demo to interpret on edit, over the shared store. This phase lights up @PLN16 **6b** (hot-swap `reload`) and **6c** (breakpoint-in-game → the IDE's variables panel against a live frame) | ☐ design written (2026-06-10) | [02-dispatch.md](02-dispatch.md) — invariant: every meaning call crosses ONE dispatch point whose target is per-fn data, target changes observable only as speed. New pieces = the inverse crossing (compiled→interp re-entry; the interpreter is always in the kernel process) + the table (name-keyed identity, frame-boundary swap atomicity). Probe-first: re-entry cost + inlining check gate slice 1 |
