...
| D | **slice 1 done** (`src/schema_sidecar.rs`): `LayoutIdentity`+`.dschema` round-trip, `classify`→`Handoff`, `is_actionable`; identity hash == golden pin (5 unit + 1 e2e). **Slice 2:** durable-store wiring + `CorruptReason::SchemaMismatch` via `on_corruption` |
