...
| **E** — The 1.0 line: what is frozen vs still moving, **and the pre-freeze AUDIT** | [RELEASE.md](../../RELEASE.md), [COMPATIBILITY.md § Before the flip](../../COMPATIBILITY.md), [INCONSISTENCIES.md](../../INCONSISTENCIES.md) | Open — **the critical path.** The `0→1` flip is a ONE-WAY DOOR gated on a thorough surface-by-surface audit (semantics · syntax · errors · stdlib · formats): fix every wart we'd regret while contract 0 still allows. A miss is permanent (live-with-it, engineer-around). Anchor on INCONSISTENCIES.md (High=silent-wrong=must-fix). This is the largest remaining 1.0 work item. **Dual-phase (owner): (1) close the open plans → settle the language [underway], then (2) a dedicated unhurried pass on the LIB side (stdlib + core libs — equally permanent).** Only then is the flip earned. **Phase-2 lib worklist PREPPED → [lib-audit.md](lib-audit.md)** (4-agent stdlib survey: the null-sentinel keystone + 9 must-fix High items + per-module tables; each item worked as a design decision — alternatives presented + conversion set enumerated). |
