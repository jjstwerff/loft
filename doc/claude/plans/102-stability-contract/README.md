...
| **B-registry** — the registry (`pr-validate`) validates the declared range on submission; and (never-break leg 2) **hosts every published version forever** — append-only, a version is never deleted, so an abandoned lib stays downloadable and a new program may still depend on it (see [COMPATIBILITY.md § Two populations](../../COMPATIBILITY.md)) | external — loft-lang/registry | Open |
