...
| **B-registry** — the registry (`pr-validate`) validates the declared range on submission AND is never-break **leg 2**: an **active compatibility gate**, not passive storage. Each new library version is checked against the last accepted one — **API verification** (public surface must be an additive superset) + **old-tests regression** (the prior version's tests must still pass). On a break, **preserve the old version first** (snapshot + keep hosted, append-only) then accept the new as a distinct opt-in version — so a pinned consumer always keeps a working copy. Holds lib authors to loft's own never-break discipline, automatically. See [COMPATIBILITY.md § Two populations](../../COMPATIBILITY.md). | external — loft-lang/registry | Open — the real infra cost |
...
| **C** — Recommended-idiom channel (**not** deprecation-toward-removal): "never break" + no usage telemetry makes the surface a one-way ratchet, so a steer can never reach a removal — it signposts a nicer idiom (warnings only, [Goal F](../../GOALS.md)) and warns the *author* of a contract-keyed semantic change (worked example = C86 / `hex_terrain`). The engineer-around is **folding**: keep the callable name, fold its implementation onto the new primitive (interface grows, duplicate code does not) — see [COMPATIBILITY.md § Folding](../../COMPATIBILITY.md). | Open — reframe captured; delivery (registry-submission vs dev `loft check`) + the "every steer ships with its fold" discipline need design |
