...
| **D** — Public bug-intake path: the fix-not-file discipline is internal and does not reach strangers | [ISSUE_TRACKING.md](../../ISSUE_TRACKING.md) · [public-bug-intake.md](public-bug-intake.md) | **▶ DESIGNED + MVP started** (2026-07-16). Like arc C, the public-facing FORM already exists (minimal-repro `bug_report.yml` + `config.yml` chooser + `CONTRIBUTING § Reporting a bug`); the gap is the INTERNAL side — the **triage bridge** (public report → fix-not-file), the **never-break promise** at the intake, and **repo-routing**. Design + the reconciliation-with-fix-not-file in [public-bug-intake.md](public-bug-intake.md). MVP step 2 (promise + routing in CONTRIBUTING) landed; steps 1 (triage bridge → ISSUE_TRACKING), 4 (ack discipline) remain. |
