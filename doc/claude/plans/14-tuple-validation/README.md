...
| [08 — P234 runtime: LOCAL tuple-with-lifetime-concern variables](08-p234-runtime-locals.md) | E2, E3, E4, E5 | D1 (local var) | Extends Phase 07's routing to LOCAL `r: (Point, integer) = …` and `r = (p, 5)` declarations + destructure temps.  Same `has_lifetime_concern` gate.  Retires the rest of T1.8a's tuple-text machinery in `src/generation/dispatch.rs::output_set` (~150 LoC).  Refactor for uniformity, not a bug fix — local tuple-with-lifetime works today via Rust ABI; this phase makes it match the function-return path.  Independently executable; can be prioritised, deferred, or skipped at will. |
