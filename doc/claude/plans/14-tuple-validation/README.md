...
| 09 — A7.1 par tuple wide-return runtime | E1 | par worker return | **Deferred 2026-05-08** after second-cut implementation hit P236 (heap-owned reference return from `if/else` body corrupts under `--native`).  Phase 07's `has_lifetime_concern` widen already unblocked 2 of 4 par_tuple_return canaries (`int_text`, `struct_text`).  The remaining 3 (`int_int`, `three_arity`, `nested`) need pure-value tuples wider than 8B routed through the same synthetic-struct path.  The size-based gate widen + recursive `rewrite_tail_tuple_to_synthetic_struct` for If/Block/Insert tails works correctly — all 4 canaries pass.  But the same widen regresses `tests/docs/28-tuples.loft::min_max(...) -> (integer, integer) { if ... { (a, b) } else { ... } }` because the rewrite triggers P236's silent native data-corruption pattern (struct return from if/else returns null).  P236 is pre-existing on `eebaaa4` and broader than tuples — affects ALL Reference / Vector / struct-enum returns from if/else bodies (not just tuples).  Fixing P236 first is the prerequisite for A7.1 closure.  See doc/claude/PROBLEMS.md P236 for the full diagnosis and fix-shape. |
