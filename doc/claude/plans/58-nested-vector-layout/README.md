...
| II | Single-NaN-sentinel read as wild rec-id | **SIGSEGV**, all 8 `single` contexts | both | no effect (orthogonal) | `cluster-II-single-sentinel.md` |
| III | Narrow-element nested read (`i32`, `boolean`) | **silent corruption** (wrong value / empty) | interp (native FAIL) | no effect (element not a vector) | (see RESULTS.md) |
