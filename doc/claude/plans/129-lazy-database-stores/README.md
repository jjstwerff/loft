...
| **B** — schema→query from `LayoutDesc`: equality from `hash`, ranges from `sorted`, composite+ordered from `index`; nothing enumerated ahead of time | **shipped** — equality on any keyed kind (a miss fetches), and a single-column range as ONE query (`store_lazy_range`). Open underneath it: the range wired to an IMPLICIT `xs[lo..hi]` slice, and a composite range (two numbers cannot say which value pins the leading column — `store_lazy_query` covers it meanwhile) |
...
| **B4** — collection-valued fields as owner-parameterised queries (`company.people`) | **the SHAPE works, with no new language surface** — `store_lazy_query(firm.people, "company_id = {firm.id}")` is the owner-parameterised query, per collection so two firms' fields hold their own rows, and a keyed lookup on the field faults like any other. What made it possible was resolving a struct-FIELD collection to its own type through the resolver the paged loader already had (#632). A DECLARED form (the field knowing its own FK, so no call is written) is still open |
