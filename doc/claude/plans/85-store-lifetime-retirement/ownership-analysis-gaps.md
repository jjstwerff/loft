...
**ATTEMPT (reverted) — the alias is NOT generated by `materialize_vector_arms_into`.** A
materialise rewrite added to that function's `Block` arm never fired for `deliver` (an `LOFT_DBG_MVA`
trace showed it is not called for `n_deliver`) and instead mis-fired on unrelated vector-match
functions (matrix CRASH). So the `_mv_items_1 = OpGetField(e,4)` alias is produced by a DIFFERENT
match-lowering path that still needs locating (grep the match-arm / enum-field-binding lowering that
reuses the retbuf var as the field binding). Once located, the materialise + dep-strip is the fix —
but it is genuinely deeper match-lowering surgery than the first-bind guards, not a quick collapse.
