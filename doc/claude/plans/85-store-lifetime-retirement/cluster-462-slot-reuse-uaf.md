...
| 8 | The **exact op that prematurely frees** the still-referenced store | ⏳ HYPOTHESIZED | the `LOFT_UAF` frame-var scan misses it (the stale DbRef lives on the operand stack / in a vector element, not a frame variable) — needs the tool extension below |
| 9 | The premature free originates in the **#457/#459 free-side-thicket deletion** (the adopt/free moved to return delivery; some path's store is no longer protected) | ⏳ HYPOTHESIZED | regression window points at #457/#459; not yet bisected to the op |
