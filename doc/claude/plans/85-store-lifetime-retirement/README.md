...
| `04-slot-init-405.loft` | heap local conditional × unused × nested loop (#405 shape) | II (#405) | 🔴 **STILL LIVE — interpret SIGSEGV; DISTINCT sub-mechanism.** `OpFreeRef(ki)` double-frees the NRVO return-buffer alias `__ref_1` via a stack-ref (#405/#306). This is the **caller-side** conditional-loop NRVO-buffer free — NOT the callee-side vector-`match`-return delivery that probe 05 exercises — so the probe-05 fix does not cover it. native completes. The next cluster-II sub-fix: scope analysis must not per-iteration-free a loop-local that aliases the loop-invariant NRVO buffer (`__ref_N`); the buffer is freed once at the caller's scope exit. |
