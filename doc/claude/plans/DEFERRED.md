...
|---|---|---|
| [`14-tuple-validation/`](14-tuple-validation/) phases 02–06 | Phases 00 + 01 shipped in PR #207 (16/17 cells PASS, P206 + T1.8a closed).  **Phase 02 pre-flight 2026-05-04 found P212** — nested tuple literals panic at `src/state/codegen.rs:1527` on both backends, single shared root cause across 3 probes.  Phases 03-06 untouched. | P212 fix triggers phase 02 wiring; phases 03-06 follow in sequence. |
...
| [`16-coroutine-validation/`](16-coroutine-validation/) | Drafted; phase 00 wiring not started.  **Pre-flight 2026-05-04: 0/7 pass (100% yield).**  Filed P210 (Y1/X1 yield int + for, native silently returns 0 — S0), P211 (Y2/X1 yield text, empty stdout / native codegen error).  Y3/Y4 likely unblocked by P210's state-machine fix. | P210 is S0 and gates everything coroutine-shaped; schedule as the next coroutine-related fix. |
...
| [`18-match-validation/`](18-match-validation/) phases 02–05 | Phase 01 closed in PR #207 (or-pattern + `@`-binding hang).  **Phase 02 pre-flight 2026-05-04 found P209 — S0** match guard arms select wrong branch on BOTH backends, *differently* wrong (interp shifts arms by one; native always matches arm 2). Range patterns work. | P209 fix is the gating bite; ranges already pass so phase 02 mostly green after P209. |
