...
|---|---|---|
| P213 layout fix | Wanted future feature (not a release blocker) — widen fn-ref struct fields from 4B (just d_nr) to 16B (d_nr + closure DbRef) so capturing closures can be stored.  Use cases: async/IO callback registries, server main loops tracking many signals, game main loops / event buses, state-machine transition tables.  Full design in [PROBLEMS.md § 213](../PROBLEMS.md#213-capturing-closures-cannot-be-stored-in-struct-fields--full-design-for-the-proper-fix).  Parse-time diagnostic shipped 2026-05-04 is the correct shipping behaviour today. | Co-schedule with whichever lands first: `server` library, `game_client` / game-loop work, or plan-15 phase 03 (step 5 of the design IS phase 03's get_free_vars extension — same shape, do them together). |
