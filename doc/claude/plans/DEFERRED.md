...
|---|---|---|
| P213 + P215 closure-in-struct-field fix (one bug, two surfaces) | Wanted future feature — capturing closures in struct fields and synthetic closure records.  Plan revised 2026-05-04 to **co-located closure record via OpAppendVector** after critical-pass against the code surfaced that the simpler "OpDatabase retarget" idea wipes the host's Store via `database.clear()`.  Current plan: per fn-ref struct field, register two database fields (`__d_nr` 4B int + `__closure_rec` 4B `Parts::Vector(closure_kt)`); field-write does OpSetInt4 + OpAppendVector deep-copying the parent-Store closure record into a new co-located record in host's Store; field-read uses new `OpVectorFirstOrNull` op.  Free/copy cascade automatic via existing `Parts::Vector` walker.  Full design in [PROBLEMS.md § 213](../PROBLEMS.md#213-typefunction-storage-layout-limit--full-design-for-the-proper-fix).  Diagnostics shipped on both surfaces are the correct shipping behaviour today. | Prioritised on its own merit (fix the layout once vs file per-symptom P-issues forever).  Co-schedule with plan-15 phase 03 (heap-capture lifetime concerns overlap). |
