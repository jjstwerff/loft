...
|---|---|---|
| P213 + P215 layout fix (one bug, two surfaces) | Wanted root-cause fix — widen `element_size(Type::Function)` from 4B to 16B so fn-ref values persist correctly through every storage container.  We've discovered this twice in one session (struct fields + nested-closure captures) and the surface inventory shows several more containers (vector, hash, sorted, index, tuple) that haven't been tested with capturing closures and will produce more P-issues over time.  Full design + 21-cell test matrix in [PROBLEMS.md § 213](../PROBLEMS.md#213-typefunction-storage-layout-limit--full-design-for-the-proper-fix).  Diagnostics shipped on both surfaces are the correct shipping behaviour today. | Prioritised on its own merit (fix the layout once vs file per-symptom P-issues forever).  Co-schedule with plan-15 phase 03 — step 5 of the design IS phase 03's `get_free_vars` extension, same shape, do them together. |
