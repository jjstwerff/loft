...
|---|---|---|
| [`14-tuple-validation/`](14-tuple-validation/) phases 02–06 | Phases 00 + 01 shipped (16/17 cells PASS, P206 + T1.8a closed in passing).  Phases 02–06 (nested tuples, closures-in-tuple, struct-ref tuples / T1.8c, struct-field decision, freeze) drafted but not started. | Validation discipline ("finish plans first") will pick these up in sequence; no external trigger needed unless an S0/S1 user-facing issue surfaces. |
| [`15-closure-validation/`](15-closure-validation/) | Drafted (README + 00-matrix); phase 00 wiring not started. | Same as plan-14: scheduled in finish-plans-first sequence.  External trigger: any closure-DbRef leak surfacing as a user-visible memory issue (plan-15 phase 03 risk). |
| [`16-coroutine-validation/`](16-coroutine-validation/) | Drafted; phase 00 wiring not started. | Same as plan-14.  Pre-flight survey gates phases 02+ on bug yield (per the "yield-gate" rule in plan-15/16/17 risk sections). |
| [`17-template-validation/`](17-template-validation/) phases 02–06 | Phase 01 partial — (A) tuple substitution + (C) built-in `Printable` impls closed; (B) bounded-T method-call return-type inference deferred (depth recorded). Phases 02–06 (multi-bound, op-sugar, two-T) not started. | Bug B's documented likely fix (drop the first-pass-bare-skip in `fields.rs:61` for bounded-T) is the trigger.  Phases 02-06 follow once phase 01 closes. |
| [`18-match-validation/`](18-match-validation/) phases 02–05 | Phase 01 partial — or-pattern + `@`-binding hang fixed.  Phases 02-05 (range/guard/null patterns, vector match) not started. | Default "finish plans first" sequence.  External trigger: any user-reported match parse hang or codegen divergence on a pattern shape not yet matrix-covered. |
| [`19-struct-enum-validation/`](19-struct-enum-validation/) | Drafted; phase 00 wiring not started.  Pre-flight found method-on-parent-enum dispatch via `.method()` on a variant value fails. | Default sequence.  Method-resolution gap is a real bug (S2). |
...
|---|---|---|
| Bug B — bounded-T method-call return type inference | plan-17 phase 01 follow-up | Investigation depth recorded in plan-17 README; likely fix is dropping the first-pass-bare-skip in `fields.rs:61` for the bounded-T case.  Trigger: any focused 2-4-hour session OR a plan-17 phase 02+ cell that re-surfaces it. |
