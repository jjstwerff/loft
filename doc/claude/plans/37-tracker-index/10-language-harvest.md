...
| 10.12 | Const vector native codegen crash (@P275) — **scope corrected**: ANY const vector crashes native, not just text | `src/generation/mod.rs::emit_const_vectors` + `src/compile.rs::extract_literal_values` (gate or recognizer) | tests/scripts/<NN>-const-vector.loft — `const NUMS = [10,20,30]; const TAGS = ["a","b","c"]`; assert both backends produce identical output.  See [10a-remaining-bugs-design.md § @P275](10a-remaining-bugs-design.md#p275--const-vector-crashes-native-broader-than-text) for fix design. |
