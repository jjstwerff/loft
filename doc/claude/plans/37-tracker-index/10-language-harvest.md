...
| 10.6 | `hash.contains(key)` method | `default/01_code.loft` (alongside other hash methods) | tests/scripts smoke |
...
| 10.10 | `if X.method(local)` mis-parses with self-slice (@P278) | `src/parser/operators.rs` (or wherever method-call argument parsing meets the slice-reassignment scope) | tests/issues — promote `/tmp/p_followups/p278_*` to a regression test in `tests/scripts/<NN>-p278-method-arg-self-slice.loft` |
| 10.11 | Conditional-reassignment `unknown(0)` typer (@P279) | `src/parser/` or `src/typedef.rs` — branch unification for text-typed reassignments | tests/issues regression that today requires the explicit `: text` annotation; passes once typer unifies the branches |
| 10.12 | Module-scope `const vector<text>` native codegen crash (@P275) | `src/generation/` const-table emission for vector literals | tests/scripts/<NN>-const-vector-text.loft — declare a `const vector<text>` at module scope; assert `--native` runs cleanly |
| 10.13 | `s[i] ?? '<char>'` chain-compare type mismatch (@P276) | `src/generation/` `??` codegen for character receivers | tests/codegen_emitter — emit and verify the `??` chain compiles under `--native` (today: rustc E0308) |
| 10.14 | Local `sorted<T[K]>` re-types to vector on `+= [T{}]` (@P277) | `src/parser/` or wherever `+=` literal-vector append re-types its LHS | tests/scripts/<NN>-local-sorted-append.loft — declare a local `sorted<T[K]>`, append, lookup, iterate; assert it compiles + behaves correctly |
| 10.15 | Two-pass forward-resolution of fn return types (@P281) | `src/parser/` pass-1 — record fn return types in symbol table before pass-2 type-checks bodies | tests/scripts/<NN>-forward-fn-call.loft — caller-defined-before-callee in same file; both should type-check |
