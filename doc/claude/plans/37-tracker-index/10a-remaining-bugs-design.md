...
|---|---|---|---|---|---|
| @P275 | Confirmed — ANY const vector crashes native (text + integer both) | `src/generation/mod.rs::emit_const_vectors` + `src/compile.rs::extract_literal_values` | const-emit | M (3-6 h) | Fix in this phase — gates clear, single mechanism |
| @P276 | Confirmed minimal repro | `src/generation/calls.rs` (`??` lowering for character receivers) — NEEDS DEEPER READ | char-typing | M (4-8 h) | Fix in this phase if cluster simplifies; else defer |
...
| @P283 | Confirmed minimal repro | `src/state/text.rs` (interp) + `src/generation/text.rs` (native) | text-handling | M (4-8 h) | Fix in this phase — same site as 10.7's append_text; cheap to dig in |
