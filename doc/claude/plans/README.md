...
|---|---|---|
| **Bug fix** | A focused fix + regression test + commit.  A [`../PROBLEMS.md`](../PROBLEMS.md) row only if you're **not** fixing it now (it blocks you, or it's M+ — see [CLAUDE.md § Bug-filing policy](../../../CLAUDE.md#bug-filing-policy--mandatory)) | Single root cause, fits in one commit, no design choices, no multi-phase sequencing.  **The default** — most bugs are neither a plan nor even a P-issue row; they're just fixed. |
