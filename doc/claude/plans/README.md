...
| [`future/34-performance-followups/`](future/34-performance-followups/) | Pointer-plan tracking 7 open optimization designs from PERFORMANCE.md (P1 superinstruction merging, P2 store indirection reduction, P3 integer-path long-sentinel verification, N1 direct-emit local collections, N2 omit stores parameter from pure native fns, N3 remove long null-sentinel from generated code, W1 wasm string representation).  Each row links to the PERFORMANCE.md section that holds the full design + the ROADMAP row that schedules it.  Sibling-plans noted for cooperators (21-retire-scratch, 25-native-debug, 33-native-codegen-followups). | Pointer-plan; PERFORMANCE.md stays at doc root as reference (benchmark results, root-cause analysis, design content).  Same shape as 33-native-codegen-followups + lib_plans/11-packages.  P1 blocked on opcode-table capacity decision; other items design-ready. |
| [`future/35-quality-followups/`](future/35-quality-followups/) | Pointer-plan tracking the active sprints + designs in QUALITY.md.  Three clusters: (1) JSON — P54 sprint (`JsonValue` enum), Q1 (parse-error diagnostics), Q2 (free-form iteration + kind peek), Q3 (`to_json` serialiser), Q4 (`JsonValue` construction in loft), P54-U (unified parser); (2) Native runtime — Dep-inference for native fn returns (cooperates with 21-retire-scratch); (3) Compiler blockers — B2-B7 struct-enum bugs gating P54 (audit needed; some may have closed via plan-17 / plan-19 sweeps).  Open programmer-biting issues + Enhancement tiers + Recommended landing order stay as reference content in QUALITY.md. | Pointer-plan; QUALITY.md stays at doc root as reference.  Same shape as 33-native-codegen-followups + 34-performance-followups + lib_plans/11-packages.  P54 active sprint is the largest in-flight item. |
...
| `finished/04-slot-assignment-redesign/` | Closure record only.  Reference for shipped slot assignment (V1 allocator + V2 shadow validator + frame layout + I1-I7 invariants + diagnostic tools) lives at [`doc/claude/SLOTS.md`](../SLOTS.md).  Plan documents the retracted V2-drive design and what shipped in its place (incremental refit: positional init ops, single function-entry `OpReserveFrame`, slot-move deletion, `OpText` deletion, I7 invariant). | 2026-04-23.  Closure-rule extraction: SLOTS.md was already comprehensive for the SHIPPED state; plan trim 2026-05-09 added "moved to SLOTS.md" pointer + "Why V2 retraction" archaeology section. |
...
| `finished/31-html-export/` | W1.1 — single-file HTML export.  Closure record only.  Reference for the shipped pipeline (cdylib codegen, WebGL2 import bridge, frame-yield contract, `wasm-opt` integration, HTML assembly format) lives at [`doc/claude/HTML_EXPORT.md`](../HTML_EXPORT.md). | Closed 2026-05-09; reference content extracted to `doc/claude/HTML_EXPORT.md` 2026-05-09 per the closure rule (canonical example of the rule's application). |
