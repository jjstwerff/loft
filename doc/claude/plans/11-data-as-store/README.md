...
- **Step 4 option 3 — background build** (no foreground `rustc` on the settling run;
  see Step 4 above).
...
**F5 — the `[store-guard]` detector is silent on all current inputs (no constructible positive control).** 🔄 *(surfaced 2026-06-05 implementing N5's E leg)*  Folding `LOFT_STORE_GUARD` into the parity harness (every mixed-path run now also runs the store-confinement detector) is cheap and correct, but **the detector could not be made to fire from loft source** — tried branch-reassignment, loop-rebind, nested-block-escape, and the plan-57 probe corpus, plain and under `LOFT_CONF_RECOVER`, all silent.  *Why (from the code):* post-plan-57 `store_lifetime_guard` re-runs `store_confinement`, whose single-store entries the cluster-I fix has already relocated to block scope (so "stored late" no longer holds), and whose multi-store / ambiguous-backing cases it `continue`s past — so the reported set is empty by construction.  This is the **expected Goal-E healthy state** ("silent corpus-wide"), but it means the E-leg assertion is a *regression tripwire with no demonstrated firing*, not a positively-controlled gate (the rigor "silence-is-evidence-only-after-a-positive-control" clause).  *Open question worth its own pass:* can `[store-guard]` fire on **any** input on today's `main`?  If not, the detector — and Goal E's "keep it silent" check that leans on it — is unfalsifiable until a constructible firing case (or a `cfg`-gated fault-injection that disables the cluster-I relocation) is found.  Until then the mixed boundary's load-bearing soundness signal is the **differential parity** (D), which *is* positively controlled by the reference-output anchor.
