...
**Open — arc B COMPLETE (`text2d` 0.4.0), `C1` shipped (`tween` 0.1.0), `D0b` answered; `C2`, `D1`, `D2` remain (`D0` is moros's clock).** These are the libraries a game author writes against
...
**`C2` is what is left that depends on nothing.** `C1` shipped 2026-08-20, and it narrowed
`C2` rather than merely unblocking it — see the `C2` row for the package-shape question `C1`
deliberately left open. `D0b` ran the same day and cleared `input` for `D1`'s replay harness
with one named limit (no modifiers) that `D2` has to plan around.
