...
**COMPLETE — every phase has shipped and every package resolves from the registry.** `text2d` 0.4.0 (arc B), `tween` 0.1.0 (`C1`), `stage` 0.18.0 (`C2` + `D1` + `D2`), `D0b` answered. **`D0` is DONE on both sides** — moros shipped `lavition_ui` 0.1.0 on 2026-08-20, the red gate in `loft-lang/registry` that predated the submission turned out to be a publish-path defect of ours, and with that fixed their PR rebased, merged and the package now resolves from the registry (2026-08-21) — and no `ui` package will be built (§ The `ui` package that is not ours). These are the libraries a game author writes against
