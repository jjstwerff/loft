...
| **A3b** — one `gl_draw_instanced` per run | `stage` | pixels identical to A2 **and** draw calls drop from N to O(runs). Both halves — same pixels alone would pass a batch that silently fell back to per-sprite. Needs a window; the four instancing primitives are bridged and gated by `tests/gl_instancing.rs` | Open |
...
| Arc | Waits on | Then |
|---|---|---|
| **A** scene | — | `A0` ✅ · `A1` ✅ · `A2` ✅ · `A3a` ✅ · **`A3b` next** → `A6` |
