...
| **A3** — batched renderer behind the same API | `stage` | ✅ **Prerequisite cleared** — the four instancing primitives (`gl_upload_instance_buffer`, `gl_instance_attrib`, `gl_draw_instanced`, `gl_update_buffer`) are bridged to WebGL2 and gated on read-back pixels by `tests/gl_instancing.rs`, not on the import existing. Then: pixels identical to A2 **and** draw calls drop from N to O(atlases). Both halves — same pixels alone would pass a batch that silently fell back | Open |
