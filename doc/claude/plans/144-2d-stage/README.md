...
|---|---|---|---|
| **A0** — falsify the batching premise | [`probe-a0.loft`](probe-a0.loft) | **DONE — the premise holds on native, and the browser cannot run it at all.** Min-of-3 after warm-up, 60 frames, SwiftShader under `xvfb`: `--native` **1.20× / 1.53× / 2.50×** at N = 100 / 1000 / 5000; `--interpret` 1.32× / 2.03× / 3.74× (higher because the interpreted per-sprite loop is part of what batching removes). Monotonic and growing with N, which is the shape A3 needs | ✅ **Shipped** |
...
| **A3** — batched renderer behind the same API | `stage` | ⚠ **Blocked in the browser until the four instancing primitives are bridged** — `gl_draw_instanced`, `gl_instance_attrib`, `gl_update_buffer` and `gl_upload_instance_buffer` are absent from `doc/loft-gl-wasm.js`, so a `--html` build compiles, each returns its zero value, and the stage draws **nothing**. A0 found this; it is a prerequisite, not a detail. Then: pixels identical to A2 **and** draw calls drop from N to O(atlases). Both halves — same pixels alone would pass a batch that silently fell back | Open |
