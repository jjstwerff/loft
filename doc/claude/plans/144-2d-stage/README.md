...
- **Effort:** MH — 18 phases in three arcs (`A3` split in two), **none above M**; see § Effort per phase
...
| **A3b** — one `gl_draw_instanced` per run | `stage` | ⚠ **Written, gate NOT trustworthy — stays open.** The draw-call half is verified (3 sprites / 2 atlases → 2 calls, and same-pixels alone would pass a silent fallback). The **pixel** half is nondeterministic: the same binary on the same source gives 0/12288 or 1528/12288 differing pixels — 1528 being exactly the rects' area, i.e. nothing drawn — at about one pass in five. Eliminated: vec2-vs-vec3 uniforms, the per-run attribute offset, reading after `gl_swap_buffers`, and first-frame upload. `gl_finish` has since been added to `graphics` and makes the failure **deterministic** rather than flaky. ⚠ **The environment is NOT at fault — that earlier conclusion is retracted.** Measured under the same xvfb: a clear colour reads back exact (before *and* after a swap), a deliberately broken shader correctly answers handle `0`, an attribute-free triangle paints 1024/1024, and a clip-space quad through `gl_upload_vertices`' attribute 0 paints 1024/1024. **FOUND — [loft-libs-graphics#33](https://github.com/loft-lang/loft-libs-graphics/issues/33).** The instanced draw path silently paints nothing when the calling library runs as a **cdylib**; the same library drawing through `gl_draw` in the same run is fine, and `LOFT_NO_NATIVE_LIBS=1` makes it pass every time. It looked intermittent because editing the library triggers the dev-interpret fallback, so the first run after any edit passes. `A3b` waits on that fix, not on this plan | Blocked on #33 |
...
| Arc | Waits on | Then |
|---|---|---|
| **A** scene | — | `A0` ✅ · `A1` ✅ · `A2` ✅ · `A3a` ✅ · `A3b` ⚠ blocked on a flaky harness · `A4` → `A6` |
