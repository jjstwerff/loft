...
| **G3** | Delete each **dead non-`_dest` native fn + its FUNCTIONS entries**, ONE batch per commit (loft decl stays — a `library_names` miss then loudly catches any residual emit). Batches: JSON (`n_kind`/`n_to_json`/`n_to_json_pretty`/`n_as_text`/`n_struct_to_json`(`_pretty`)/`i_parse_errors`/`n_json_errors`), text methods (`replace`/`to_lowercase`/`to_uppercase` non-dest), util (`n_source_dir`/`n_store_memory`/`n_ymd_days_ago`/`n_env_variable`/`n_parallel_buf_get_text`) | ~13 dead producer fns + registrations | per batch: `issues` (JSON), `wrap` (text), the producer's own regression (`196`–`198`) |
