...
Concrete, verifiable implementation plan for @PLN82 Phase C/D revival.
Drafted on the `scripting` branch (2026-05-31) after the
lib-extraction approach was **measured** to deliver ~0 ms and rejected
(see [`lib_plans/59-lazy-stdlib`](../../lib_plans/59-lazy-stdlib/README.md)).
...
**matches** the WASM parse-bound profile in the @PLN82 README — the
...
`make ci` green; this doc + @PLN82 README updated with real numbers.
...
- ⬜ `make ci` green throughout; default-on with `LOFT_NO_CACHE=1`
  escape (Step 5).
- ⬜ This doc + @PLN82 README carry the final before/after numbers.
