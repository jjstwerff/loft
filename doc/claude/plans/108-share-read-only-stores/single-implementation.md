...
| **R4b** | **queue_ref adopt + delete `clone_for_worker`.** Probe 3 caught it: queue_ref's dispenser-based output-slot adoption **collides with the light clone's self-allocated scratch** (`"Allocating a used store #8"`, suite 46/47) — the self-scratch occupies local indices the dispenser also assigns. Needs a scratch-free (or dispenser-aware) borrow variant of the clone. Only then does the last `clone_for_worker` go. | Open — queue_ref adopt on borrow: total=288 + leak-clean + suite 47/47 |
