...
**13 of 18 shipped** (`E1` · `W0` · `W1` · `W2` · `W3` · `F1` · `F2` · `F3` · `F4` · `F5` · `F6` · `F7a` · `F7`),
**`W4` blocked on [loft#1085](https://github.com/loft-lang/loft/issues/1085)** (its renderer is done and exact on `--native`;
the interpret lane is an honest red — [W4.md](W4.md)), 5 open — and **arc F really is complete now**: `F7` was still blocked when
...
- **Effort:** H — 18 phases in three arcs, none above M. **Design:** ✓. **`E1` shipped 2026-08-19; `W0` + `F7a` 2026-08-21; `F1` + `F2` + `F3` + `F5` + `F6` 2026-08-22; `F4` + `F7` + `W1` + `W2` 2026-08-23; `W3` 2026-08-24.**
...
| **W4** — `Petals` and `Fronds` | `drawing` | pixel-identical over the corpus subset — **and the seeded field reproduces**: same seed, same pixels, on `--interpret` and `--native` alike, because a jittered array whose backends disagree is not a renderer | 🟡 **Renderer done, gate honestly red** — [W4.md](W4.md): **37 of 37 scenes, 0 pixels different** on `--native`, `UNBUILT` down to `Petals` alone (no scene uses it). The seeded arithmetic needed no new dependency. Two findings, and the second is why this row is not ✅. The phase's recorded blocker was already fixed upstream ([loft#1082](https://github.com/loft-lang/loft/issues/1082) in #1084), so the WIP's rewrite came out — the **committed** form renders all 37 exactly. And `w2.sh`'s two backends were **one binary**: a `use`d library resolves to its `native-auto/` cdylib EVEN UNDER `--interpret`, so the interpret lane had been re-measuring compiled `drawing`. `LOFT_NO_NATIVE_LIBS=1` separates them, the gate now sets it — and the interpreter draws a DIFFERENT picture: `661.7595` hand-computed from the oracle against `663.464032166672`, 1167 pixels on `leaf`. Reduced to fourteen library-free lines it is [loft#1085](https://github.com/loft-lang/loft/issues/1085), a stale return buffer freed twice so a live local's store slot is recycled under it |
