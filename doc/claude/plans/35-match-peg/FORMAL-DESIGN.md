...
| **D-F5** | **Struct-variant patterns only** (no positional tuple-variant `Num(i64)`). | loft's enum model is struct-variant (`types.md` / `matching.md`); tuple-variant sugar is separable. | out of scope; `Num(i64)` stays a parse error until a separate feature. |
