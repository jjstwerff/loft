...
| **M3.15** | The two native-`Value`-clone clusters: `generate_set` (builds synthetic `Value::Call` protect/copy nodes + `value.clone()` + `set_var`/`gen_set_first_at_tos`) and `generate_call` (threads `parameters` through `gather_key`/`try_text_dest_pass`, inspects `parameters[0]` as `Var`).  These need a **materialize-sub-value** handle op (`IrNode::to_owned() -> Value`: native clones, store `read_value`s) so the synthetic-construction paths get a native `Value`, plus conversion of their `&Value`/`&[Value]` sub-helpers.  This is M5-design-adjacent, not mechanical. | suite green | M |
