...
| **M3.2…n** | Convert the remaining `generate_inner` groups + the `gen_*` helpers' `Value`/`Type` matches, **function-group by function-group** (one commit each, native backing); the `unreachable!` list in `match val` shrinks as arms move up to the `kind()` match.  Single/compound children use the child accessors + `native()` recursion bridge. | suite green per group | several S |
