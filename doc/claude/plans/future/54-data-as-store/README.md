...
| **M3.6…n** | Finish `generate_inner` (Block/Loop via a `Block` handle; TupleGet/TuplePut via helper extraction), then the **other** `gen_*` helpers' `Value`/`Type` matches across `state/codegen.rs` (gen_fn_ref_value, gen_set_first_*, generate_call/_block/_var, …) — the bulk of the 451 sites.  Same group-by-group discipline. | suite green per group | several S |
