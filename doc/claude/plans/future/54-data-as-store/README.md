...
| **D2a** | **Cache the database type schema** (chosen path 2; user 2026-06-02) so a loaded `Data`'s baked `known_type`s stay valid with no parse-time `fill_all` rebuild.  Steps: **(1) ✅** transcribe the database schema types into `ir.loft` (`DbType`/`DbParts`[17]/`DbField`/`DbContent` + `EnumPair`/`KeyField`, reusing `Key`/`LinkedFieldGroup`; `Type.parents` derived/not stored) + a `Bundle { data, types }` root; regenerate `ir_schema_gen.rs` (98 structs + 4 enums); guard green.  **(2)** materialize `Stores.types` into the store; **(3)** read it back; **(4)** wire into `Data::save`/`open` via the `Bundle` root; **(5)** green the warm-path test.  Prereq ✅: `from_snapshot` sets `done=true`. | per-step: guard test; then a warm-load-vs-cold-parse `compare_data` + schema-equality test | M–L (step 1 done) |
