...
| **D** — mmap load | `Data::open(path)` → `Store::open` → live IR, zero rebuild.  Wire into the startup path behind the bundle cache key. | Probe done — mmap → `read_data` rebuild loop, **~12× faster than `parse_dir`**; **D1** (`Data::save`/`open`) + **D2a** (schema cached in the store via the `Bundle` root, so a warm load reconstructs `Data` **and** a valid type schema — the @PLAN28 schema-rebuild wall resolved) landed.  Remaining: **D2b** (main.rs opt-in startup wiring) + drift Q4 / mutability Q1 / `caller_index` Q3. |
