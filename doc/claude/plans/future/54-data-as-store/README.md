...
| **M6** 🔄 | Drop the native `Vec<Definition>`/`Box<Value>` body graph once every reader is store-backed — reads become **zero-copy**.  **Codegen pipeline fully store-backed:** M6.1 `extract_literal_values`, M6.2 `build_const_vectors` (feeds the persistent store), M6.3 `def_code`'s null/empty-stub checks — with M2 (body lowering), **every body reader inside `byte_code_from` now reads from the persistent store**, proven (issues 684 + native suite via `LOFT_CODEGEN_STORE`).  **Remaining for the physical drop:** the *pre-codegen* reader `scopes::check` (4630 lines, 392 `Value` matches — it **rewrites** bodies: `relocate_null_init`/`lastuse_reclaim`, so a store-backed scopes needs **E2** mutability), the debug `show_code` dumps, the store-lifecycle restructure (materialise once after `scopes`, drop the native bodies, thread the persistent store into `byte_code`), and the cached-load win (skip `read_data`'s native reconstruction) which is likewise E2-gated. | suite green; bench | M — **codegen readers done** |
