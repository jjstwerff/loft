...
| **E** — bundle snapshots | Core `stdlib.store` (shared) + per-script bundle snapshot (core + sorted lib-set), each keyed for drift. | In progress — **cache-robustness landed** (atomic temp+rename write so a killed process never leaves a half-bundle; `Store::is_store_file` pre-check + graceful reparse on a corrupt/foreign file — `corrupt_bundle_falls_back_to_parse`).  Open: whole-bundle keying (core + the script's `use`d libs) — **blocked on the snapshot boundary** since main's lazy auto-`use` loads libs *interleaved* with user code, so "snapshot core+libs, reparse user" needs a lightweight `use`-scan to key the bundle without parsing.  Plus mutability Q1 / `caller_index` Q3 (E2). |
...
| **E1** | Bundle cache key = stdlib key + sorted lib-set + content hashes; drift ⇒ reparse (Q4).  **Robustness half ✅** — atomic `save_bundle` (temp+rename) + `open_bundle` validity pre-check (`Store::is_store_file`) → a corrupt / partial / foreign bundle is a clean cache miss, not a crash.  **Open:** the whole-bundle key itself (the stdlib half is the D2b key; extending to the `use`d lib-set needs a `use`-scan to determine the lib-set without a full parse — see § arc E row re: lazy auto-`use`). | `corrupt_bundle_falls_back_to_parse`; then drift unit tests | M (robustness done) |
