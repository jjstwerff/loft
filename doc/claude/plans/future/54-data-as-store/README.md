...
| **M3.9…n** | Remaining `state/codegen.rs` `&Value`/`&[Value]`-taking helpers → `IrNode`/`IrNodeList`: `gen_if` (needs `is_divergent`/`==Null` handle forms — SIGSEGV-sensitive), `generate_set`, `generate_call`/`_call_ref`/`gen_parallel` (iterate `IrNodeList`), `generate_block`/`gen_loop` (need a **`Block` handle**), plus `size_code`.  Each removes more `as_native()` bridges; when none remain, `generate_inner` is fully store-capable. | suite green per group | several S–M |
