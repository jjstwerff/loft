...
**In-progress — arc A0 + arc A landed; arc B's write path COMPLETE (whole native `Data` materializes into a store, exercised on the real stdlib).**  Next: arc C (store→native read accessors), which also unlocks bit-for-bit `compare_data` validation of the materialized store.
