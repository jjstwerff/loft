...
- **Arc B write path (in-progress)** — `src/data_store.rs` grew the write side: per-variant `write_null/write_int/write_call/write_block/write_loop`, generic typed field accessors (`field_int`/`set_field_int`, …float/single/bool/str, `field_vec`, `set_discriminant`), and `ValueType` + `value_type` now cover all 34 `Node` variants.  `src/ir_store.rs::materialize_node` (the `data_to_json` walk with a store-writer sink) covers **32 of 34** `Node` variants — every scalar + `vector<Node>` shape, the inlined `Block`/`Loop`, and the inlined-sub-struct variants `Span`(Position) + `ParFor`(ParForBody).  The remaining 2 — `Keys` (`vector<Key>`, needs a non-Node struct-vector handle) and `FnRef` (`vector<TypeT>`, needs the TypeT half) — land with the next increment.  All baked discriminants + offsets are pinned by the `baked_layout_mirrors_loft_schema` guard (probed from the real schema, not guessed; inline sub-struct offsets verified as base + relative).  11 lib tests green (8 materializer round-trips + 3 handle/guard).  Arcs C/D/E remain open.
...
| **B** — write path | Materialize a parsed native `Data` into store records (validates the schema).  **Greenfield, not "largely done":** @PLAN28 shipped a *JSON* snapshot (native-side, `LOFT_DUMP_SNAPSHOT` → `data_to_json`), **not** a store-format one.  Reuses `ir_schema::data_to_json`'s exhaustive native-IR walk as the traversal skeleton (JSON sink → `data_store`-handle store-writer sink) + `compare_data` as the equivalence oracle.  **Progress:** `src/ir_store.rs::materialize_node` covers **32/34 Node variants** (all scalar + `vector<Node>` shapes, inlined `Block`/`Loop`, and inlined sub-structs `Span`/`ParFor`); `data_store.rs` has generic typed field accessors + all-34 `ValueType`/`value_type` + guard-pinned offsets.  **Remaining:** `Keys` (needs a non-Node `vector<Key>` handle) + `FnRef` (needs `TypeT`), then the **24 `TypeT` variants** + `Definition`/`Attribute`/etc., then a whole-`Data` materializer validated by `compare_data` against a fresh parse. | In-progress — 32/34 Node variants materialized + round-trip tested |
...
2. **B (write)** — native `Data` → store.  Testable artifact; validates A
   ...
   fork, below) ✅ done.  **In-progress:** `ir_store::materialize_node` covers
   32/34 Node variants; next increments add `Keys` + `FnRef`, the 24 `TypeT`
   variants + `Definition`/`Attribute`, then a whole-`Data` materializer.
