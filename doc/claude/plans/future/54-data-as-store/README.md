...
**In-progress — arc A0 + arc A landed; arc B's write path COMPLETE; arc C started — the `Value`/`Type` store→native reader landed (round-trips validated by the IR's own derived `PartialEq`).**  Next within arc C: the `Definition`-level reader (`Attribute`/`Function`/`Variable`/`Definition`/`Data`) → whole-`Data` round-trip → bit-for-bit `compare_data` validation.
...
- **Arc C read path (in-progress)** — `src/ir_read.rs` is the store→native reader, the exact inverse of `ir_store.rs`.  **`read_value(&Stores, Node) -> Value`** rebuilds all 34 `Node` variants and **`read_type(&Stores, Record) -> Type`** rebuilds all 24 `TypeT` variants, plus every sub-struct reachable from them (`Block`, `ParForBody`, `Position`, `Key`, `IntegerSpec`, `vector<SortKey>`/`vector<NameRef>` key lists, `vector<integer>` dep lists).  Box-of-one `vector<…>` fields read back as `Box<Value>`/`Box<Type>`; N-element vectors as `Vec`.  `Block.name` (`&'static str`) is reconstructed via a bounded `Box::leak`, mirroring the @PLAN28 JSON decoder (open question 2).  Validated by **`native → store → native` round-trips asserted with the IR's own derived `PartialEq`** — a stronger oracle than the JSON re-encode, and needing no JSON.  7 round-trip tests (all `Value` leaves + recursive/box-of-one/Block/Loop/Span/ParFor/Keys/FnRef variants; all 24 `Type` variants + nested recursion; an explicit `forced_size` check since `IntegerSpec`'s `PartialEq` ignores it).  445-test lib suite green; fmt-clean, clippy-0.
  ...
  **Next within arc C:** the `Definition`-level reader (`Attribute`/`LinkedFieldGroup`/`Variable`/`Function`/`Definition`/`Data`) and a whole-`Data` reader, then wire `compare_data(fresh, read_data(materialize_data(fresh)))` on the real stdlib for bit-for-bit validation.  That step must resolve the `Function.names`/`inline_ref_vars` question: the store omits both (the schema stores only the 9 codegen-read `Variable` fields).  `Function::from_snapshot` takes `names` + `inline_refs` explicitly, so the reader must either rebuild them from the variable list (`names` is `name → index`, likely reconstructible; `inline_ref_vars` is compile-derived and may **not** be) **or** the store schema must grow to hold them.  `compare_data` compares the serialized `variables` block, so this is load-bearing for the bit-for-bit oracle — a design decision to settle when the `Definition` reader lands.
...
| **C** — read accessors | `data.def(dnr)` + `value` / `type` matching read from the store instead of `Vec`/`Box`.  The ~940-site migration — **done incrementally via the accessor seam, never at once** (see § Incremental migration).  Minimum seam (`src/data_store.rs` handle layer) landed; the **store→native reader** (`src/ir_read.rs` — `read_value`/`read_type` over all 34 Node + 24 TypeT variants + every sub-struct, `PartialEq` round-trip validated) landed as the first arc-C increment.  Next: `Definition`-level reader + whole-`Data` `compare_data` validation; then the bulk read-site migration. | In-progress — `Value`/`Type` reader done; `Definition`/`Data` reader + bulk migration are the remainder |
