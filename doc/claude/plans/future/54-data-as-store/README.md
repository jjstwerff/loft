...
**Phase A — wire `use` → native dispatch (the headline, on the real binary)** [maps to N3 productization]
- **A1 ⬜ Manifest opt-in** — `[library] compile = "native"` field + parse (`manifest.rs`).  Explicit for now; B drops it.
- **A2 ⬜ Parser hook** — `use foo` resolves an opt-in library (`apply_manifest_side_effects`) → record `(lib_source, pkg_dir)`.
- **A3 ⬜ `main.rs` orchestration** — after parse: `mark_native_exports(data, {d | def.source()==lib_source})`.  After `byte_code`: `build_shared_cdylib(...)` → push `.so` to `pending_native_libs`.  Add `wire_shared_native_fns` beside `wire_native_fns`.
- **A4 ⬜ Fixture + subprocess test** — a real `tests/lib/mylib/` + an `exit_codes`-style test asserting `cargo run -- uses-mylib.loft` output **== the interpreted run**.  *First proof of the ideal-state core.*
