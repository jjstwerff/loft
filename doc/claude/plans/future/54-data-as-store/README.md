...
| **D2a** | **Cache the database type schema** (chosen path 2; user 2026-06-02) so a loaded `Data`'s baked `known_type`s stay valid with no parse-time `fill_all` rebuild.  Steps: **(1) ✅** transcribe the database schema types into `ir.loft` (`DbType`/`DbParts`[17]/`DbField`/`DbContent` + `EnumPair`/`KeyField`, reusing `Key`/`LinkedFieldGroup`; `Type.parents` derived/not stored) + a `Bundle { data, types }` root; regenerate `ir_schema_gen.rs`; guard green.  **(2) ✅** baked layout consts (guard-pinned) + `ir_store::materialize_schema` writes `Stores.types` → `vector<DbType>` (all 17 `Parts` variants, `Field`/`Content`, the `Type.{complex,linked,size,align}`/`Field.other_indexes` read seam, `Record` float/single accessors), exercised on the real stdlib schema.  **(3)** `read_schema` (inverse); **(4)** wire into `Data::save`/`open` via the `Bundle` root; **(5)** green the warm-path test. | per-step: guard test; `materialize_stdlib_schema_smoke`; then warm-load-vs-cold-parse `compare_data` + schema-equality | M–L (steps 1–2 done) |
