...
**G2/M5 reached for the interpreter (2026-06-03): the interpreter codegen runs fully store-backed, proven byte-identical to native.**  `generate_inner` and every helper it reaches read the IR through the `IrNode`/`IrNodeList`/`IrBlock` handles (the native-`Value`-clone clusters `generate_set`/`generate_call` materialise at their boundary); `def_code` flips to `IrNode::Store` under `LOFT_CODEGEN_STORE`, lowering the whole program from a store with identical output across all `tests/scripts/*.loft` + the M5 equivalence test.  Built on G1 (cold-start cache shipped) + the M0 equivalence harness.  **Remaining:** M4 (the `src/generation/` native backend — the second, equal-sized lowering surface) → its M5 flip; M6 (drop the native graph for zero-copy); E2 (locked-mmap mutability).
...
| **M5 (native backend)** | Same flip for `src/generation/` once M4 has converted it. | native suite | M |
