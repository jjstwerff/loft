...
1. **Serialization mechanism.**  Two candidates:
   - **Reuse [@PLAN38 loft-store-durable](../38-loft-store-durable/README.md)** mmap/durable-store machinery — the store is already a word-addressed heap with a durable-format arc in flight.  Cheapest if PLAN38 lands a serializable store.  **But:** see Q2 — the mmap-reinterpret form is Miri-hostile.
