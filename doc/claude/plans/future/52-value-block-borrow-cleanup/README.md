...
| C — Fix design (OPTIONAL) | ⏸️ pending — cluster I fix shape is now uniquely determined (parent-scope text temp à la B5-L3 for is_return=false).  Cluster III likely closes incidentally with cluster I.  **Cluster IV-Vec/Hash/Enum needs separate native-codegen fix** (the null-check predicate emit at src/generation/emit.rs — needs to project the DbRef's null sentinel before testing).  Three independent fix surfaces; may want Stage C deliverable to compare. |
| D — Implementation | ⏸️ pending Stage B-finish + Stage C design choice |
...
|---|---|---|---|---|---|
| I ✅ | `??`-on-text — ANY non-Var LHS lowers to a value-block whose tail returns `Str` borrowing into block-local `_ncc_N`; scope-exit `OpFreeText` invalidates the stack Str before the consumer reads.  **Shape-only, allocation-class-independent.**  Three failure sub-modes by consumer:  **IA (NUL fill)**: pure-Set consumers (var = …, tuple destructure, concat-after) → libmalloc free-fill produces NUL.  **IB (garbage fill)**: buffer-write consumers (`OpAppendText` via `+=` / format-string / hash-insert / struct-field / vec-append / enum-ctor-field / direct `print` with prior buffer state) → target buffer's prior content reused at the freed region.  **IC (false-equality)**: equality consumer reads dangling bytes, returns false. | Silent data corruption | Interpret only | 02, 04-06, 11, 13-17, 20, 24-26, 29-34, 37-39, 44 | _(TODO: `cluster-I-ncc-text.md` once fix lands)_ |
| I-crash ✅ | **NEW 2026-05-29**: cluster I with method-chain consumer (`(vec[i] ?? "x").to_uppercase()`) produces **SIGBUS** at opcode 220, PC ~6941 on the interpreter.  Method-dispatch reads the dangling Str's `ptr` from stack; the bytes-region is unmapped between scope-exit and dispatch.  This is a **hard crash**, not silent corruption — the most severe cluster I sub-mode discovered. | Hard crash (SIGBUS) | Interpret only | 46, 49 | _(bundle with cluster I)_ |
...
| III ✅ | Format-string `{x ?? "y"}` interpolation — `??`-DEPENDENT (probe 18: plain `"{vec[i]}"` PASSES).  Garbage-fill variant of cluster I-IB; format-buffer's `OpFormatText` writes to `__work_N` between the if-result push and `OpFreeText`, so the freed memory gets rewritten with format-buffer leftovers.  Compounds with multi-`??` (probe 19). | Silent corruption inside formatted output | Interpret only | 09, 19, 30 | _(TODO: `cluster-III-format-string.md`)_ |
...
| IV-Vec ✅ | `Vector` value-blocks via `??` — interpret silently returns zero-valued elements (probe 21: tag=0; probe 36: iter runs 1× instead of 3×); native COMPILE FAILS with E0308 (`if var__ncc_N {...}` emits `DbRef` where rustc expects `bool` — null-check codegen wrong). | Silent corruption + native compile error | BOTH backends fail | 21, 36 | _(TODO: `cluster-IV-heap-typed.md`)_ |
| IV-Hash ✅ | `Hash` value-blocks via `??` — interpret null collection; native E0308 same as IV-Vec. | BOTH fail | BOTH backends | 22 | _(bundle with IV-Vec)_ |
| IV-Sorted ✅ | `Sorted` value-blocks via `??` — same shape as IV-Hash; interpret null; native E0308. | BOTH fail | BOTH backends | 41 | _(bundle with IV-Vec)_ |
| IV-Index ✅ | `Index` value-blocks via `??` — same shape; interpret null; native E0308. | BOTH fail | BOTH backends | 50 | _(bundle with IV-Vec)_ |
| IV-Spacial ✅ | **PARSER hang** (corrected diagnosis 2026-05-29 via `LOFT_TIMEOUT=8`).  `spacial<Point[name]>` causes the PARSER to enter an infinite loop — the breadcrumb (PLAN49 cooperative-exit lever) localises to `phase=parse` at the probe file.  Probe 62 confirms the hang fires WITHOUT `??` too, so the root is in spacial type resolution.  In-plan investigation: probes 88-90 to be added vary key types + parser instrumentation to localise the recursion site.  Likely fix: depth cap or memoisation in `src/parser/` type-resolution.  Kept in-plan because the type-resolution machinery interacts with `??`'s type-merging step that cluster I + IV codegen also touches — fixing in isolation risks missing cross-cluster overlap. | Parser hang | BOTH backends — parse-phase, never reaches codegen | 51 | _(TODO: `cluster-IV-Spacial-parser.md` once probes 88-90 localise the recursion)_ |
| IV-Enum ✅ | struct-`Enum` value-blocks via `??` — interpret returns fallback variant entirely; native E0308. | BOTH fail | BOTH backends | 23, 38 | _(bundle with IV-Vec)_ |
| IV-Tuple ✅ | `Tuple` value-blocks via `??` — interpret returns fallback; native E0308.  Tuples join the IV family. | BOTH fail | BOTH backends | 40 | _(bundle with IV-Vec)_ |
...
| VI ✅ | **NEW 2026-05-29**: Closures with `??` in their body — interpret has cluster I NUL fill (probes 45/65/86) AND native E0308 compile failure.  Closures aren't covered by either side's mitigation; the closure-synthetic-fn path doesn't get the same scope-exit-free protection.  In-plan investigation: probe 87 (to be added) will trace the closure-synth fn IR — if mechanism is identical to cluster I's `_ncc_N` pattern, the cluster I fix MUST cover the closure-synth fn body (one fix surface); if different, a targeted scope-handler fix in the closure path.  Kept in-plan because the cluster-overlap question is load-bearing for the cluster I fix design. | Silent corruption + native compile error | BOTH backends fail | 45, 65, 67, 86 | _(TODO: `cluster-VI-closure.md` once probe 87 confirms mechanism)_ |
| VII ✅ | **NEW 2026-05-29**: Chained `call() ?? call() ?? literal` and recursive `??`-using fns — interpret silent corruption (cluster I), native fails with `if-else have incompatible types` E0308 — a SEPARATE native codegen bug from IV's null-check gap.  The chained-coalesce join codegen emits branches with mismatched types. | Silent corruption + native compile error | BOTH backends fail | 47, 48, 82 | _(TODO: `cluster-VII-chained-call.md`)_ |
...
| `21-vector-coalesce.loft` | `vec_of_vecs[i] ?? other_vec` — Vector value-block | **IV-Vec ✅** | **FAIL** `a[0].tag = 0` | **FAIL** E0308 codegen |
| `22-hash-value-coalesce.loft` | `wrapper.maps ?? other_hash` — Hash value-block | **IV-Hash ✅** | **FAIL** `e.value = null` | **FAIL** E0308 codegen |
| `23-enum-value-coalesce.loft` | `vec_of_enums[i] ?? other_enum` — struct-Enum value-block | **IV-Enum ✅** | **FAIL** got fallback | **FAIL** E0308 codegen |
...
| `36-iter-over-vec-coalesce.loft` | `for x in (vec_of_vecs[i] ?? other)` — iter consumer | **IV-Vec (iter)** | **FAIL** count=1 want 3 | **FAIL** E0308 |
...
| `40-tuple-value-coalesce.loft` | `vec_of_tuples[i] ?? other_tuple` | **IV-Tuple ✅** | **FAIL** returned fallback | **FAIL** E0308 |
| `41-sorted-coalesce.loft` | `wrapper.sorted_field ?? other_sorted` | **IV-Sorted ✅** | **FAIL** `null` | **FAIL** E0308 |
...
| `47-fallback-chain.loft` | `lookup1(k) ?? lookup2(k) ?? "default"` | I + new native bug | **FAIL** `'    '` | **FAIL** `if-else have incompatible types` |
| `48-recursive-fn.loft` | recursive fn with `??` chain | I + new native bug | **FAIL** `'  '` | **FAIL** `if-else have incompatible types` |
| `49-sigbus-minrepro.loft` | minimum reproducer of probe 46's SIGBUS | I-crash | **SIGBUS** at op=220, PC 6941 | PASS |
| `50-index-coalesce.loft` | `index<Entry[name]>` value-block | **IV-Index ✅** | **FAIL** `null` | **FAIL** E0308 |
| `51-spacial-coalesce.loft` | `spacial<Point[name]>` value-block | **IV-Spacial PARSER hang** | **PARSER infinite loop** (phase=parse per PLAN49 breadcrumb) | same parser hang |
...
| **1** | **Fix IV-Spacial parser hang** — pure parser-side, no codegen/scope interaction.  Per `cluster-IV-Spacial-parser.md` Option A/B/C depending on the loop-site finding | Probes 51/62/88/89/90 PASS on both backends; Set H still green; `make ci` green | 1-2 days | LOW |
| **2** | **Fix cluster IV + VII** — same `src/generation/emit.rs` value-block return path; predicate emit + branch-type unification.  Per `cluster-IV-heap-typed.md` + `cluster-VII-chained-call.md` | Sets E + G PASS on both backends; Set H still green; `make ci` green; moros_* native suite green | 2-3 days | LOW |
