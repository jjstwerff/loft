...
| IV-Vec ❌ out of PLAN52 scope | `Vector` value-blocks via `??` — investigation 2026-05-29 cycle 4 found probes 21/36 test a **pre-existing vec-of-vec lookup bug** (`a = o.lists[0]` where `o.lists: vector<vector<X>>` returns `len(a)=0` even WITHOUT `??` on both backends).  Not a value-block-cleanup issue.  Track as separate P-issue (vec-of-vec indexing); reclassify out of PLAN52. | Vec-of-vec indexing, not `??` | BOTH backends — pre-existing | 21, 36 | _(separate bug; out of PLAN52)_ |
...
| `21-vector-coalesce.loft` | `vec_of_vecs[i] ?? other_vec` — Vector value-block | **IV-Vec ❌ out of scope** | **FAIL** (vec-of-vec lookup pre-existing bug, not `??`) | **FAIL** runtime (same pre-existing bug) |
...
| `36-iter-over-vec-coalesce.loft` | `for x in (vec_of_vecs[i] ?? other)` — iter consumer | **IV-Vec (iter) ❌ out of scope** | **FAIL** count=1 want 3 (vec-of-vec lookup pre-existing) | **FAIL** runtime (same) |
