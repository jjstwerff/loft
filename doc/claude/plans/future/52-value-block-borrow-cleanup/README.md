...
| IV-Vec 🟡 | `Vector` value-blocks via `??` — interpret silently returns zero-valued elements (probe 21: tag=0; probe 36: iter runs 1× instead of 3×); native COMPILES after predicate fix (2026-05-29).  Vector doesn't go through the `OpReplaceKeyed` deep-copy path closed in 2026-05-29 dep-strip fix, so still FAILs at runtime — needs a separate fix targeting `OpReplaceVector` (or the equivalent vector deep-copy path). | Silent corruption (interpret); native runtime FAIL | BOTH backends fail | 21, 36 | [`cluster-IV-heap-typed.md`](cluster-IV-heap-typed.md) (open: vector path) |
...
| `21-vector-coalesce.loft` | `vec_of_vecs[i] ?? other_vec` — Vector value-block | **IV-Vec 🟡** | **FAIL** `a[0].tag = 0` | **FAIL** runtime (was E0308 — predicate fix 2026-05-29; vector deep-copy path remains open) |
