...
| IV-Vec 🟡 | `Vector` value-blocks via `??` — interpret silently returns zero-valued elements (probe 21: tag=0; probe 36: iter runs 1× instead of 3×); native now COMPILES after predicate fix (2026-05-29) but runtime-panics in `OpReplaceKeyed` because `var_a` is initialised to the null-DbRef sentinel.  **Compile gate closed; runtime gate remains.** | Silent corruption (interpret); native runtime panic (was compile error) | BOTH backends fail | 21, 36 | [`cluster-IV-heap-typed.md`](cluster-IV-heap-typed.md) (predicate fix iteration 1 logged) |
| IV-Hash 🟡 | `Hash` value-blocks via `??` — interpret null collection; native compiles after fix, runtime-panics same as IV-Vec. | BOTH fail | BOTH backends | 22 | _(bundle with IV-Vec)_ |
| IV-Sorted 🟡 | `Sorted` value-blocks via `??` — same shape; interpret null; native compiles, runtime panic. | BOTH fail | BOTH backends | 41 | _(bundle with IV-Vec)_ |
| IV-Index 🟡 | `Index` value-blocks via `??` — same shape; interpret null; native compiles, runtime panic. | BOTH fail | BOTH backends | 50 | _(bundle with IV-Vec)_ |
...
| IV-Tuple ❌ | `Tuple` value-blocks via `??` — interpret returns fallback; native E0308.  **Predicate fix does NOT cover Tuple** (tuples aren't heap-DbRef typed; need a Tuple-specific sentinel). | BOTH fail | BOTH backends | 40 | _(bundle with IV-Vec; separate sentinel)_ |
...
| `21-vector-coalesce.loft` | `vec_of_vecs[i] ?? other_vec` — Vector value-block | **IV-Vec 🟡** | **FAIL** `a[0].tag = 0` | **PANIC** runtime (was E0308 — predicate fix landed 2026-05-29) |
| `22-hash-value-coalesce.loft` | `wrapper.maps ?? other_hash` — Hash value-block | **IV-Hash 🟡** | **FAIL** `e.value = null` | **PANIC** runtime (was E0308) |
...
| `36-iter-over-vec-coalesce.loft` | `for x in (vec_of_vecs[i] ?? other)` — iter consumer | **IV-Vec (iter) 🟡** | **FAIL** count=1 want 3 | **PANIC** runtime (was E0308 — predicate fix 2026-05-29) |
...
| `40-tuple-value-coalesce.loft` | `vec_of_tuples[i] ?? other_tuple` | **IV-Tuple ❌** | **FAIL** returned fallback | **FAIL** E0308 (not covered by 2026-05-29 predicate fix — needs Tuple sentinel) |
| `41-sorted-coalesce.loft` | `wrapper.sorted_field ?? other_sorted` | **IV-Sorted 🟡** | **FAIL** `null` | **PANIC** runtime (was E0308 — predicate fix 2026-05-29) |
...
| `50-index-coalesce.loft` | `index<Entry[name]>` value-block | **IV-Index 🟡** | **FAIL** `null` | **PANIC** runtime (was E0308 — predicate fix 2026-05-29) |
...
| **2** | **Fix cluster IV + VII** — same `src/generation/emit.rs` value-block return path; predicate emit + branch-type unification.  Per `cluster-IV-heap-typed.md` + `cluster-VII-chained-call.md`.  **2026-05-29 iteration 1**: predicate fix landed (`output_test_predicate` helper) — closes IV-Enum (probe 23) on native; IV-Vec/Hash/Sorted/Index now compile but expose secondary `var_a` null-sentinel init bug at runtime; IV-Tuple and Cluster VII (text-branch unification) NOT covered — both need separate iterations. | Sets E + G PASS on both backends; Set H still green; `make ci` green; moros_* native suite green | 2-3 days | LOW |
