...
| D — Implementation | 🟢 cluster II shipped (`ff8b0730`); **cluster I foundation landed (`00dc10ae`)** — `store_confinement` map extracted + the **rc-misdiagnosis RESOLVED** (the coupling is the `__vdb` *scope registration*, not rc/relocation; rc=1, relocation failed only on `compute_intervals` *timing*).  The fix — a **gated two-phase scan** that re-registers the confined `__vdb` at its block scope — is **mapped, not yet implemented**.  Then drive `LOFT_STORE_GUARD` silent corpus-wide + promote to a `debug_assertions` assert.  Cluster III = a separate `scopes.rs` dead-store free at the overwrite. |
