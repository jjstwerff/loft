...
| D — Implementation | 🟢 cluster II shipped (`ff8b0730`); **cluster I-a (block-confined) SHIPPED + verified (`3ec7d94c`)** — `relocate_null_init` block-scopes the confined `__vdb`'s slot (the crux was the **body-0 null-init hoist**, not rc/timing).  Verified general across every single-store / distinct-variable shape (flat `if` 7→3, nested 8→4, distinct `if/else` & `match` →3, loops 3); `Stores::peak` watermark field added; `172` green both backends; full suite 1921✅.  **Open:** I-b function-level sequential (last-use freeing) + the **shared-variable-across-blocks residual = cluster III** (single-valued dep drops the overwritten store's owner).  **Last-use freeing in progress** — plan [`fix-design-last-use-freeing.md`](fix-design-last-use-freeing.md). Phase-1 diagnostic (`store_liveness_walk`/`last_use_guard`) DONE + cross-validated (`eaed473b`). Phase-2 spike (`bb237b7a`, gated `LASTUSE_FREE`) **refuted the def-point-free thesis**: the watermark is **locked at body-0** — the `__vdb` null-init hoist allocates all stores up front (reversed order via `work_refs` BTreeSet + `insert(0)`), and monotonic `peak`/top-slot-only `max`-shrink mean free-insertion alone can't lower it. **Real lever = relocate the null-init (the allocation), the I-a mechanism extended.** **Next:** Phase-2.5 tag/verify gate (store-identity `debug_assert` — safety net), then Phase-3 relocate+free. `confine_reassign_safe` + `multi_store` are the inert shared-block (cluster-III Route 2) foundation. |
