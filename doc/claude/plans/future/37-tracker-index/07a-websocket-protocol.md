...
| **Shared mmap** | Same machine works, but doesn't generalize to the multi-project / remote-VM-via-port-forward case (@PLAN37 phase 08).  WebSocket forwards through a single SSH `-L` cleanly. |
