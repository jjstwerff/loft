...
- **One file format.**  Either MapFile JSON (today) or the eventual
  path-backed `Store` (`store_persist_bind` from
...
| Smoke trails | Ring buffer of recent positions per plane (~90 entries at 30 Hz / 3 sec); ribbon mesh from quad strip.  Same data drives projector overview + each phone's first-person view | New; pure GL work atop `lib/graphics` (server) + WebGL (phone) |
...
- [@PLAN46 dryopea](https://github.com/jjstwerff/dryopea)
  — editor that authors the world; plan 06 stencil pipeline gives
  us authored prop libraries when it lands.
