...
| `net.bounce_keyframe_priority` | true | bool | flag | When true (default), bounce events bypass rate-LOD entirely: every in-sight recipient gets a POSE for the bouncing plane immediately on the bounce instant, regardless of their normal-pose-LOD band.  Cost is bounded by the event rate (~few/sec/plane); benefit is crisp bounce rendering at all distances ([interp_test.loft](../../../../tools/audience-demo-50/interp_test.loft) shows no interp method alone can recover a missed bounce; the keyframe provides the high-curvature sample that interpolation needs).  Off by default would re-introduce the 0.3–1.0 m visual error on missed bounces at outer-band rates |
...
| `view.interp_method` | "hermite" | linear / hermite | enum | Pose-frame interpolation method.  **Hermite** (cubic with end velocities) is the default — [`tools/audience-demo-50/interp_test.loft`](../../../../tools/audience-demo-50/interp_test.loft) shows it eliminates the 67 mm peak position error that linear interp produces on a constant turn at 7.5 Hz outer-band rate (Hermite cuts it to 0 — circular arcs are exactly reconstructed when endpoint tangent velocities are used).  Cost is a few extra multiplies per peer per render frame, trivial.  "linear" is retained as a fallback for low-power phone targets that can't afford the cubic |
