...
OSS-Fuzz row), which keeps the **direct** structure-fuzz targets
(`store_alloc`, `vector_collection`) and the rest of Wave 2 (macOS-ARM leg,
TSan, `LOFT_POISON` keystone, Miri-set growth, LSan triage, native-ASan).
...
| **F4** | **`LOFT_POISON` pairing** — run the fuzzers with @PLAN53's arena poison-on-free active so store-internal use-after-free (the @P377/@P378 dangling-`DbRef` family) fails loudly under the fuzzer.  **Depends on @PLAN53 `LOFT_POISON` keystone (W2-3) landing first.** | Poisoning active under the fuzz targets; any @P377/@P378-class read produces a sentinel-value panic rather than silent stale data. | Blocked on @PLAN53 W2-3 |
...
4. **F4 after @PLAN53 W2-3** — the poisoning keystone is a hard dependency.
...
- **@PLAN53 Wave 2** — owns the `fuzz/` crate, the direct structure-fuzz
  targets, the `LOFT_POISON` keystone (F4's hard dependency), the macOS-ARM
  leg, TSan, and the rest of Wave 2.  This plan adds program-level targets to
  the same crate; coordinate to avoid target-name collisions.
- **@PLAN53 `LOFT_POISON` (W2-3)** — F4 depends directly on this landing.

## See also

- [`plans/future/53-sanitizer-ci-lever/`](../53-sanitizer-ci-lever/README.md) — parent plan; Wave 2 items #4, #6, and OSS-Fuzz spun off here.
