...
**Status: SHIPPED (closure record).**  Variable persistence + the REPL.X
value-snapshot interim + own-format serializer all landed (2026-06-08); the
re-run *cost* and exact resume are the store-resident successor — @PLN14
([§ Convergence, moved](#convergence--moved-to-pln14)).  This file is kept as the
build narrative for what shipped.
...
## Convergence — moved to @PLN14

The convergence design (REPL.X, auto-resume, and persistence are one
store-resident fix — with *why not mmap the stack*, *why the stores DO mmap*, the
RNG decision, and the four remaining needs) was the live design source for the
store-resident successor.  On @PLN12's close it **moved to its build plan**:
[`plans/14-store-resident-repl-session/CONVERGENCE.md`](../14-store-resident-repl-session/CONVERGENCE.md).

What stays here is the **closure record**: the REPL.X value-snapshot interim and
the own-format serializer (above) are the band-aid the convergence design
replaces; @PLN14 builds the real fix.
