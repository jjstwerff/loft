...
| ID | Description |
|----|-------------|
| **REPL.X** | ✅ **SHIPPED — scalar + text first cut (2026-06-08)** via **value-snapshot** (not the preserved-frame model).  A binding's RHS runs **once** at binding time; its value is captured and the binding is stored as a literal (`name = 42`), so re-running `body` on later observes is side-effect-free.  Capture builds `fn cap() -> <T> { <body> <rhs> }` and reads the return off the stack top (where `execute_at` reads its return — no new execution entry point).  `integer`+`text` captured directly; **struct/vector/enum fall back to storing the RHS as source** (re-run, harmless for pure constructions — the residual).  Built on the own-format serializer (steps 1–3) + parser alignment.  `ReplSession::capture_binding`; `side_effecting_binding_runs_once` / `…_text_…` / `struct_binding_falls_back_to_source` in `tests/repl.rs`.  **Deferred:** full-type capture (struct/vector/enum) via reading the `DbRef` return + `show_loft` (serializer already built); and the re-run *cost* (a long session still re-runs a literal body) — that stays the store-resident model ([§ Convergence](03-state-reset-and-append.md)). |
