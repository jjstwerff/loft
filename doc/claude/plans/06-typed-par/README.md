...
| 4 | [04-typed-input-output.md](04-typed-input-output.md) | **partial** | M | Typed surface: `parallel_for(input: vector<T>, fn: fn(T) -> U, threads: integer) -> vector<U>` — `element_size` and `return_size` retire; the type system carries them.  Sub-phase 4c landed: `Stitch::Concat` (no payload) replaces `ConcatLegacy`; `parallel_execute_and_collect` routes via the caller-supplied `DispatchMode`.  `grep ConcatLegacy src/` returns zero matches (spec acceptance).  Sub-phases 4a (typed-arity declaration), 4b (5-arg-form retirement), 4d (flat-vector-only input fix), 4e (caller-supplied destination via ref_return) remain. |
