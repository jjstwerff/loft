...
**Status:** Open (depends on phase 02 + `lib_plans/63-lsp/` LSP.1)
...
| Risk | Mitigation |
|---|---|
| loft-lsp from @PLN63 isn't ready | Phase 03 depends on @PLN63 LSP.1 explicitly.  If @PLN66 phases 00-02 ship before @PLN63 LSP.1, phase 03 waits. |
| loft-lsp positions / URIs differ from LSP spec | Translator module in the bridge handles divergences.  Coordinate with @PLN63 to keep loft-lsp spec-compliant. |
...
- [`lib_plans/63-lsp/`](../63-lsp/README.md) — the
