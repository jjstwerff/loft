...
[`lib_plans/63-lsp/`](../63-lsp/README.md) once that ships.
...
3. **First-class loft tooling means treating loft files
   ...
   (`lib_plans/63-lsp/`) builds the loft-lsp SERVER —
...
3. **Reusable across loft tooling.**  The same
   ...
   future browser IDE (`lib_plans/62-web-ide/`),
...
| 3 | [loft-lsp integration](03-loft-lsp.md) | M | Once `lib_plans/63-lsp/` LSP.1 ships, bridge spawns `loft-lsp` for `.loft` files.  Same hover / def / refs UX as rust-analyzer.  First-class loft treatment. | Open (depends on @PLN63 LSP.1) |
...
- Same UX works on `.loft` files (via loft-lsp from @PLN63)
...
- [`lib_plans/63-lsp/README.md`](../63-lsp/README.md)
...
- [`plans/42-tracker-index/README.md`](../../plans/42-tracker-index/README.md)
...
- [`lib_plans/62-web-ide/README.md`](../62-web-ide/README.md)
  — future browser IDE; reuses `loft-lsp-bridge` as its
  language-intelligence layer.
- [`plans/36-developer-experience/README.md`](../../plans/36-developer-experience/README.md)
  — DX umbrella; the viewer + LSP is one of the largest DX
  wins on the roadmap.

## Why this is a separate plan from @PLN63

Plan-09 (`lib_plans/63-lsp/`) builds the loft-lsp
...
They COMPOSE in @PLN66 phase 03: once both are live, `.loft`
...
"loft language intelligence"; @PLN66 is about "multi-language
