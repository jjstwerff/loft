...
(symbol defaulted), and registration derives from the function
definitions.  `server` + `imaging` are converted; `web` + `graphics`
(and the already-published `loft-libs-core` / `loft-libs-net` external
repos, which still carry manifests) convert as they're touched.
