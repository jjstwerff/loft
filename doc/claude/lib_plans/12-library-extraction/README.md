...
| 5b | `loft-libs-graphics` Stage B — extract `graphics` + `imaging` Stage A first (the remaining halves of chunk 5); then remove monorepo `lib/shapes/` + `lib/gridmesh/` + `lib/graphics/` + `lib/imaging/`; migrate consumers (`lib/moros_*` + `lib/audience_crystal` + `lib/graphics/examples/25-brick-buster.loft`) to registry-version deps | 5 + 6.6 (auto-install for the `tools/` consumers) + 6.12 (fixture pattern for loft-dev tests) | **OPEN — M (~3-5 days)** — split out 2026-05-31 to make Stage B execution explicit; see [ci-and-warnings.md § Stage B execution per chunk](ci-and-warnings.md#stage-b-execution-per-chunk-proposed-2026-05-31) |
...
| 6b | `loft-libs-net` Stage B — remove monorepo `lib/web/` + `lib/server/` + `lib/game_protocol/`; migrate `tools/audience-demo*` + `tools/viewer` + `lib/audience_crystal` to registry-version deps (audience_crystal has a `loft.toml`; the `tools/` scripts rely on 6.6 auto-install) | 6 + 6.6 + 6.12 | **OPEN — S-M (~2-3 days)** — split out 2026-05-31; net Stage B is simpler than graphics 5b because there's no Phase-5-pending Stage A work; see [ci-and-warnings.md § Stage B execution per chunk](ci-and-warnings.md#stage-b-execution-per-chunk-proposed-2026-05-31) |
