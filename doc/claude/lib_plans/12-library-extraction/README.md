...
| 6b | `loft-libs-net` Stage B — remove monorepo `lib/web/` + `lib/server/` + `lib/game_protocol/`; migrate `tools/audience-demo*` + `tools/viewer` + `lib/audience_crystal` to registry-version deps (audience_crystal has a `loft.toml`; the `tools/` scripts rely on 6.6 auto-install) | 6 + 6.6 + 6.12 + **NEW**: native cdylib build from `~/.loft/registry/<pkg>/native/` must be reliable (issues found 2026-05-31, see below) | **BLOCKED — attempted 2026-05-31, rolled back**.  Move-then-delete refactor surfaced two friction points building chunk-resident native cdylibs from `~/.loft/registry/<pkg>-<ver>/native/`: (1) `cargo build` writes to `<install_dir>/native/target/` which is restricted on shared / sandboxed machines + (2) building `web`'s native crate from the registry copy triggers a `ring` C-build path issue with macOS SDK lookup (`TargetConditionals.h` not found from the env cargo gets handed when invoked from this path).  Neither fires for the in-monorepo `lib/<pkg>/native/` because cargo's existing workspace context handles them.  Real blocker for 6b's "happens to work in CI" promise — the native auto-build flow from registry installs needs hardening before 6b can safely remove the monorepo crates.  Move-then-delete scaffolding (Rust harness path updates, `tests/integration/` lockfile) is sound but rolled back to keep the tree green pending the native-build fix |
