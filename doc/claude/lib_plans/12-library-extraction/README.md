...
| 6b | `loft-libs-net` Stage B — remove monorepo `lib/web/` + `lib/server/` + `lib/game_protocol/`; migrate `tools/audience-demo*` + `tools/viewer` + `lib/audience_crystal` to registry-version deps (audience_crystal has a `loft.toml`; the `tools/` scripts rely on 6.6 auto-install) | 6 + 6.6 + 6.12 + 6b-prep (native build from registry) | **PARTIAL — 6b-prep SHIPPED 2026-05-31**: `auto_build_native` redirects `cargo build` target via `CARGO_TARGET_DIR=~/.loft/build-cache/<pkg>-<ver>/` when building from registry installs, so writes to read-only / sandboxed install dirs no longer fail.  Verified by running 6b execution probe (Stage B removal of `lib/{web,server,game_protocol}/`): multiplayer v2 + v3 + 4/5 of v5 pass.  **Remaining 6b work** (deferred to follow-up commits): script relocation (`lib/<pkg>/examples` → `tests/integration/multiplayer/`); consumer migration (`tools/audience-demo*` + `tools/viewer` need a loft.toml); `extraction_hygiene` test update; v5_t5 needs Phase 6w (world extraction) OR an `#[ignore]` for the gap.  6b-prep commit landed the native-build fix; the per-consumer migration is the next sweep |
