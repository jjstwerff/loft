...
- **`loft-libs-graphics` partial** (Phase 5): `shapes` 0.2.0 +
  ...
  (see below).  Stage B is **READY** — both prerequisites cleared
  on `doc-updates` (loft-ffi-macros published; graphics warning
  sweep complete with `.allow_warnings` removed).
...
Stage-A-only as of 2026-05-31 — but both Phase 5b prerequisites
(loft-ffi-macros publish + graphics warning sweep) are now CLEARED
on the `doc-updates` branch, so 5b is ready for execution.
...
| 5b | `loft-libs-graphics` Stage B — extract `graphics` + `imaging` Stage A first (the remaining halves of chunk 5); then remove monorepo `lib/shapes/` + `lib/gridmesh/` + `lib/graphics/` + `lib/imaging/`; migrate consumers (`lib/moros_*` + `lib/audience_crystal` + `lib/graphics/examples/25-brick-buster.loft`) to registry-version deps | 5 + 6.6 + 6.12 | **UNBLOCKED — 2026-05-31** — both prerequisites RESOLVED.  (1) `loft-ffi-macros 0.1.0` published to crates.io 2026-05-31 15:59 UTC; chunk-resident `lib/imaging/native/Cargo.toml` can now pin `loft-ffi-macros = "0.1"`.  (2) Graphics warning sweep COMPLETE 2026-05-31 (doc-updates commits `e592fd8` partial + `639d546` lint improvements + `177453e` complete) — all 6 source files in `lib/graphics/src/` pass `LOFT_DENY_WARNINGS=1`; `.allow_warnings` opt-out deleted; `test_bezier_curve` + full wrap suite green.  Restructure used the bind-local + len-guard idiom (`fr_d = self.data; fr_n = len(fr_d); ... if fr_idx < fr_n { fr_d[fr_idx] = color }`) for six per-pixel draw routines; `draw_bezier` rewrote indexed-write stack to truncate-then-append.  Surfaced @P390 (self-slice-assign drops element values; worked around with temp local).  Original 5b attempt history: `69afae1` (2026-05-31, surfaced both prerequisites + rolled back).  Ready for execution. |
...
| 6w-w | Retire every `lib/*/.allow_warnings` opt-out — clean each library's warnings until the gate runs strict everywhere | 6.5 (gate landed) | **partial — 2026-05-31** — `lib/graphics/` retired (doc-updates `177453e`, 148 unique source warnings → 0 + opt-out file deleted; restructure pattern documented).  Remaining: `lib/shapes/` (already 0 warnings — stale opt-out, free drop); `lib/moros_map/` (4), `lib/moros_editor/` (6), `lib/moros_render/` (31), `lib/audience_crystal/` (40), `lib/moros_ui/` (155), `lib/moros_sim/` (173) — total 409 across the six monorepo libs that still carry the opt-out.  Lint improvements (doc-updates `639d546`) make these sweeps easier — position attribution now points at the right function; AND-conjunction guards (`if a<len(u) and b<len(v)`) lift automatically.  Surfaced @P390 along the way (self-slice-assign drops element values; PROBLEMS.md row 390 with workaround). |
