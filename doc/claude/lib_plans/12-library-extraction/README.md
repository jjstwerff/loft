...
| 5b | `loft-libs-graphics` Stage B — extract `graphics` + `imaging` Stage A first (the remaining halves of chunk 5); then remove monorepo `lib/shapes/` + `lib/gridmesh/` + `lib/graphics/` + `lib/imaging/`; migrate consumers (`lib/moros_*` + `lib/audience_crystal` + `lib/graphics/examples/25-brick-buster.loft`) to registry-version deps | 5 + 6.6 + 6.12 + warning sweep for `graphics` (209 warnings, `.allow_warnings` opt-out) | **PARTIALLY UNBLOCKED — 2026-05-31** — prerequisite #1 (`loft-ffi-macros` on crates.io) RESOLVED: `loft-ffi-macros 0.1.0` published to crates.io 2026-05-31 15:59 UTC (~15min after PR #238 merge); chunk-resident `lib/imaging/native/Cargo.toml` can now pin `loft-ffi-macros = "0.1"`.  Prerequisite #2 still OPEN: `lib/graphics/` carries 209 warnings + `.allow_warnings` opt-out — shipping with the opt-out carries that debt into the chunk repo against the "every new chunk starts warning-free" Phase 6.5 discipline.  Remaining prep work: graphics warning sweep (Phase 6w-w slice for graphics) before 5b can execute.  Original attempt history: `69afae1` (2026-05-31, surfaced both prerequisites + rolled back). |
