...
|---|---|---|
| _(none active)_ | — | Promote the next-highest-priority plan from `future/` when you start library work. |
...
| [`future/19-gridmesh/`](future/19-gridmesh/) | `gridmesh` — shared toolkit of chunk-local, bounded-extent grid→mesh primitives (spatial-index neighbour queries, halo-aware gather, bounded mesh accumulation keyed by owning cell, dirty-region / per-chunk rebuild, parallel chunk build).  Extracted from the `audience_crystal` prototype; destined for `moros_render` (per-chunk world meshes with neighbour-pattern wall placement + edge rounding, replacing the global flat-quad `build_hex_meshes`).  Toolkit not framework — each consumer supplies its per-cell rule. | **Phase A landed 2026-05-21** (coord + spatial-index primitives extracted, `audience_crystal` + projector re-pointed, output/memory byte-identical both backends).  Phases B (chunking + halo + bounded extent + dirty rebuild + parallel chunk build) and C (moros consumer) open. |
