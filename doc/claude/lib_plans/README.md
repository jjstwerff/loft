...
| [`future/23-http-session-auth/`](future/23-http-session-auth/) | HTTP response headers + a cookie-jar session for `lib/web` — the concrete consumer-driven slice of the 06-web-services HTTP client.  Two parts: P1 expose response headers (`header`/`headers_for`, XS–S, additive — mirrors the existing `LAST_BODY` thread-local); P2 a cookie-jar `HttpSession` (get/post/put/delete + redirect control, M — mirrors the `ws_client` slot pattern, flips `ureq`'s `cookies` feature).  Unblocks **native Garmin login** (training port A2 — credential POST → MFA POST must carry `Set-Cookie`).  Related: P3 base64 stdlib helper (XS, optional); the `exec()` gap (moot — superseded by this workstream); TLS/JA3 impersonation E1 (deferred, conditional). | **P1 + P2 + P3 shipped 2026-05-27** (response headers + cookie-jar `HttpSession` + base64, all verified on interpret + `--native`; filed/fixed @P365 along the way).  Awaiting formal close to `finished/`.  Tail: TLS/JA3 impersonation E1 stays deferred (conditional). |
