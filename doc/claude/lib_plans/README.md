...
| [`future/11-packages/`](future/11-packages/) | Package management open follow-ups — pointer-plan tracking the items still to BUILD (not the format spec, which lives at PACKAGES.md as reference).  PKG.REG (central registry MVP — `loft install <name>` / `loft publish` / signing / verification, 0.8.6), PKG.7 (`loft.lock` for reproducible builds — small, in `manifest.rs`), PKG.EXTRACT (move `lib/*/` to per-family GitHub repos via PKG.REG — 1.1+, depends on PKG.REG). | Pointer-plan (each row links to the relevant PACKAGES.md section).  PACKAGES.md stays at doc root as reference for the SHIPPED package format (loft.toml, layout, function binding, build pipeline) — 6 `lib/*` packages already use the format.  Same shape as plans/future/33-native-codegen-followups/. |
