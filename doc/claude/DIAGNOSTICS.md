...
| `nullable-collection-field` | warning | A `?` on a COLLECTION field (`vector`/`hash`/`sorted`/`index`/`radix`/`trie`) promises an absence the storage cannot hold: the field is a record id whose zero already means empty, so `null` and `[]` write the same bytes and `f == null` is always false. A distinct marker would change the stored format of every existing collection field, so the declaration is where this is answerable until the `Some` layout exists (loft#917, shared with loft#938). Quiet on scalar and struct `?` fields, which read back as null correctly, and on a nullable ELEMENT (`vector<vector<T>?>`). | Test `len(f) == 0` for absence and clear with `[]`, or drop the `?`. | M M · `@F1` |
