...
| `module-name-shadowed` | advice | Two packages declare a module of the same file name, so this `use` binds the other package's file and this package's own is never loaded. | Write `use self::<module>`, which binds this package's own file under `<package>::<module>` and cannot be captured downstream. Outside a package there is nothing to qualify with, so there the cure is to rename one file and its `use`. | M · `@F16` |
