...
| `variant-field-unchecked` | warning | A struct-enum field access names a field only SOME variants declare. The access resolves at compile time to the first variant that has it and the tag is never consulted, so on any other variant it reads that variant's bytes at this offset and a write lands there with the tag left alone. Quiet when EVERY variant declares the field (one shared slot — the case C89 promises works) and for a synthetic `__nullable<S>`. | Reach the payload per-variant with `match` or `is`, whose bindings are scoped to the arm. | C C · `@F29` |
...
| `module-name-shadowed` | warning / advice | Two packages declare a module of the same file name, so this `use` binds the other package's file and this package's own is never loaded. **Warning** when the file that won belongs to the ROOT PROJECT and the one that lost to a DEPENDENCY: a published package then answers differently than it does on its own, which is a wrong result (loft#949). **Advice** the other way round — a package losing its own name to a file elsewhere in the graph — which is what the shipped `graphics` ⟷ `mesh3d` overlaps are. | Warning: rename the project's own `<module>.loft`, since a consumer cannot edit the dependency; the dependency's author ends it for everyone with `use self::<module>`. Advice: write `use self::<module>`, which binds this package's own file under `<package>::<module>` and cannot be captured downstream. Outside a package there is nothing to qualify with, so there the cure is to rename one file and its `use`. | M · `@F16` |
