...
| `integer` → `u8`/`u16`/`i8`/`i16`/`u32` | Explicit `as` at storage sites | NARROWING — a plain `integer` is 64-bit; writing one into a narrow **struct field** (or other narrow storage) requires `as` ("cannot implicitly narrow integer to u16 … cast explicitly").  A **constant that provably fits** the target is exempt (`x: u16 = 5`, `f(200)`).  The check is range containment, so it covers every alias declared with a `limit(…)`.  (@PLAN48 / @P370) |
| `integer` → `i32`                  | **Unchecked — loft#931** | `i32` is declared `integer size(4)` with no `limit(…)`, so it inherits the full integer's bounds and the range test above cannot see the narrowing.  An `i32` **struct field silently truncates** (`5000000000` reads back `705032704`, both backends); an `i32` local or parameter is an inert annotation that keeps all 64 bits.  Arming the check needs `as i32` to retype first — today it is a width tag that leaves the value `integer`, so there would be no spelling that satisfies the gate.  Until then, keep a value you need narrowed in an aliased type that carries its own `limit(…)` |
