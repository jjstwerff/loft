...
|---|---|---|---|---|
| 1 | **Pass-2 arity-growth CASCADE — Reference returns DONE 2026-06-12**; the residual is the **Vector/struct-Enum arm extension**. The one-buffer design (arity = signature+1 fixed at declaration; every return path delivers in THE `__retbuf` — outermost-ref chain / named-local copy) is BUILT for `Type::Reference`: all four crash cells (forward caller, forward chain, self-recursion, mutual recursion) fixed and un-ignored, 10-cell matrix green on both backends, armed corpus + full suite green. Remaining, filed en route (all pre-existing): **#355** — the Vector arm still grows per-site and a forward caller of a multi-site vector fn silently returns the WRONG element (extend the same binding to Vector/struct-Enum — same site, same mechanism); **#356** — mid-body `return f(g(x))` returns the null sentinel on native (the scopes `Return(Null)` fall-through); fn-refs to struct-returning fns cannot be CALLED (CallRef never fills hidden buffer args). Verify the @PLAN59 growth assert on two lib fns fell out (DEPS_INVENTORY § Status residual). | S–M residual | ◐ Reference done | [STABILITY_SWEEP § THE FIND](STABILITY_SWEEP.md) (design round + build record); `tests/issues.rs::pass2_arity_growth_*` (live guards); [#355](https://github.com/loft-lang/loft/issues/355), [#356](https://github.com/loft-lang/loft/issues/356) |
...
| 3 | **Armed-corpus residuals ×4 — TRIAGED 2026-06-14** (branch `stability2nd`); the channel's silence is now trustworthy (each failure explained): **`132` FIXED** (was a SILENT interpreter UAF — field-of-element set of a struct literal with a vector freed the live backing via an unguarded reassignment `OpFreeRef`; routed through `OpFreeRefIfDistinct`; regression `tests/scripts/372-…`). **`166`** VERIFIED over-strict armed assert (@P390 text self-slice materialise self-ref — runtime correct on both backends under stress). **`75-native-stub`** now clean on the rebuilt armed binary (expected-fail by design). **`collections.loft`** had TWO bugs, BOTH FIXED 2026-06-14: **bug 1 (empty `{}` keyed-literal → Void block under-filling the struct)** — `parse_object_field` accepts `{}` as the primed empty collection + warns toward `[]` (regression `tests/scripts/373`); **bug 2 (duplicate hash key in a vector+hash sibling pair corrupted the vector via the `other_indexes` secondary-index linking)** — `dedup_keyed` gains a `secondary` flag so a sibling index only UNLINKS the stale mapping, never deletes the record the primary vector owns (the @P305 update-only path for `other_indexes`; regression `tests/scripts/374`). | S each | ✅ all four armed residuals resolved (132 + collections fixed; 166 over-strict; 75 clean) | [STABILITY_SWEEP residual table + 2026-06-14 triage](STABILITY_SWEEP.md); armed build cmd in the same section |
