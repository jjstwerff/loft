...
| # | Item | Size | Status |
|---|---|---|---|
| **F** | **Cluster F — the deferred-marker walk: DONE.** `rewrite_generic_type_defaults` recursed through its own arm list (10 of 17 child-bearing `Value` variants); a marker in a `Tuple` shipped to the monomorph as the placeholder — silent at `T = integer`, SIGSEGV in `OpFreeText` at `T = text`, E0308 on `--native`. Now delegates to `Value::for_each_child_mut`. Proven under both gates: a 9-function carrier corpus byte-identical, and a carrier x position x type matrix correct on both backends. **The predicate half is PART done and the doc's claim was corrected by measuring:** only `type_mentions_tv` was a real duplicate (folded onto `Type::contains_def`); `is_type_var_operand`'s `base()` peel is correct by design, and the collapse does NOT fix `-> (T?, integer)`. **Still open:** `__nullable<T>` is not re-minted for a tuple ELEMENT (the refusal), and a tuple element's native emitter flag conflates two facts, so `T = text` in a tuple still will not compile on `--native` (E0716). | S+S done; 2 residuals | ▶ partial — walk landed (41befa0f, 4733f5e6) |
