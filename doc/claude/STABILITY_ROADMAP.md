...
| # | Item | Size | Status |
|---|---|---|---|
| **F** | **Cluster F — "is this type still a type variable?"** (NEW 2026-08-20). One recursive question over `Type`, answered by three predicates at three different depths, none recursing through the `Type::for_each_child` keystone that already exists. The deepest (`type_mentions_tv`) ends `_ => false` and misses `Iterator`/`Function`/`RefVar`/`Rewritten`; the middle (`is_type_var_operand`) peels only `Optional`. That is the shape behind loft#1014/#1016/#1020/#1023/#1025/#1028, each cured per-site with its own `TV_*` marker — one marker per DECISION, so the next type-keyed decision is the next defect. **Step 1:** add `Type::mentions_type_var` on the keystone and collapse all three spellings onto it. **Step 2 (independent):** `rewrite_generic_type_defaults` is a walk that must be TOTAL but descends 10 of 17 child-bearing `Value` variants and ends `other => other` — convert it to the extraction/recursion split, the same fix loft#815 got. Live both-backend repro (`-> (T?, integer)` refused) is evidence the duplicate is load-bearing, deliberately NOT filed. | S (step 1) + S (step 2) | ⬜ open — next up |
