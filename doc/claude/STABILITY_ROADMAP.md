...
| # | Item | Size | Status |
|---|---|---|---|
| **F** | **Cluster F — the deferred-marker walk and the nullable minting: DONE.** `rewrite_generic_type_defaults` recursed through its own arm list (10 of 17 child-bearing `Value` variants), so a marker in a `Tuple` shipped as the placeholder — silent at `T = integer`, SIGSEGV in `OpFreeText` at `T = text`, E0308 on `--native`; it now delegates to `Value::for_each_child_mut`. `type_mentions_tv` folded onto `Type::contains_def`. And the synthetic `__nullable<S>` was being minted for a template's `T` (an attribute-less placeholder struct that passed every eligibility test), which refused `-> (T?, integer)` on both backends; the two spellings of that eligibility now read one `synth_nullable_target`. Each proven under both gates — byte-identical corpora for untouched paths, tuple-vs-bare-twin matrices for the changed ones. **Doc claims corrected by measuring:** `is_type_var_operand`'s shallow peel is correct by design, and the predicate collapse did NOT fix the tuple refusal. **One residual:** `T = text` in a tuple still fails `--native` — a tuple element's emitter flag conflates two facts (append `.to_string()` to a literal / do not borrow a `TupleGet`); a scoped fix was tried and REVERTED because it re-breaks loft#1004. | S x3 done; 1 residual | ▶ residual only (41befa0f, 4733f5e6, ebb32a31) |
