...
> **▶ NEXT: step 3 (H6, one null-sentinel table).** Step 1 (H5) is ✅ done; step 2
> (plan-53 cluster 2) is parked WIP on a separate branch. H6 is the next ready,
> self-contained S–M item ("do in any gap"). Steps 6–8 (H3 / H8 / H4) are the L/M
> design rounds that hold the real forward-risk leverage; each opens with the
> design-protocol.
...
| 3 | **H6 — one null-sentinel table** — `sentinel(tp) -> Encoding` in `src/data.rs` next to `byte_width`; fill.rs templates, `default_native_value`, `emit_typed_null`, narrow-vector paths consume it. Absorbs the F3 deferred cell (dual DbRef null encoding — sentinel vs zero-default — as latent byte-level risk). Independent; "do in any gap". **Matrix gathered 2026-06-16** (two-axis model + verified stored-encode table in [HOTSPOTS § H6 design notes](STABILITY_HOTSPOTS.md)); surfaced a `set_byte`/`get_byte` null-decode asymmetry to settle matrix-first BEFORE building the table. Build not started. | S–M | ◐ design gathered | [STABILITY_HOTSPOTS § H6](STABILITY_HOTSPOTS.md); F3 row in [STABILITY_SWEEP](STABILITY_SWEEP.md) |
