...
|---|---|---|---|---|
| 1 | **H5 leftover asserts** — the two-pass contract asserted where it's cheap. **Attr-COUNT-per-def-equal-across-passes LANDED** (961e6c27, `assert_pass2_def_attr_stable`; post-arity-cascade an invariant, now enforced end-to-end not just at the `ref_return` growth site; silent across the 270-script debug corpus). Residual: **work-ref counter equality per fn** (the synthetic-name-stability half — marginal; the attr-count assert already covers the load-bearing divergence). | S | ◐ attr-count landed | [STABILITY_HOTSPOTS § H5](STABILITY_HOTSPOTS.md) |
