...
> **▶ QUEUE DRAINED 2026-06-17.** Steps 1 (H5), 3 (H6), 4 (H7-short), 6 (H3 —
> resolved by design-protocol: premise over-stated, facts already carried),
> 8 (H4-medium GET↔SET), and 10 (de-dup tail — triaged clean) are ✅ done/resolved.
> What's left is NOT active hardening that should be ground out now: **step 5
> (H7-long F9 macro)** is lower-priority (the two codec corpus guards now cover the
> field-gap risk for corpus-used variants; the macro only adds build-time cover of
> corpus-UNUSED variants); **step 7 (H8)** is deferred behind its own trigger (next
> `par` feature work); **step 9** is future plan slots (@PLN53/@PLN54); **step 2**
> is parked WIP on a separate branch.  Each remaining item opens with the
> design-protocol in a quiet window — none is a rush-now item.
...
| 7 | **H8 — the `Stores.allocations` privacy pass** — design the accessor surface (worker-slot claim/release, swap-back, lock APIs) as ONE batch with THREADING.md in hand, then convert the 60+ direct touches mechanically. Absorbs the STABILITY_PASS2 deferred accessor rows (`types[].parts` reads, Definition field reads). **DEFERRED with a concrete trigger (2026-06-17): "MUST precede any new `par` feature work" is its own gate — no `par` feature work is in flight, the raw touches are a forward-risk not a live bug, and the load-bearing part is the parallel worker-slot-swap API (memory-safety), which must be designed in a quiet window with THREADING.md, not rushed.** Re-open when `par` work resumes (the parallel-agent track). | M–L | ⬜ deferred (trigger: next `par` work) | [STABILITY_HOTSPOTS § H8](STABILITY_HOTSPOTS.md); deferred rows in [STABILITY_PASS2 § Work list](STABILITY_PASS2.md) |
