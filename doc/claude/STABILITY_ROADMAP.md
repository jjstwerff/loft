...
|---|---|---|---|---|
| 1 | **Pass-2 arity-growth CASCADE** — the only known LIVE release crash in the queue: a forward caller of a multi-return-site fn halts codegen ("Too few parameters"), both backends; a caller's single tail call materialises one work ref per hidden callee buffer, so no pass-1 syntactic count bounds a forward chain. Needs a DESIGN ROUND: between-pass buffer-count fixpoint vs caller-local backing for non-chained work refs. A per-site pass-1 pre-provision was tried and REVERTED. Likely subsumes the @PLAN59 growth assert on two lib fns (DEPS_INVENTORY § Status residual) — verify they fall out. | M–L | ⬜ next | [STABILITY_SWEEP § THE FIND](STABILITY_SWEEP.md); `tests/issues.rs::pass2_arity_growth_forward_{caller,chain}` (`#[ignore]`); repros `/tmp/p_followups/p14_forward_caller.loft`, `p15_three_level.loft`; armed corpus file `298` |
