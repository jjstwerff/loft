...
| # | Item | Size | Status |
|---|---|---|---|
| **F** | **Cluster F — five collapses landed.** The deferred-marker walk now delegates to `Value::for_each_child_mut` (it enumerated 10 of 17 child-bearing variants, so a marker in a `Tuple` shipped as the placeholder — silent at `integer`, SIGSEGV at `text`, E0308 on native). `type_mentions_tv` folded onto `Type::contains_def`. The synthetic `__nullable<S>` is no longer minted for a template's `T`. The tuple emitter's owned-text flag is split from the literal that passes through it. `tuple_has_text_leaf` peels `Optional`, and the return path's inline copy reads it — which fixed a PLAIN `-> (text?, integer)` that never compiled on `--native`. Each proven under both gates: byte-identical corpora for untouched paths (six references), twin-compared matrices for the changed ones. **Residual:** a discharged `a?` in a TEXT tuple answers the null sentinel on the interpreter and the empty text on native — the backends disagree, and both render as nothing, so it needs `len()`/`== \"\"` to see. A cell for it was written and deliberately NOT committed rather than freeze one backend's bug; 1016 carries the measurement. **The residual is INVESTIGATED, not just noted** (0691438f): four explanations are falsified by measurement — element aliasing, the `?` discharge itself, `ref_return` promotion, and the `text["a"]` element dep — and the boundary needs four axes together, so any probe moving one reads as a pass. What survives is "no block-result dep + a `__ref_1` retbuf + an element that is not a place", and `__ref_1` comes from the `__retbuf` machinery, i.e. row A's territory. Read STABILITY_REDFLAGS § Cluster F before probing. | 5 collapses done; 1 residual | ▶ residual only (41befa0f, 4733f5e6, ebb32a31, ad961977, 0691438f) |
| **C-holdout** | **`validate_claims` arm set made exhaustive.** The keystone fold was correctly ruled out for it (row C — defensive walk, bounds-checks before dereferencing). Untouched by that decision: it ends `_ => {}`, so it silently walks past `Array`, `Index`, `Ordered`, `Radix`, `Trie` — five of the thirteen kinds `copy_claims` knows — and answers "no problems" for a container it never looked at. `store_verify` is a caller. Fix is the arm set, not the fold; an arm that deliberately does nothing is fine when it says so. | XS | ⬜ open |
