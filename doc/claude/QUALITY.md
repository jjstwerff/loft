...
| **@PLN130 F9 / D-bind-9** — a `&` reference is still copied on two of the three disturbances | [formal/binding.md § D-bind-9](formal/binding.md) | **Known-broken, decided, not yet built.** B-Ref-Reshape declines a REMOVAL under a live `&`; a RE-KEY (`c = &s[30]; c.key = 5`) and a container REASSIGNMENT (`c = &bx.inner; bx = …`) still silently downgrade the reference to a copy, both backends, each with a *"copied out of"* advice line. No design question remains — C79's 2026-08-05 widening (*decline what we cannot implement safely*) covers both, and each fix is named in the deviation entry: a filter change in `def_reshape_refusals` for the reassignment (the liveness walk already computes it), a refusal at `note_key_field_write` for the re-key. **S effort each.** |
| **@PLN130 F8** — `par` / coroutine paths for a view live across a container reassignment | [@PLN130 probe 37](plans/130-element-view-invalidation/probes/37-f8-boundary.loft) | **Unmeasured**, not known-broken, of the now-closed @PLN130.  Probe 37 moved container kind, chain depth, reassignment site and sibling route together (17 cells, both backends, all green); the concurrency paths were never added to it.  The fix is a compile-time dep strip in `scopes.rs` with no thread-local state, so there is no specific reason to expect divergence — but nobody has run it.  S effort: two more cells on an existing probe. |
| **@PLN130 Q6** — which uncovered copy families should be silenced rather than removed | [@PLN130 § Open questions (added by the guard)](plans/130-element-view-invalidation/README.md) | **Design question, framing settled.**  The owner's rule is recorded — an accept written at the SITE, never a blanket env exemption, so the decision stays reviewable — but which of the measured families qualify is not chosen.  Blocks nothing: an unaccepted copy is reported, which is the state the model requires. |
| **@PLN130** — the uncovered copy set | [@PLN130 § Probe gaps](plans/130-element-view-invalidation/README.md) | **Sized, not drained: 29 distinct sites over a 90-script sample, across four origins.**  A legitimate resting state under the plan's decision 3 (`Avoidable` copies may stand while STATED and tracked), and `LOFT_COPY_MANIFEST=1` is the standing instrument that keeps the *Unknown* bucket empty.  This is the largest known unfinished measurement of the plan, not a defect.  L effort. |
