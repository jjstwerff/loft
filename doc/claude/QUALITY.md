# QUALITY — Open Issues, Active Design  Re-measured again (re-measured after loft#1371 gave the `&` LOCAL link the lowering the `&` PARAMETER has, for a text and a vector source): optional `723 | 360 | 5 | 358`, unspan `409 | 385 | 24` (re-measured after loft#1386's value-position match refusal) (re-measured after loft#1382's statement-position flag, and on the tree that now also holds loft2's loft#1379/#1380 and loft#1381) (re-measured again after loft#1387's monomorph return-tail bind) (re-measured again after loft#1368's return-tail bind; and on the tree that holds BOTH streams, after picking loft2's loft#1378 fix in place of this branch's — neither side's number, as every join so far; and after loft#1372 closed D-bind-17: the `&τ?` decline came OUT, one bare `Optional` test with it, and the nine sites that asked a link's inner type bare now ask `base()`).  Re-measured again (re-measured after @PLN153 phase 4 batch 1 (the `&` lowering's source tests read through base())): optional `717 | 359 | 5 | 353`, unspan `402 | 378 | 24`.  Re-measured again (re-measured after @PLN153 phase 3c joined with the @FR-O-Witness walk): optional `716 | 358 | 5 | 353`, unspan `402 | 378 | 24`.s, Enhancement Plan
