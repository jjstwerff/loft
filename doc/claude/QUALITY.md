...
| Item | Section | Status |
|---|---|---|
| **Cluster III Route 2** — reassignment store-free across shared blocks | [plan-57](plans/2-vector-store-watermark/cluster-III-reassignment-pin.md) | **Benign watermark residual** of the now-closed plan-57 (exit-safe, below the `LOFT_STORES=warn` floor — not a correctness/leak bug).  A shared `z` reassigned across `else` blocks / `x` across `match` arms keeps the overwritten store pinned to scope exit → watermark O(reassignment SITES, not of executed arms).  **Re-measured 2026-08-21:** peak 6/8/12/20 at 2/4/8/16 sites, and the executed path does not matter — 16 sites read peak 20 whichever single arm runs.  Only `recover_backer` is gated (`LOFT_CONF_RECOVER`); `confine_reassign_safe` itself is live, load-bearing code, so "the foundation is inert" was too strong.  With the gate ON the peak is a flat 5 at every site count, values identical on both backends, and the **full suite passes (4232)** — the un-gate decision now has evidence from 2.2× the tests it was parked on in 2026-06.  Values under both settings are pinned by `tests/scripts/reassign-across-sibling-blocks.loft`.  M effort; "do nothing, it's benign" is still explicitly on the table, and un-gating remains an owner call. |
