...
| **@PLN85 cluster IV** — @PLN51 hidden-buffer-aliasing latent residuals | [@PLN85 README cluster IV](plans/85-store-lifetime-retirement/README.md) · [finished/51](plans/finished/51-hidden-buffer-aliasing/) | Re-probed at @PLN85 closure: ~11 raw @PLN51 probes still show **native-mostly latent leaks on edge shapes** (tuple-of-canvas, lambda/operator vector-return, capture-heap-return, mixed-lit-call).  **Pre-existing** (identical on `origin/main` — not a @PLN85 regression), below the graduated-corpus leak-gate; the standing instruments (leak-gate + ASan + Miri) cover the graduated class.  L effort; edge-shape, not shipped-consumer-reachable. |
