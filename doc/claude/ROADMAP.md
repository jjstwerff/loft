...
| C60    | **Hash iteration in ascending key order.** Split into 9 testable steps in CAVEATS.md: (1) `hash_records` stdlib fn; (2) `hash_sorted` single-field asc; (3) parser desugars `for e in h` → `for e in hash_sorted(h)`; (4) ship MVP; (5) loop attributes; (6) multi-field keys; (7) `-` descending; (8) filter clause; (9) `#remove` diagnostic. Step-by-step scopes sum to ~1 focused day | MH | CAVEATS.md |
