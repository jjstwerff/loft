...
| `LOFT_VERIFY_STACK=1` (@PLN154 phase 1) | **A frame slot nothing wrote, reported at the READ.** One tag byte per stack byte, carried by the stack `Store` itself: a write through `Store::addr_mut::<T>` tags the bytes it covers, a byte move (`copy_block`, `addr_span_mut`) carries the tags with the bytes, and a slot that leaves the live frame — a pop, a `reserve_frame`, any route that lowers the stack pointer — loses them. The check sits at `get_stack` / `get_var`, never at `Store::addr`, which is what the debugger and the frame renderer read stale slots with on purpose.  **Tag low, check high.**  Reports only the ABSENCE of a tag; a read WIDER than the value written at the slot is counted, not reported, because an eval slot is stepped to eight bytes while a `boolean` writes one — that is phase 2's question. | **The definite-assignment class `LOFT_POISON` was built for, without needing the slot to hold a distinguishable byte pattern.**  Reach for it when a value appears from nowhere, when a result differs between RUNS, or when a free refuses on a frame word.  Calibrated both ways: silent on all 1103 runnable corpus programs at HEAD, four distinct sites on `64437246` (the nullable-local pre-init control).  Its yield is narrow by construction — a stack slot is nearly always recycled with something in it, so loft#1386's control is silent under it and loft#1254's only moves the wider-read counter.  `--interpret` only. |
