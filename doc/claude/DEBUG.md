...
| `scripts/probe-matrix` — boundary-matrix scaffolder | S | 2026-06-12 vector-ABI session (the 93-vsort leak): the 18-cell matrix was built by hand-written bash heredocs; the FIRST version was vacuous (all 24 cells parse errors read as "clean") and no cell had a hand-computed expected value, so HEAD/main agreeing on `acc=39` (true value 12) passed as green | Scaffold a probe dir from axis lists (one `.loft` cell per combination), run every cell on {working binary, baseline worktree binary} × {`--interpret`, `--native`}, and emit one table: cell / expected / per-binary actual / leak-warning. Two validity guards BAKED IN, not optional: (a) error when a cell's `expected` field is blank — an expectation-free cell can only catch crashes, never wrong results; (b) error when a cell produces no output (vacuous-probe guard), and require one deliberately-broken control cell to stay red so the harness proves it can detect failure. Institutionalizes the prebuilt-A/B-worktree pattern that made the session's regression attribution (HEAD vs `e700c4d4` vs main) take seconds instead of rebuilds. Canonical method: CLAUDE.md § matrix-first, step 2. |
