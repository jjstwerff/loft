...
| ~~Free-before-dependent-read overlay~~ (temporal extension of `--show-ownership`) | ~~S~~ **Shipped 2026-07-19** | The captured-group element-access UAF (`plans/captured-group-elem-uaf.md`) was INVISIBLE to the @PLN103 inspector: the static ownership verdict is temporal-agnostic (a correct free and a use-after-free render identically) and the runtime timeline tracks leaks, not reads-after-free. So the tool built to see store-lifetime bugs could not see this class. | `use_analysis::free_before_dependent_read` walks the committed IR: along each straight-line path, an `OpFreeRef(S)` followed by a DEREFERENCE of any TRANSITIVE view of `S` prints `⚠ UAF: …` under `--show-ownership`. Two axes make it precise — **transitive** deps (a nested `match arg{…}` derefs `arg` through an intermediate that only transitively views the store) and **deref-only** reads (a bare `Var` in a `return`/move is a safe delivery, not a UAF — clears the `__ret_N` return-hoist false positives). Verified: fires on `35m` + the repro, silent on 511/512 corpus scripts. Gate: `tests/introspect.rs::ownership_overlay_flags_free_before_dependent_read`. NOT covered: a bug whose ROOT is a dropped/missing dep (`35c`) — a dep-based check cannot see a dep that isn't there. |
