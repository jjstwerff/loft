...
| Item | Section | Status |
|---|---|---|
| **Bind a text-typed BRANCH → native miscompile** (interp correct; an accept/reject divergence) | this row | **OPEN, characterised 2026-07-10, boundary matrix below.**  Binding a branch that produces `text` to a local miscompiles on `--native` while `--interpret` is correct.  Not match-specific — `if`, `match` and `??` all fail, with three rustc masks: `q = match k { 1 => c0(), _ => "" }; q` → **E0716**; `q = if k == 1 { c0() } else { "" }; q` → **E0308**; `q = g(k) ?? "fb"; q` → **E0308**.  Trigger is the callee needing a caller-provided destination buffer (a COMPUTED text body); a literal-bodied callee returns an owned `String` and passes.  **Cells that PASS** (so the fix must not touch them): a `Var` value (format-string block borrowing a work-var that OUTLIVES the block); a `Call` value (single-wildcard match, plain `q = c0()`); a text SUBJECT match (passes only *by accident* — its `OpFreeText` on the subject copy lands after the value, so `has_trailing_void` is incidentally true and the block materialises).  **Mechanism:** each arm emits `&*(callee(…))`, a borrow of the `Str` temporary the callee returned; the temporary dies at the arm's `}`, so the consumer's `.to_string()` outside the block reads a dangling borrow.  `has_trailing_void` (`src/generation/emit.rs`) is a PROXY for "materialise this value", not the fact — the @P323 comment there already states the real invariant.  **Fix must be parser-side, not emit-side:** widening `has_trailing_void` to any branch-valued text block was TRIED and REVERTED — it materialises an inner block that is itself an if-ARM, so the arm yields `String` while its sibling yields `&str` (E0308, `tests/docs/29-match.loft`).  The correct shape is the one the TAIL case already uses: per-arm delivery into the destination (`push_text_arms_into`, the `do_if_acc` analogue), applied at the bind site.  **The differential-oracle corpus has NO cell for this** — add one WITH the fix, never before: a corpus cell added now makes the gate born-red, and a gate that has never been green cannot distinguish a regression from a birth defect ([ROADMAP § top picks](ROADMAP.md)).  Sibling of the tail-position bug fixed in `if_tail_yields_text` (guard: `tests/scripts/536-text-match-tail-buffer-callee.loft`). |
