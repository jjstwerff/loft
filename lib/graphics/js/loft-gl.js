...
export function initLoftGL(canvas) {
  ...
  Object.assign(window.loftHost, {
    ...
    save_png(path, w, h, data) {
      ...
      for (let i = 0; i < data.length; i++) {
        const px = data[i];
        img.data[i * 4 + 0] = (px >>> 16) & 0xff;
        img.data[i * 4 + 1] = (px >>> 8) & 0xff;
        img.data[i * 4 + 2] = px & 0xff;
        img.data[i * 4 + 3] = (px >>> 24) & 0xff;
      }
    ...
    // Font / text
    gl_load_font(_path) { return 0; },

    gl_measure_text(_fontIdx, text, size) {
      const c2 = document.createElement('canvas').getContext('2d');
      c2.font = `${size}px monospace`;
      return c2.measureText(text).width;
    },

    gl_text_height(_fontIdx, size) {
