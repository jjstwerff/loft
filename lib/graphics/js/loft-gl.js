...
export function initLoftGL(canvas) {
  ...
  Object.assign(window.loftHost, {
    ...
    // Live drawing-buffer size, in the same pixel space as gl_mouse_*.
    // Mirrors the native getters; screen-space UI (palette / button bounds)
    // anchors + hit-tests against these.
    gl_window_width() { return canvas.width; },
    gl_window_height() { return canvas.height; },
    ...
    // Initiative 03 Phase 4: runtime fullscreen toggle.  Browser
    // rules require a user gesture for `requestFullscreen` and the
    // gesture threshold is usually lower than for `createFullscreen`
    // because it's called from a keypress handler.  Exit fullscreen
    // doesn't need a gesture.
    gl_set_fullscreen(on) {
      if (on) {
