...
def tag_commit_date(ver: str) -> str:
    """When the tagged commit was made, as the index spells it (RFC3339, UTC, `Z`).

    This is the value the release workflow records (`git log -1 --format=%cI HEAD` on
    the tag's checkout), so an entry regenerated by hand matches the one the release
    attached byte for byte.  It refuses rather than guessing: a placeholder date would
    be a wrong answer the signature then attests, and nothing downstream could tell.
    """
    tag = f"v{ver}"
    try:
        out = subprocess.run(
    ...
    if not out:
        sys.exit(
            f"gen-toolchain-entry.py: cannot read the date of tag {tag} -- run inside a "
            "loft checkout that has the tag, or pass --published <RFC3339>"
        )
    return rfc3339_utc(out)


def rfc3339_utc(stamp: str) -> str:
    """`2026-09-04T23:56:26+00:00` (git's `%cI`) -> `2026-09-04T23:56:26Z`."""
    ...
    entry = {
        ...
        "versions": {
            ver: {
                ...
                "loft": ">=0",
                "published": rfc3339_utc(args.published) if args.published else tag_commit_date(ver),
                "binaries": binaries,
            }
        },
    }
