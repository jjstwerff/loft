...
1. **C integer width** (the plan's stated load-bearing question). loft `integer`
   is i64; C is `int` / `long` / `size_t`. `lc_i32` + `lc_neg_i32` decide whether
   the declaration must spell the C type (`#c "PQstatus" "int(void*)"`), because
   they fail loudly when it does not.
