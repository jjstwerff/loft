...
#[unsafe(no_mangle)]
pub extern "C" fn loft_ext_vec_sum(data_ptr: *const i64, data_count: u32) -> i64 {
...
#[unsafe(no_mangle)]
pub extern "C" fn loft_ext_vec_sum_f32(data_ptr: *const f32, data_count: u32) -> i64 {
    if data_ptr.is_null() || data_count == 0 {
        return -1;
    }
    let data = unsafe { std::slice::from_raw_parts(data_ptr, data_count as usize) };
...
loft_ffi::vec_wrapper!(n_ext_vec_sum_f32, loft_ext_vec_sum_f32(data: vec<f32>) -> i64);

// ── Pattern 4: Scalars before vector ─────────────────────────────────

/// offset + sum(data). C-ABI version.
#[unsafe(no_mangle)]
pub extern "C" fn loft_ext_offset_sum(offset: i64, data_ptr: *const i64, data_count: u32) -> i64 {
    if data_ptr.is_null() || data_count == 0 {
        return offset;
    }
    let data = unsafe { std::slice::from_raw_parts(data_ptr, data_count as usize) };
    eprintln!("[ext_offset_sum] offset={} count={}", offset, data.len());
    offset + data.iter().sum::<i64>() as i64
}

loft_ffi::vec_wrapper!(n_ext_offset_sum, loft_ext_offset_sum(offset: i64, data: vec<i64>) -> i64);

// ── Pattern 5: Vector between scalars ────────────────────────────────

/// a + sum(data) + b. C-ABI version.
#[unsafe(no_mangle)]
pub extern "C" fn loft_ext_sandwich_sum(a: i64, data_ptr: *const i64, data_count: u32, b: i64) -> i64 {
    if data_ptr.is_null() || data_count == 0 {
        return a + b;
    }
    let data = unsafe { std::slice::from_raw_parts(data_ptr, data_count as usize) };
    eprintln!("[ext_sandwich_sum] a={} count={} b={}", a, data.len(), b);
    a + data.iter().sum::<i64>() as i64 + b
}

loft_ffi::vec_wrapper!(n_ext_sandwich_sum, loft_ext_sandwich_sum(a: i64, data: vec<i64>, b: i64) -> i64);
...
#[unsafe(no_mangle)]
pub extern "C" fn loft_ext_struct_vec_len(data_ptr: *const i64, data_count: u32) -> i64 {
    eprintln!("[ext_struct_vec_len] count={}", data_count);
    let _ = data_ptr;
    data_count as i64
}

loft_ffi::vec_wrapper!(n_ext_struct_vec_len, loft_ext_struct_vec_len(data: vec<i64>) -> i64);

// ── Pattern 7: Vector inside loop with if ────────────────────────────

/// Same as vec_sum but with logging to detect store issues in loops.
#[unsafe(no_mangle)]
pub extern "C" fn loft_ext_loop_vec_sum(data_ptr: *const i64, data_count: u32) -> i64 {
    if data_ptr.is_null() || data_count == 0 {
...
loft_ffi::vec_wrapper!(n_ext_loop_vec_sum, loft_ext_loop_vec_sum(data: vec<i64>) -> i64);
