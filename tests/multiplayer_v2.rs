...
impl ServerGuard {
    ...
    fn wait_listening(&self) -> bool {
        wait_for_port(self.port, Duration::from_secs(5), 50)
    }
}
...
fn v2_single_client_completes_game() {
    ...
    let _server = {
        ...
        assert!(s.wait_listening(), "server failed to start within 5s");
        s
    };

    let client = spawn_client("S");
    let (out, status) = drain_with_timeout(client, Duration::from_secs(15));
...
fn v2_late_join_independent_games() {
    ...
    let a = spawn_client("A");
    let (a_out, a_status) = drain_with_timeout(a, Duration::from_secs(15));
    ...
    // already finished.
    let b = spawn_client("B");
    let (b_out, b_status) = drain_with_timeout(b, Duration::from_secs(15));
