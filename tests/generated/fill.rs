...
fn null_ref_sentinel(s: &mut State) {
    let new_value = DbRef { store_nr: u16::MAX, rec: 0, pos: 0 };
    s.put_stack(new_value);
}
...
fn set_ref(s: &mut State) {
    ...
    let v_val = *s.get_stack::<DbRef>();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; s.database.store_mut(&db).set_int(db.rec, db.pos + u32::from(v_fld), v_val.rec as i32);}
}

fn get_field(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = DbRef {store_nr: v_v1.store_nr, rec: v_v1.rec, pos: v_v1.pos + u32::from(v_fld)};
    s.put_stack(new_value);
}
...
fn get_int(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; if db.rec == 0 { i32::MIN } else { s.database.store(&db).get_int(db.rec, db.pos + u32::from(v_fld))} };
    s.put_stack(new_value);
}
...
fn get_character(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; if db.rec == 0 { char::from(0) } else { char::from_u32(s.database.store(&db).get_int(db.rec, db.pos + u32::from(v_fld)) as u32).unwrap_or(char::from(0)) } };
    s.put_stack(new_value);
}
...
fn get_long(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_long(db.rec, db.pos + u32::from(v_fld))};
    s.put_stack(new_value);
}
...
fn get_single(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_single(db.rec, db.pos + u32::from(v_fld))};
    s.put_stack(new_value);
}
...
fn get_float(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_float(db.rec, db.pos + u32::from(v_fld))};
    s.put_stack(new_value);
}
...
fn get_byte(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_byte(db.rec, db.pos + u32::from(v_fld), i32::from(v_min))};
    s.put_stack(new_value);
}
...
fn get_enum(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_byte(db.rec, db.pos + u32::from(v_fld), 0) as u8};
    s.put_stack(new_value);
}
...
fn set_enum(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; s.database.store_mut(&db).set_byte(db.rec, db.pos + u32::from(v_fld), 0, i32::from(v_val));}
}
...
fn get_short(s: &mut State) {
    ...
    let v_min = *s.code::<i16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; s.database.store(&db).get_short(db.rec, db.pos + u32::from(v_fld), i32::from(v_min))};
    s.put_stack(new_value);
}
...
fn get_text(s: &mut State) {
    let v_fld = *s.code::<u16>();
    let v_v1 = *s.get_stack::<DbRef>();
    let new_value = {let db = v_v1; let store = s.database.store(&db); Str::new(store.get_str(store.get_int(db.rec, db.pos + u32::from(v_fld)) as u32))};
    s.put_stack(new_value);
}
...
fn set_int(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; s.database.store_mut(&db).set_int(db.rec, db.pos + u32::from(v_fld), v_val);}
}
...
fn set_character(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; s.database.store_mut(&db).set_int(db.rec, db.pos + u32::from(v_fld), v_val as i32);}
}
...
fn set_long(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; s.database.store_mut(&db).set_long(db.rec, db.pos + u32::from(v_fld), v_val);}
}
...
fn set_single(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; s.database.store_mut(&db).set_single(db.rec, db.pos + u32::from(v_fld), v_val);}
}
...
fn set_float(s: &mut State) {
    ...
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; s.database.store_mut(&db).set_float(db.rec, db.pos + u32::from(v_fld), v_val);}
}
...
fn set_byte(s: &mut State) {
    ...
    let v_val = *s.get_stack::<i32>();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; s.database.store_mut(&db).set_byte(db.rec, db.pos + u32::from(v_fld), i32::from(v_min), v_val);}
}
...
fn set_short(s: &mut State) {
    ...
    let v_val = *s.get_stack::<i32>();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; s.database.store_mut(&db).set_short(db.rec, db.pos + u32::from(v_fld), i32::from(v_min), v_val);}
}
...
fn set_text(s: &mut State) {
    ...
    let v_val = s.string();
    let v_v1 = *s.get_stack::<DbRef>();
    {let db = v_v1; let s_val = v_val.str().to_string(); let store = s.database.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + u32::from(v_fld), s_pos as i32);}
}
...
fn remove_vector(s: &mut State) {
    ...
    let v_index = *s.get_stack::<i32>();
    let v_r = *s.get_stack::<DbRef>();
    let new_value = vector::remove_vector(&v_r, u32::from(v_size), v_index, &mut s.database.allocations);
    s.put_stack(new_value);
}
