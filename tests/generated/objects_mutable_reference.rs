...
fn n_add(stores: &mut Stores, mut var_r: &mut DbRef, mut var_val: i32) { //block_1: void
  ...
  if !((var_r).rec != 0) { //block_2: void
    ;
    } /*block_2: void*/ else {()};
  {let db = (var_r); stores.store_mut(&db).set_int(db.rec, db.pos + u32::from((0_i32)), (external::op_add_int(({let db = (var_r); if db.rec == 0 { i32::MIN } else { stores.store(&db).get_int(db.rec, db.pos + u32::from((0_i32)))} }), (var_val))));};
  let mut var__elm_1: DbRef = OpNewRecord(stores, var_r, 19_i32, 1_i32);
  {let db = (var__elm_1); stores.store_mut(&db).set_int(db.rec, db.pos + u32::from((0_i32)), (var_val));};
  OpFinishRecord(stores, var_r, var__elm_1, 19_i32, 1_i32);
...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = { //block_2: text["__work_1"]
    ...
    OpDatabase(stores, var_v, 19_i32);
    {let db = (var_v); stores.store_mut(&db).set_int(db.rec, db.pos + u32::from((0_i32)), (1_i32));};
    {let db = (var_v); stores.store_mut(&db).set_int(db.rec, db.pos + u32::from((4_i32)), (0_i32));};
    let _pre13 = ;
    ...
    { //Formatted string_3: text["__work_1"]
      ...
      OpFormatDatabase(stores, &var___work_1, var_v, 19_i32, 0_i32);
  ...
  let _pre15 = { //Formatted string_4: text["__work_2"]
    ...
    var___work_2 += " != \"{num:6,values:[2,3]}\"";
    &var___work_2
    } /*Formatted string_4: text["__work_2"]*/;
  n_assert(stores, (&var_test_value) == ("{num:6,values:[2,3]}"), _pre15, "mutable_reference", 17_i32);
  ;
  ;
  ;
  } /*block_1: void*/
