...
fn n_test(stores: &mut Stores) { //block_1: void
  ...
  let mut var_test_value = { //block_2: text["__work_1"]
    ...
    {let db = (stores.get_ref(&(vector::get_vector(&(DbRef {store_nr: (var_s).store_nr, rec: (var_s).rec, pos: (var_s).pos + u32::from((0_i32))}), u32::from((6_i32)), (0_i32), &s.database.allocations)), u32::from((0_i32)))); let s_val = ("bb").to_string(); let store = stores.store_mut(&db); let s_pos = store.set_str(&s_val); store.set_int(db.rec, db.pos + u32::from((0_i32)), s_pos as i32);};
  ...
  let _pre13 = { //Formatted string_4: text["__work_2"]
    ...
    var___work_2 += " != \"{n:\"bb\",v:12} v=12\"";
    &var___work_2
    } /*Formatted string_4: text["__work_2"]*/;
  n_assert(stores, (&var_test_value) == ("{n:\"bb\",v:12} v=12"), _pre13, "assign_text", 10_i32);
  ;
  ;
  ;
  } /*block_1: void*/
