...
# proof, made quantitative.  Asserts threaded frames >= 2x sequential and a
# smaller max inter-frame gap, with the value stable.  Exit 0 pass / 1 fail.
set -u
cd "$(dirname "$0")/../.."
...
run() { : > "$REPORT"; timeout 40 "$CHROME" --headless=new --no-sandbox --disable-gpu \
  "http://127.0.0.1:$PORT/par-ui-responsive.html?threads=$1&work=$WORK&elems=$ELEMS&window=$WINDOW" >/dev/null 2>&1 & local ch=$!; sleep $(( WINDOW/1000 + 6 )); kill $ch 2>/dev/null; head -1 "$REPORT"; }
echo "== UI responsiveness (heavy par per rAF frame, window=${WINDOW}ms) =="
TH="$(run 8)"; echo "  threaded:   $TH"
...
[ "${tf:-0}" -ge $(( sf * 2 )) ] 2>/dev/null || { echo "FAIL: threaded frames ($tf) < 2x sequential ($sf)"; fail=1; }
