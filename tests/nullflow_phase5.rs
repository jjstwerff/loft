...
const ABS_NULL: &str = "struct S{f:integer} fn main(){ y=0; n=10/y; s=S{f:0}; s.f=abs(n); print(\"r={s.f}\\n\"); }\n";

#[test]
fn max_null_propagates_off_via_guard() {
    let (ok, out, _w) = run(MAX_NULL, "--interpret", false, "max_off");
    assert!(ok, "{out}");
    assert!(out.contains("r=-1"), "OFF: max(null,5) must be null (guard), not 5: {out}");
}
#[test]
...
fn max_null_propagates_on_native() {
    ...
    assert!(ok, "{out}");
    assert!(out.contains("r=-1"), "ON native: max(null,5) must be null: {out}");
}
#[test]
fn abs_nullable_off_launders_no_warning() {
    let (ok, _o, warns) = run(ABS_NULL, "--interpret", false, "abs_off");
    assert!(ok);
    assert_eq!(warns, 0, "OFF: abs(nullable) is not yet propagated (unchanged)");
}
#[test]
fn abs_nullable_on_propagates_and_guards_interpret() {
    let (ok, out, warns) = run(ABS_NULL, "--interpret", true, "abs_on_i");
    assert!(ok, "{out}");
    assert_eq!(warns, 1, "ON: abs(nullable) is integer? → store warns: {out}");
    assert!(out.contains("r=null"), "ON: abs(null) must be null (guard, not overflow): {out}");
}
#[test]
fn abs_nullable_on_propagates_and_guards_native() {
    ...
    assert!(out.contains("r=null"), "ON native: abs(null) must be null: {out}");
}
#[test]
fn non_null_args_unaffected() {
    let (ok, out, warns) = run("fn main(){ print(\"r={max(3, 7)}\\n\"); }", "--interpret", true, "nn");
    assert!(ok, "{out}");
    assert_eq!(warns, 0);
